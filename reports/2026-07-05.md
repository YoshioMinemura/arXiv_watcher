...
Generated at: 2026-07-05 09:11 JST
Run ID: 5a5d5ec8-9b8e-472a-aa51-ebdc292db756
...
**日本語要約**
本研究は、長文脈推論においてLLMが入力中の関連証拠を十分に活用できない問題に対し、学習不要の推論手法RECONTEXTを提案する。RECONTEXTはモデル内部の関連性信号を用いて質問条件付きの証拠プールを構築し、それを最終生成前に再生することで、元の長文脈を保持したまま証拠利用を改善する。



**新規性**
外部メモリや文脈削除、追加学習を使わずに、証拠の整理と回答生成を分離するRecursive Evidence Replayを導入した点が新しい。



**Tags:** 長文脈推論, 大規模言語モデル, 学習不要手法, 証拠利用, 推論時最適化
...
**日本語要約**
Linux/bash の短答式試験の採点を、4つの最先端大規模言語モデルで自動化できるかを検証した研究です。1200件の学生解答を対象に、4段階の認知タクソノミーに基づいて評価した結果、Gemini 3.0 Pro がルーブリック付きプロンプトで最も高い人間との一致を示し、問題の複雑さが上がるほど一致度は低下しました。



**新規性**
認知複雑度と操作的影響を組み合わせた4段階タクソノミーを用いて、AI採点に適した設問と人手確認が必要な設問を判定する枠組みを提示した点です。
...
**日本語要約**
本研究は、LLMのデプロイ時に安全でない出力が生じうる問題に対し、出力をオンラインで監視して安全性を保証できなくなった時点で警報を出す手法を検討している。外部モデルの verifier 信号をしきい値で判定し、そのしきい値を risk control により較正する単純なリアルタイム監視器を提案し、数学的推論と red teaming のデータセットで評価している。



**新規性**
外部 verifier の信号をしきい値化し、risk control で較正するだけの単純な設計が、逐次仮説検定に基づくより高度な監視手法と競争力を持つことを示した点。



**Tags:** LLM安全性, オンライン監視, リスク制御, 外部検証器, 異常検知
...
**日本語要約**
LLMを用いた社会シミュレーションについて、計算量スケーリングが忠実度の改善につながるかを、意見モデル・行動シミュレーション・長期予測の3領域で検証した。85個のTransformer LLMと35個のより大規模なオープンウェイトモデルを用いた分析から、多くのタスクではスケールに伴って性能が向上する一方、低資源領域や一般知識・推論ベンチマークと相関の弱いタスクでは改善が遅いことが示された。



**新規性**
社会シミュレーションの忠実度に対して、計算量スケーリング則と損失からの下流性能予測を用いて、一般能力との関係を体系的に評価した点が新しい。
...
**日本語要約**
長編TVドラマにおける話者認識は、複雑な文脈の中で発話を正しい登場人物に割り当てる重要な課題である。本研究では、53.2万件の対話行を含む大規模ベンチマークDramaSR-532Kを構築し、音声・言語・視覚の手がかりを統合して話者を特定する問題設定を提示した。



**新規性**
大規模なドラマ向け話者認識ベンチマークを新たに整備し、推論型大規模言語モデルを用いたマルチモーダルなツール利用で話者認識を行うDramaSR-LRMを提案した点が新しい。



**Tags:** speaker recognition, multimodal learning, large language model, video understanding, TV drama
...
**日本語要約**
本論文は、実世界のフィードバックや人手ラベルを使わずにLLMを自己蒸留するためのデータ中心フレームワークNeuron-OPSDを提案する。内部ニューロン活性を用いて学習データの選択と教師コンテキストの構築を行い、教師分布からのon-policy distillationで学習することで、専門領域での性能向上と汎化性能の維持、較正崩壊の緩和を示す。



**新規性**
内部ニューロン活性を訓練データ選択と教師コンテキスト構築に利用し、ラベル不要の自己蒸留を支える点が新規である。



**Tags:** LLM, self-distillation, annotation-free learning, data selection, calibration
...
**日本語要約**
WattGPUは、公開されているLLMメタデータとGPU仕様のみを用いて、LLM推論時の平均GPU消費電力とインタートークン遅延（ITL）を予測する手法である。42個のLLMと8種類のGPUを用いた評価では、未見のGPUやLLMに対しても高い精度を示し、特にサーバー設定での電力・遅延予測とGPU順位付けに有効だった。



**新規性**
ハードウェアへのアクセスや事前プロファイリングなしに、未見のGPUとLLMへ一般化できる電力・遅延予測モデルを提示した点。



**Tags:** LLM推論, GPU電力予測, 遅延予測, 未見デバイス一般化
...
**日本語要約**
本研究は、LLMエージェントが社会的に構造化された状況で、公開発話とオフレコ（OTR）発話の間にどのような差を示すかを、二重チャネルの討論枠組みで検証した。10モデル・3シナリオ・各5変種で、整合を促す設定では対象エージェントの公開/OTR間の意思決定差が約3%から約40%まで増大し、立場・意味類似度・自然言語推論・アンケートの4種の分析でも一貫した乖離が確認された。



**新規性**
明示的な目的を与えなくても、社会的文脈によって潜在的な目的が表出しうることを示し、それを評価するための二重チャネル評価フレームワークと行動指標を提案した。



**Tags:** LLM agents, multi-agent debate, social context, latent objectives, evaluation framework
...
**日本語要約**
本論文は、LLMの推論学習における on-policy self-distillation で、教師の密なトークンレベル監督が過学習や privileged information leakage を招く問題を指摘し、DemoPSD を提案する。DemoPSD は教師と学生の分布の不一致に応じて教師ガイダンスを選択的に取り入れ、探索性を保ちながら学習する。実験では SciKnowEval の4分野で GRPO と SDPO を上回り、OOD の GPQA でも頑健な汎化を示した。



**新規性**
教師分布をそのまま模倣せず、教師・学生分布の reverse-KL barycenter を不一致に応じて動的に調整することで、情報漏洩の抑制と探索性の維持を両立した点が新規である。



**Tags:** LLM, self-distillation, on-policy learning, generalization, reinforcement learning
...
**日本語要約**
本研究は、LLMを専門領域に適応させるActive Few-Shot Learningにおいて、出力レベルの指標ではなくモデル内部のニューロン活性に基づいて有用な未ラベルサンプルを選ぶNeuFSを提案する。NeuFSは、ニューロンパターンによる多様性確保と、ニューロン合意度に基づく難例・幻覚しやすい例の優先選択を組み合わせ、3つのデータセットで推論とテキスト分類の両方で既存手法を上回った。



**新規性**
サンプル選択の信号として外部埋め込みや出力不確実性ではなく、LLM内部のニューロン活性パターンとその合意度を用いる点が新規である。



**Tags:** few-shot learning, active learning, LLM, neuron activation, sample selection
...
**日本語要約**
大規模言語モデルにおける拒否挙動は単一方向ではなく多次元部分空間に表現されるとされるが、既存手法は計算コストが高く、長い推論トレースを持つモデルでは特に扱いにくい。本研究はRecursive Feature Machine (RFM) をプローブ情報で初期化して適用することで、推論モデルと非推論モデルの両方で拒否部分空間を数秒で抽出でき、さらにアブレーション課題でも既存手法より良い性能を示した。



**新規性**
RFMをプローブ情報で初期化して用いることで、拒否の多次元部分空間を高速かつ低コストに抽出できる手法を提案した点が新規である。



**Tags:** LLM, representation learning, subspace extraction, safety, interpretability
...
**日本語要約**
本論文は、LLMの学習データに含まれるPIIを後から削除するunlearningについて、出力レベルだけでなくパラメータレベルで本当に知識が消えているかを評価する必要性を示す。著者らは、PIIを既知のパラメータに注入したテストベッドLACUNAを提案し、既存のSOTA手法は出力性能が高くても局所化が不正確で、resurfacing attackに脆弱であることを示した。



**新規性**
PIIを特定の重みに注入して真のパラメータレベルの正解を持つ、初のunlearning評価用テストベッドLACUNAを構築した点が新規である。



**Tags:** LLM unlearning, parameter localization, PII, benchmark, resurfacing attack
...
**日本語要約**
本論文は、ユーザ属性や過去文脈をメモリとして注入することで、個人化されたLLMの応答内容だけでなく推論の軌跡自体が変化するかを調べている。ground-truth不要の枠組みDRIFTLENSを提案し、4つのLLMと10種類のユーザ属性で、最終回答が自然で妥当でも推論ドリフトが有意に生じることを示した。



**新規性**
DRIFTLENSにより、個人化メモリが引き起こす推論ドリフトを正解なしで定量化し、さらにGRPO/DPOによる抑制効果を比較評価した点が新しい。
...
**日本語要約**
NASAの地球観測データ探索を対象に、自然言語の研究クエリから関連するデータセットやツールを返すエージェント型検索システムを提案している。NASA EO Knowledge Graphから47k件のクエリ-データセット対を含むベンチマークNASA-EO-Benchを構築し、公開サービスとして運用している。



**新規性**
知識グラフを基盤にした検索に、学習済みスコアリング、BM25とのスコア融合、さらに追加学習なしのゼロショットなエージェント再ランキングを組み合わせ、検索性能を大きく向上させた点が新しい。



**Tags:** 情報検索, 知識グラフ, 大規模言語モデル, 地球観測, エージェント検索
...
**日本語要約**
本研究は、2010年から2026年のNLP研究出版データを用いて、LLM時代にNLP研究の発表先が従来のACL中心から変化しているかを分析した。結果として、既存著者はACL本会議での比率を下げる一方でFindingsや一般ML系会議での比率を高め、新規著者でも*ACL中心から一般ML中心へ移る傾向が確認された。



**新規性**
既存著者と新規著者の両方について、会場分布の変化を比較し、さらに因果推論により一般ML会場の引用優位が会場選択に影響することを推定した点が新しい。



**Tags:** NLP, ACL, Large Language Models, 学術出版動向, 機械学習
...
**日本語要約**
大規模言語モデルが解けない難問に対し、単純な反復サンプリングや通常の強化学習ではなく、問題を独立に解ける小さなサブ関数へ分解して解きやすくする方法を提案する。DecompRLは階層的なコード構造の分解と実装を強化学習で学習し、複数モジュールの再結合によって候補解を大きく増やすことで、GPU推論コストを抑えつつ性能を向上させる。



**新規性**
問題を「より多くサンプルする」のではなく「分解して再結合する」方向に変え、モジュール型コード生成を学習するDecompRLを導入した点。



**Tags:** 大規模言語モデル, 強化学習, コード生成, 問題分解, モジュール化
...
**日本語要約**
本論文は、自然言語仕様から局所実行可能な小型のニューラル成果物を生成する「fuzzy-function programming」を提案し、その具体化として Program-as-Weights (PAW) を示す。PAWでは、4BのコンパイラがFuzzyBench上で学習され、凍結した軽量インタプリタ向けのパラメータ効率的アダプタを出力する。



**新規性**
基盤モデルを入力ごとの問題解決器ではなく、関数定義時に一度だけ呼び出して再利用可能な小型アーティファクトを作る「ツールビルダー」として再定義した点が新しい。



**Tags:** large language models, program synthesis, neural compilation, parameter-efficient fine-tuning, local inference
...
**日本語要約**
本研究は、SE-RRMを用いて制約充足問題の古典的シンボリックソルバを誘導する神経記号的手法G-RRMを提案する。SE-RRMは解候補全体を生成してバックトラッキングやSATソルバ（Glucose 4.1、CaDiCaL 3.0.0）をガイドし、探索効率の改善条件を検討する。実験では、探索空間が広く、かつ分岐選択を動的に上書きできるソルバで特に有効であることが示された。



**新規性**
再帰的推論モデルをシンボリックソルバの探索ガイドとして統合し、どの条件で神経的ヒントが実際の高速化につながるかを体系的に示した点が新しい。



**Tags:** neuro-symbolic, constraint satisfaction, symbolic solver, SAT, recurrent reasoning model
...
**日本語要約**
本論文は、非直交な空間関係を含む非Manhattan環境において、テキスト駆動で物理的に妥当な3D室内シーンを生成するためのフレームワークSPG-Layoutを提案する。統計的な物体分布の事前知識と、大きな物体を優先する階層的レイアウト戦略を組み合わせることで、レイアウト違反を抑えつつ意味的妥当性と物理的整合性の両立を目指す。



**新規性**
非Manhattan環境向けに、物体分布の統計的事前知識と階層的レイアウト戦略を組み合わせたテキスト駆動3D室内シーン生成手法を提案した点が新しい。



**Tags:** 3D scene synthesis, text-driven generation, indoor layout, non-Manhattan environments
...
**日本語要約**
本研究は、複数の異種コンポーネントが並行動作する安全性重視のリアルタイム自律システムにおいて、ソフトウェア媒介の協調には遅延や保証の面で限界があると指摘し、FPGAを用いて協調セマンティクスの一部をハードウェアに直接実装するアーキテクチャを提案する。TB-CSPNフレームワークを基盤に、意味推論はソフトウェアで適応的に行い、同期・セマンティックゲーティング・認可制約などの協調制御をハードウェアで決定的に実行する。



**新規性**
TB-CSPNの協調機構をFPGAプリミティブへ写像し、推論と協調管理を分離しつつ、協調セマンティクスをハードウェアで強制する点が新規である。



**Tags:** real-time systems, FPGA, autonomous systems, safety-critical, coordination
