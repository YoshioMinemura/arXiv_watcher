...
Generated at: 2026-09-02 09:59 JST
Run ID: 8a7e1880-87be-4788-9fd3-ef31ad98de3b
...
- PDF: [link](https://arxiv.org/pdf/2608.31100.pdf)
- Published: 2026-09-01 02:05 JST
- Updated: 2026-09-01 02:05 JST
...
**日本語要約**
S3Gymは、LLMが外部環境での経験を自らテストし、評価し、その経験を次の意思決定に活かせるかを調べるための対話型ベンチマークである。7つのテキストベースゲームと実行可能な環境検証器を用い、履歴をそのまま使う方法、要約メモを条件にする方法、パラメータ学習の3経路を比較している。



**新規性**
Self-Testing、Self-Judging、Self-Improvementを一体化した評価枠組みを提案し、経験の取り込み方がタスク構造によって大きく異なることを示した。



**Tags:** LLM, self-improvement, benchmark, interactive evaluation, text-based games
...
- PDF: [link](https://arxiv.org/pdf/2608.31137.pdf)
- Published: 2026-09-01 02:44 JST
- Updated: 2026-09-01 02:44 JST
...
**日本語要約**
本論文は、異なる種類のオントロジーアライメント手法の予測を、投票ベースの融合と事後選択ポリシーからなる2段階の仕組みで統合する OntoAligner-Ensemble を提案する。8つのベンチマーク課題で評価し、アンサンブル化により精度と再現率のバランスが改善し、単独アライナを上回ることが多いと報告している。



**新規性**
異種のアライメント手法をアライナ非依存に統合し、投票融合と選択ポリシーを組み合わせた再現可能なアンサンブル枠組みを提示した点が新しい。



**Tags:** ontology alignment, ensemble learning, voting fusion, LLM, knowledge graph embedding
...
- PDF: [link](https://arxiv.org/pdf/2608.31068.pdf)
- Published: 2026-09-01 01:43 JST
- Updated: 2026-09-01 01:43 JST
...
**日本語要約**
本論文は、推論ベンチマークで大規模言語モデルが誤答しても、内部表現には正しい答えの情報が残っている場合があることを示します。特に、sequence scoring が構造的バイアスで崩壊していても、hidden-state probe では正答を復元できることを複数のベンチマークで確認しています。



**新規性**
ラベル不要の最小限の加法補正を用いる診断手法を提案し、少数の未ラベル例だけで崩壊した sequence score から 9〜34 ポイントの精度回復を実証した点が新規です。



**Tags:** 大規模言語モデル, 推論, 内部表現, ベンチマーク評価
...
- PDF: [link](https://arxiv.org/pdf/2608.31082.pdf)
- Published: 2026-09-01 01:53 JST
- Updated: 2026-09-01 01:53 JST
...
**日本語要約**
本論文は、Webページや報告書、契約書などの非構造データに対して、LLMエージェントが質問応答を行う際の高コストを問題にしている。提案手法のagentic data crackingは、推論のために文書を開いたタイミングで、関連しそうな構造化情報を副次的に抽出・蓄積し、将来の質問を文書再読込なしで処理できるようにする。FanOutQAでは、関連質問を1つ追加した条件でコストを53%削減しつつ精度を維持した。



**新規性**
推論時に文書を読むたび、既存コンテキストから低追加コストで構造化を“適応的かつ推測的”に進め、将来の質問に備える点が新規である。



**Tags:** LLMエージェント, 非構造データ, 情報抽出, コスト削減, データ構造化
...
### 5. Evidence-Bounded Mental Health Reasoning from Heterogeneous Speech Protocols
- arXiv: [2608.31014](https://arxiv.org/abs/2608.31014)
- PDF: [link](https://arxiv.org/pdf/2608.31014.pdf)
- Published: 2026-09-01 00:58 JST
- Updated: 2026-09-01 00:58 JST
...
**日本語要約**
本研究は、音声・テキストを用いたメンタルヘルス判定において、自由面接や音読課題など異なる臨床プロトコルが持つ証拠の境界を明示的に扱う「evidence-bounded reasoning」として問題を再定式化した。6種類の異種ソースからなるEvidence Package Benchmarkを構築し、プロトコルに応じて推論範囲を制御するEviBoundを提案して、抑うつ判定で高いAUROCとゼロの主張違反を示した。



**新規性**
異なる臨床音声プロトコルの証拠許可を明示的に制御し、境界違反を抑えるプロトコル認識型の推論枠組みEviBoundを提案した点が新規である。



**Tags:** mental health screening, speech protocols, multimodal reasoning, evidence-bounded reasoning, clinical NLP
...
### 6. A Model with No Head and Many Thoughts
- arXiv: [2608.31069](https://arxiv.org/abs/2608.31069)
- PDF: [link](https://arxiv.org/pdf/2608.31069.pdf)
- Published: 2026-09-01 01:45 JST
- Updated: 2026-09-01 01:45 JST
...
**日本語要約**
本論文は、言語モデルの推論時に大きな語彙ヘッドを使う代わりに、軽量なプロジェクタで埋め込み空間上の自己回帰ロールアウトを行う Soft Latent Thinking を提案する。これにより、推論ステップを離散トークン化せず連続表現のまま進められ、chain-of-thought の各ステップ計算量を削減できる。DeepSeek-Qwen-1.5B と LLaMA-3.2-3B で、pass@k が一貫して改善した。



**新規性**
推論時の LM head を軽量プロジェクタに置き換え、離散トークン生成なしに埋め込み空間で連続的に思考させる点が新しい。



**Tags:** large language models, reasoning, embedding space, efficient inference, chain-of-thought
...
### 7. LLM Judges Verify Presence, Not Absence: Omission Blindness in AI Clinical Notes and What Recovers It
- arXiv: [2608.31016](https://arxiv.org/abs/2608.31016)
- PDF: [link](https://arxiv.org/pdf/2608.31016.pdf)
- Published: 2026-09-01 00:59 JST
- Updated: 2026-09-01 00:59 JST
...
**日本語要約**
本研究は、AI臨床ノートの主要な誤りである「欠落（omission）」に対して、従来のLLM judgeが情報の存在は検出できても不在は見落としやすいことを示した。500件の単一誤りペアを用いた評価では、追加・改変は高精度に判別できる一方、欠落の検出は偶然水準に近く、単独ノートでも信頼して欠落を指摘できなかった。



**新規性**
欠落検出を「ノートを直接判定する」方式ではなく、「転写で確立された事実を列挙し、各事実がノートにあるか確認する」形に再構成することで回復できることを示し、パイプライン方式と単一プロンプト方式の2手法を提示した。



**Tags:** LLM judge, clinical notes, omission detection, benchmark, prompt optimization
...
### 8. DIASENTINEL: An Auditable Multi-Agent System for Guideline-Grounded Diabetes Risk Screening
- arXiv: [2608.31128](https://arxiv.org/abs/2608.31128)
- PDF: [link](https://arxiv.org/pdf/2608.31128.pdf)
- Published: 2026-09-01 02:40 JST
- Updated: 2026-09-01 02:40 JST
...
**日本語要約**
DIASENTINELは、電子カルテから1年以内の2型糖尿病リスクをスクリーニングし、ADAガイドラインに基づく報告書を生成する、完全オンプレミスのマルチエージェントシステムです。校正済みリスク予測、決定的な臨床シグナル抽出、ガイドライン検索、ルールベース検証とLLM推論を組み合わせ、引用付きの説明と検証結果を提示します。



**新規性**
LLMの臨床意思決定支援に対して、ガイドライン準拠・監査可能性・プライバシー保護を同時に満たす実用的な多段検証フレームワークを提示した点が新規です。



**Tags:** clinical decision support, large language models, diabetes risk screening, electronic health records, guideline grounding
...
- PDF: [link](https://arxiv.org/pdf/2608.31075.pdf)
- Published: 2026-09-01 01:48 JST
- Updated: 2026-09-01 01:48 JST
...
**日本語要約**
本論文は、Large Reasoning Models（LRMs）を人間の監督が徐々に減る状況でも改善し続ける方法を、報酬と経験の2軸から整理している。人間評価から検証器・再利用可能な報酬への移行や、人間作成の課題から自己生成カリキュラム・自律的な共進化への移行を、L0〜L4の5段階で体系化し、関連するリスクと評価観点もまとめている。



**新規性**
人間監督の縮小を前提に、報酬軸と経験軸を統合したL0〜L4の階層フレームワークでLRMのスケーリングを整理した点が新規である。



**Tags:** large reasoning models, reinforcement learning with verifiable rewards, human supervision, self-generated curricula, superintelligence
...
### 10. BLOOM-WILT: Logit Tilting for Behaviour Elicitation in Automated LLM Auditing
- arXiv: [2608.31105](https://arxiv.org/abs/2608.31105)
- PDF: [link](https://arxiv.org/pdf/2608.31105.pdf)
- Published: 2026-09-01 02:10 JST
- Updated: 2026-09-01 02:10 JST
...
**日本語要約**
BLOOM-WILTは、稀な望ましくない振る舞いを自然なマルチターン対話として引き出すための自動LLM監査パイプラインです。監査モデルが過去のスコア付き対話から方策を更新し、同時にターゲットモデルの次トークン分布を用いてデコードを適応的に再重み付けすることで、訓練コストや追加アクセスなしにサンプル効率を改善します。



**新規性**
入力側の対話戦略更新と出力側のログイット調整を組み合わせ、ターゲットの次トークン分布だけを使って稀な振る舞いの誘発を高効率化した点が新規です。



**Tags:** LLM監査, 行動誘発, サンプリング効率, 安全性評価, 対話生成
...
### 11. Improving Information Extraction with Learned Queries
- arXiv: [2608.31058](https://arxiv.org/abs/2608.31058)
- PDF: [link](https://arxiv.org/pdf/2608.31058.pdf)
- Published: 2026-09-01 01:35 JST
- Updated: 2026-09-01 01:35 JST
...
**日本語要約**
本論文は、情報抽出の性能向上には抽出モデル自体の改善だけでなく、情報を引き出すための質問設計が重要であることを示す。4つの臨床ベンチマークと5つのLLMで、質問設計の改善だけでF1が18.6ポイント向上し、より大きな抽出モデルを使う場合を上回ることを報告している。



**新規性**
文書ごとの質問集合を生成するLoQと、抽出結果に基づいて質問を反復改善するFeedQを提案し、最適化された質問を学習可能な対象として扱えるようにした。



**Tags:** information extraction, question generation, large language models, clinical NLP, prompt optimization
...
- PDF: [link](https://arxiv.org/pdf/2608.31076.pdf)
- Published: 2026-09-01 01:48 JST
- Updated: 2026-09-01 01:48 JST
...
**日本語要約**
自律的な科学研究エージェントは、曖昧な研究指示に対して必要な分析や成功基準を見落とすことがある。本論文は、実行前にタスク固有の実行可能なルーブリックを自動生成し、それを用いて実行・検証・反復修正を行う評価先行フレームワークAutoSciRubを提案する。



**新規性**
曖昧な研究タスクを、文献とデータに基づく原子的な科学目標へ分解し、検証可能なルーブリックとして先に明示してから研究を進める点が新しい。



**Tags:** 自律研究エージェント, ルーブリック生成, 評価先行, 科学ワークフロー, 反復修正
...
### 13. LLM Post-Training as Brownfield Maintenance: An Industrial Perspective on Dataware Engineering
- arXiv: [2608.31102](https://arxiv.org/abs/2608.31102)
- PDF: [link](https://arxiv.org/pdf/2608.31102.pdf)
- Published: 2026-09-01 02:08 JST
- Updated: 2026-09-01 02:08 JST
...
**日本語要約**
本論文は、LLMのpost-trainingを「既に配備されたチェックポイントを保守するブラウンフィールド作業」と捉え、固定された計算資源と混合比制約の下で、既存性能を崩さずに局所的改善を行う工学的課題を整理している。特に、ゼロサムなmixture設計、yieldを主要指標とすること、そして不確実性下でのエンドツーエンド統合という3つの課題を、産業的なコード生成改善事例を通じて論じている。



**新規性**
post-trainingを「datawareの保守」として位置づけ、bounded mixture patchesとyield中心の設計という産業的な保守フレームワークを提示した点が新規である。



**Tags:** LLM post-training, dataware engineering, code generation, industrial perspective, mixture design
