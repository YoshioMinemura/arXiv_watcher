...
Generated at: 2026-07-10 09:16 JST
Run ID: df3d491f-9274-46fb-99e8-a3b785fbb368

## Summary
- Queries executed: 1
- New matched papers: 20
...
- PDF: [link](https://arxiv.org/pdf/2607.07626.pdf)
- Published: 2026-07-09 01:43 JST
- Updated: 2026-07-09 01:43 JST
- Authors: Sahil Kale
...
**日本語要約**
本研究は、LLMの信頼度推定を時間的観点から分析し、回答前のFOKよりも回答後のJOLの方が一貫して較正性と識別性に優れることを示した。さらに、隠れ表現に対する線形プローブは、モデルが明示的に言語化するよりも豊かな信頼度情報を捉えられることを明らかにした。



**新規性**
回答後の正解性プローブが出力する信頼度を教師信号として、回答前の隠れ表現から信頼度を予測する future confidence distillation を提案し、低コストで較正改善を実現した。



**Tags:** large language models, confidence estimation, calibration, distillation, hidden representations
...
- PDF: [link](https://arxiv.org/pdf/2607.07669.pdf)
- Published: 2026-07-09 02:24 JST
- Updated: 2026-07-09 02:24 JST
...
**日本語要約**
DiaLLMは、英語方言の理解（robustness）と生成の間にギャップがあることを検証するため、3つのオープンウェイト言語モデル系列を対象に、オーストラリア英語・インド英語・北部イギリス英語への継続事前学習と複数の事後学習手法を比較した。結果として、ベンチマーク上の性能は継続事前学習やSFTの影響を強く受ける一方、生成の方はアライメント手法によって大きく変化し、両者は一致しないことが示された。



**新規性**
方言の頑健性と生成を切り分け、継続事前学習・SFT・複数のアライメント戦略を統制比較した初の研究として、評価指標では捉えにくい生成品質とのギャップを明らかにした。



**Tags:** 大規模言語モデル, 方言適応, 継続事前学習, アライメント, 英語方言
...
- PDF: [link](https://arxiv.org/pdf/2607.07469.pdf)
- Published: 2026-07-08 23:32 JST
- Updated: 2026-07-08 23:32 JST
...
**日本語要約**
本論文は、EC向け属性値抽出のために、12,726商品・229商品タイプ・792属性・4言語を含む大規模な人手検証済みベンチマークSynthAVEを提示する。人手ラベルの代替として、21種類のLLM判定器によるマルチLLMアリーナで各サンプルを独立評価し、多数決で最終ラベルを決定することで、品質を保ちながら大規模な合成ラベル検証を可能にした。



**新規性**
複数のLLM判定器を組み合わせた多数決ベースの検証枠組みにより、合成ラベルを人手レビューに近い精度で大規模に品質管理できることを示した点が新規である。



**Tags:** attribute extraction, synthetic labeling, LLM validation, e-commerce, benchmark
...
- PDF: [link](https://arxiv.org/pdf/2607.07494.pdf)
- Published: 2026-07-08 23:55 JST
- Updated: 2026-07-08 23:55 JST
...
**日本語要約**
LLM事前学習における勾配通信は大きなボトルネックであり、FP8やNVFP4のような低精度化は通信量を減らせる一方、ユークリッド空間での量子化は異方的な勾配に対して性能劣化を招くことがある。GIFTは勾配を幾何学的にほぼ等方な座標系へ変換してから低精度通信を行う手法で、Llama-300M/600Mで収束を検証し、Llama-600Mの事前学習時間を64台のNVIDIA GH200上で7.6%短縮した。



**新規性**
オプティマイザや通信方式、低精度フォーマットを変えずに、勾配通信の座標系だけを幾何学的に変換して低精度表現の忠実度を高める点が新しい。



**Tags:** LLM事前学習, 勾配通信, 低精度量子化, 幾何学的変換, 分散学習
...
- PDF: [link](https://arxiv.org/pdf/2607.07508.pdf)
- Published: 2026-07-09 00:02 JST
- Updated: 2026-07-09 00:02 JST
...
**日本語要約**
本論文は、長いホライズンを持つエージェント型タスクにおける大規模言語モデルの強化学習を対象に、同期的でバッチ交互型の従来手法の非効率性と、非同期学習における安定性・オフポリシー問題を扱う。提案手法SAOは、グループ単位のサンプリングを1プロンプト1ロールアウトの単一ロールアウトに置き換え、さらに価値モデル学習設計と厳密な両側トークンレベルクリッピングを導入することで、安定した学習とベンチマーク性能の向上を示す。



**新規性**
非同期エージェント型RLにおいて、グループサンプリングの代わりに単一ロールアウトを用い、両側トークンレベルクリッピングで安定性を高めた点が新規である。



**Tags:** reinforcement learning, large language models, asynchronous optimization, agentic RL, stability
...
- PDF: [link](https://arxiv.org/pdf/2607.07690.pdf)
- Published: 2026-07-09 02:49 JST
- Updated: 2026-07-09 02:49 JST
...
**日本語要約**
本論文は、検証可能な報酬に基づく強化学習では最終回答のみが評価され、推論過程そのものが十分に学習されないという問題を指摘する。提案手法Agonでは、2つの競合モデルが互いの解答を読みながら同じ問題を解き、相手よりよく推論できたかどうかを暗黙的に評価することで、プロセスラベルや報酬モデルなしに推論能力を鍛える。



**新規性**
2つのモデルを互いの採点者として競わせることで、単一モデルRLでは得にくい、より強い相手に対する継続的な競争学習を実現した点が新しい。



**Tags:** reinforcement learning, reasoning models, competitive training, large language models
...
### 7. CARLA-GS: Decoupling Representation, Reasoning, and Physics Simulation for Autonomous Driving Corner-Case Synthesis
- arXiv: [2607.07601](https://arxiv.org/abs/2607.07601)
- PDF: [link](https://arxiv.org/pdf/2607.07601.pdf)
- Published: 2026-07-09 01:20 JST
- Updated: 2026-07-09 01:20 JST
...
**日本語要約**
本研究は、自動運転の安全評価に向けて、危険な相互作用を含むコーナーケースをフォトリアルかつ物理的に整合した形で生成するためのモジュール型パイプラインCARLA-GSを提案する。実走行データから編集可能なガウス表現のシーンを再構成し、LLMによるシーン推論で意図レベルの軌道を生成し、CARLA上のPID制御で実行することで、意味的意図と物理実現性を両立する。Waymo Open Datasetでの実験により、制御可能で時空間的一貫性のあるコーナーケース生成が示された。



**新規性**
視覚表現、シーン推論、物理シミュレーションを分離しつつ連結することで、意味的に妥当で物理的にも実行可能なコーナーケース生成を単一枠組みで実現した点が新規である。



**Tags:** autonomous driving, corner-case synthesis, simulation, LLM, photorealistic rendering
...
- PDF: [link](https://arxiv.org/pdf/2607.07702.pdf)
- Published: 2026-07-09 02:57 JST
- Updated: 2026-07-09 02:57 JST
...
**日本語要約**
長期タスクを行うエージェントの失敗トレースは冗長で異質なため、そのままでは最適化に使いにくいという問題に対し、STRACEは失敗パターンの抽出と因果的な局所化を組み合わせて、高信号・低ノイズな最適化コンテキストを構築する手法を提案する。実験では、標準的なコンテキスト削減手法より優れ、特に形式検証タスクでは人手設計エージェントの成功率を42.5%から58.5%へ改善した。



**新規性**
バッチ単位で代表的な失敗トレースを選別し、各トレース内では依存グラフに基づいて非因果ステップを除去して根本原因モジュールを特定する点が新しい。



**Tags:** agent optimization, trajectory analysis, causal extraction, LLM, reflection
...
- PDF: [link](https://arxiv.org/pdf/2607.07646.pdf)
- Published: 2026-07-09 02:04 JST
- Updated: 2026-07-09 02:04 JST
...
**日本語要約**
本研究は、RLによるpost-trainingが単に既存の原始的スキルを増幅するだけでなく、それらを組み合わせて新しい高次の戦略を構成できるかを、rewrite-grammar環境で検証した。結果として、RLは未学習問題を解けるようにし、まず原始的な削減能力を強化した後、逐次的・並列的な合成手続きを発見して再利用可能な安定したレパートリーへと整理した。



**新規性**
RL post-trainingが、原始スキルの単純な強化ではなく、逐次合成と並列合成を含む構成的推論戦略を生成・定着させることを示した点が新規である。



**Tags:** reinforcement learning, post-training, compositional reasoning, transformer, rewrite grammar
...
### 10. Do LLM-Generated Skills Make Better AI Data Scientists? A Component Ablation Across Data-Science Workflows
- arXiv: [2607.07504](https://arxiv.org/abs/2607.07504)
- PDF: [link](https://arxiv.org/pdf/2607.07504.pdf)
- Published: 2026-07-09 00:00 JST
- Updated: 2026-07-09 00:00 JST
...
**日本語要約**
本研究は、データ準備・抽出・統計分析・報告の4段階にわたるデータサイエンス業務で、LLM生成の再利用可能なスキルファイルがタスク単独のプロンプトより性能を改善するかを検証した。56タスク、9つのモデル構成、3プロバイダにまたがる7,560回の実験の結果、フルスキルも分解した各スキル要素も有意な改善を示さず、タスク単独プロンプトとの差は小さかった。補足実験でも、フルスキルはタスクに無関係なスキル形式の内容と同程度であり、単発のデフォルト戦略としてLLM生成スキルを使うことには注意が必要だと示された。



**新規性**
LLM生成スキルをデータサイエンスの複数ワークフローで成分分解し、フルスキル・部分スキル・トークン整合対照まで含めて、性能改善が再現的に見られないことを大規模に示した点。



**Tags:** LLM, data science, ablation study, prompting, evaluation
...
### 11. Search, Fail, Recover: A Training Framework for Correction-Aware Reasoning
- arXiv: [2607.07492](https://arxiv.org/abs/2607.07492)
- PDF: [link](https://arxiv.org/pdf/2607.07492.pdf)
- Published: 2026-07-08 23:53 JST
- Updated: 2026-07-08 23:53 JST
...
**日本語要約**
本研究は、単一の左から右への連鎖では表しにくい推論課題に対し、部分解の検証付き探索として推論を学習・推論するフレームワークPyligentを提案する。検証器で生成継続と失敗をラベル付けし、continue/finish/backtrackの3動作に対する教師信号へ変換することで、遅延失敗からの回復を学習させる。



**新規性**
失敗した分岐を明示的に監督し、backtrackを含む修正可能な推論行動を学習させる点が新しい。



**Tags:** 推論, 探索, バックトラッキング, 教師あり学習, 検証器
...
### 12. Does Bielik Know What It Doesn't Know? Activation Dispersion Separates Entity Familiarity from Factual Reliability Across Model Scale
- arXiv: [2607.07670](https://arxiv.org/abs/2607.07670)
- PDF: [link](https://arxiv.org/pdf/2607.07670.pdf)
- Published: 2026-07-09 02:24 JST
- Updated: 2026-07-09 02:24 JST
...
**日本語要約**
Bielikの4つのモデル（1.5B〜11B）を対象に、出力前のMLP活性の分散指標が、既知の実在エンティティと架空エンティティを高い精度で分離できることを示した研究です。さらに、この信号は実在だが未知の名前にも一般化し、エンティティの「見覚え」と回答の事実性が別の現象であることを示唆します。



**新規性**
単一のforward passで得られる活性分散指標により、エンティティの親しみやすさと事実的信頼性を分離し、しかもその内部信号がモデル規模をまたいで強く保たれることを示した点です。



**Tags:** large language models, hallucination, activation analysis, entity familiarity, factual reliability
...
### 13. PALS: Percentile-Aware Layerwise Sparsity for LLM Pruning
- arXiv: [2607.07557](https://arxiv.org/abs/2607.07557)
- PDF: [link](https://arxiv.org/pdf/2607.07557.pdf)
- Published: 2026-07-09 00:51 JST
- Updated: 2026-07-09 00:51 JST
...
**日本語要約**
本論文は、Transformerの全層に同一の疎率を適用する従来の一発剪定法（Wanda, SparseGPT）の問題に対し、層ごとの活性値分布の違いを考慮したPALSを提案する。PALSは活性値振幅の99パーセンタイルに基づいて層別の疎率を調整し、目標疎率の±5%以内に制約することで、LLaMA-2-7Bの50%疎化でWikiText-2 perplexityを改善した。一方で効果はアーキテクチャ依存であり、LLaMA-3-8Bでは小さく、Mistral-7Bでは見られなかった。



**新規性**
活性値の99パーセンタイルを用いて層ごとの疎率を自動調整する、低コストで微調整不要の剪定手法を提案した点が新しい。



**Tags:** LLM pruning, layerwise sparsity, activation statistics, one-shot pruning, transformer
...
### 14. A Unified Detection Framework for AI-Related Content and Artifacts
- arXiv: [2607.07527](https://arxiv.org/abs/2607.07527)
- PDF: [link](https://arxiv.org/pdf/2607.07527.pdf)
- Published: 2026-07-09 00:21 JST
- Updated: 2026-07-09 00:21 JST
...
**日本語要約**
本論文は、AI関連コンテンツやアーティファクトの検出を、Mahalanobis distance scores（MDS）に基づく統一的な枠組みとして提案している。LLM生成文、幻覚、ウォーターマーク、敵対的例など複数の設定に適用でき、正例の深層表現に対する共分散行列の頑健な推定を中核にしている。



**新規性**
casewiseおよびcellwiseのminimum covariance determinant（MCD）推定量に対するjoint estimation法を開発し、収束性と高breakdown point性を示した点が新しい。



**Tags:** AI検出, Mahalanobis距離, 頑健推定, LLM生成文, 敵対的例
...
### 15. Max Out GRPO Signal: Adaptive Trace Prefix Control for Hard Reasoning Problems
- arXiv: [2607.07674](https://arxiv.org/abs/2607.07674)
- PDF: [link](https://arxiv.org/pdf/2607.07674.pdf)
- Published: 2026-07-09 02:32 JST
- Updated: 2026-07-09 02:32 JST
- Authors: Vladislav Beliaev
...
**日本語要約**
GRPOは難問で全ロールアウトが失敗すると勾配信号が消え、学習が進みにくいという問題に対し、AdaPrefix-GRPOは参照解の正しい接頭辞を与える長さを訓練中に調整し、各問題の成功率を約50%に保つことで有効な学習信号を維持します。学習後は接頭辞の支援を外して推論時には自力で解かせる設計で、難しい数理タスクでGRPOより高い精度を示しつつ、トレース長も短縮しています。



**新規性**
問題ごとの接頭辞長をフィードバック制御で動的に調整し、GRPOの学習信号が最大化される成功率帯を保ちながら、最終的には支援を完全に除去する点が新規です。



**Tags:** GRPO, 強化学習, 難問推論, 適応制御, 数学推論
...
- PDF: [link](https://arxiv.org/pdf/2607.07696.pdf)
- Published: 2026-07-09 02:55 JST
- Updated: 2026-07-09 02:55 JST
...
**日本語要約**
本論文は、外部DBに保存されたデータをJDBC/ODBC経由ではなく、DBのストレージファイルを直接読み出して列指向のメモリバッファへ変換するJailbreakを提案する。PostgreSQLとMySQLのストレージファイルを対象に、TPC-HでJDBC/ODBCベースの手法と正しさを比較し、分析処理のスループットを最大27倍向上させた。



**新規性**
LLMがソースコードやドキュメントを入力として、DBごとのストレージ読取コンポーネントを人手のパーサ実装なしに再生成し、DBバイパスを可能にした点が新しい。



**Tags:** database systems, LLM, storage reader, columnar analytics, performance
...
- PDF: [link](https://arxiv.org/pdf/2607.07507.pdf)
- Published: 2026-07-09 00:02 JST
- Updated: 2026-07-09 00:02 JST
...
**日本語要約**
本研究は、視覚言語モデルにおける幻覚を単なる生成時の誤りではなく、その後の推論に影響する「Post Hallucination Reasoning (PHR)」として捉え、HIVEという評価基盤で体系的に調べている。9つのタスクと9つのモデルで、忠実なキャプションと幻覚を含むキャプションを比較し、幻覚キャプションが視覚言語タスクでは精度を改善する一方、テキストのみのタスクでは効果が限定的または不安定であることを示した。



**新規性**
幻覚の検出・抑制ではなく、幻覚が推論文脈に入った後の影響を評価するPHRという観点と、そのためのHIVE評価基盤を提案した点が新しい。



**Tags:** vision-language models, hallucination, reasoning, evaluation, multimodal
...
### 18. MedPMC: A Systematic Framework for Scaling High-Fidelity Medical Multimodal Data for Foundation Models
- arXiv: [2607.07673](https://arxiv.org/abs/2607.07673)
- PDF: [link](https://arxiv.org/pdf/2607.07673.pdf)
- Published: 2026-07-09 02:26 JST
- Updated: 2026-07-09 02:26 JST
...
**日本語要約**
MedPMCは、許諾された医学文献から高忠実度の画像-テキスト対を継続的に自動構築するフレームワークで、PMC 6.1百万件から1100万組を収集しました。各種コンポーネント評価と人手評価で高い精度と医学的妥当性が示され、既存のPMC由来データセットより関連画像の割合が大きく向上しました。さらに、MedPMCで学習したCLIP系モデルやマルチモーダルLLMの視覚エンコーダは、複数ベンチマークや臨床データで性能改善を示しました。



**新規性**
大規模PMC文献を高忠実度な医学画像-テキスト資源へ自動変換し、継続更新可能な基盤として公開した点が新規です。



**Tags:** medical multimodal learning, foundation models, dataset curation, PubMed Central, CLIP
...
### 19. Think Big, Search Small: Where Capacity Matters in Hierarchical Search Agents?
- arXiv: [2607.07548](https://arxiv.org/abs/2607.07548)
- PDF: [link](https://arxiv.org/pdf/2607.07548.pdf)
- Published: 2026-07-09 00:46 JST
- Updated: 2026-07-09 00:46 JST
...
**日本語要約**
本研究は、階層的検索エージェントを「委譲」「実行」「回答生成」の3役割に分解し、どの役割にモデル容量を配分すべきかを5つの多段QAベンチマークで検証した。役割分解は単一エージェントより一貫して高性能であり、特に委譲側の容量拡大が精度向上に大きく効く一方、実行側の拡大効果は小さいことが示された。



**新規性**
階層的検索における容量配分を役割ごとに制御して比較し、ボトルネックが分解・委譲側にあることを実験的に示した点が新しい。



**Tags:** hierarchical search, multi-agent, multi-hop QA, model capacity, retrieval
...
### 20. Co-LMLM: Continuous-Query Limited Memory Language Models
- arXiv: [2607.07707](https://arxiv.org/abs/2607.07707)
- PDF: [link](https://arxiv.org/pdf/2607.07707.pdf)
- Published: 2026-07-09 02:59 JST
- Updated: 2026-07-09 02:59 JST
...
**日本語要約**
本論文は、事実知識を重みではなく外部KBに保持する limited memory language model（LMLM）を提案する。提案手法CO-LMLMは、連続的なキーとテキスト知識を対応づけ、低コストなベクトルクエリで人間可読かつ帰属可能な知識を生成時に取得する。Wikipediaに限定しない注釈パイプラインも導入し、複数規模・複数データセットで既存LMLMや通常のLLMより困惑度と事実精度が向上した。



**新規性**
連続キーとテキスト値を用いるcontinuous-query型のLMLMを導入し、従来の関係型KB・クエリ依存から脱却した点が新規である。



**Tags:** language model, knowledge base, factual precision, retrieval, pretraining
