...
Generated at: 2026-07-12 09:06 JST
Run ID: a1d7532d-0909-4994-ab14-830ca2d9a6f6
...
**日本語要約**
BiSCo-LLMは、LLMの重み圧縮において、コードブックを使わないbinary spherical codingを用いる極低ビット圧縮手法です。局所的な重みチャンクを単位超球面上で二値化してビット列として保持し、さらに残差BSQとカテゴリ別のrecovery distillationを組み合わせて、再構成誤差とモジュール置換後の不整合を抑えます。



**新規性**
明示的なコードブックやインデックス参照を使わず、binary spherical codingと残差BSQ、カテゴリ別蒸留を組み合わせて極低ビット圧縮を実現している点が新規です。



**Tags:** LLM圧縮, 低ビット量子化, バイナリ符号化, コードブック不要, 知識蒸留
...
**日本語要約**
本論文は、LLMのテスト時モデル選択において、同一モデルの再サンプリングと別モデルへの再ルーティングを、1クエリあたりの単一予算の競合する使い方として定式化している。さらに、推定された単位コスト当たりの正解率改善に基づくオンラインなRoR（resample-or-reroute）方策を提案し、複数ベンチマークでコストと品質の良いトレードオフを示した。



**新規性**
再サンプリングと再ルーティングを別々ではなく、単一の予算配分問題として扱い、検証器の品質を考慮したオンライン方策を提案した点が新しい。



**Tags:** large language models, model routing, test-time selection, budget-aware optimization, resampling
...
**日本語要約**
HCC-STARは、電子カルテの記述を入力として、HCCのリスク層別化、ガイドライン整合的な治療候補の順位付け、個別化生存予測を同時に出力する臨床推論LLMです。約3万例のSEER由来データを用いてEMR風の学習データを構築し、中国12施設6,668例の多施設コホートで、治療推奨とリスク層別化において既存のガイドラインや競合モデルを上回りました。



**新規性**
臨床文脈を反映したEMRナラティブを用い、ステップ検証可能な複合報酬で最適化した、HCC向けの説明可能な推論型LLMを提案した点が新規です。



**Tags:** hepatocellular carcinoma, large language model, risk stratification, treatment recommendation, electronic medical records
...
**日本語要約**
LLMを用いたワークフローでは、ツール利用、検索、分岐、チェックポイント、人手承認などを含む明示的な実行手順が増えている。本論文は、ワークフロー定義や実行インスタンス、推論記録、文脈スナップショット、依存関係を共有知識基盤上の永続的な知識オブジェクトとして扱う概念モデルを提案し、ワークフローを検査・再開・レビュー可能な対象として捉える。



**新規性**
deterministicなderiveと、LLM判断を伴うinferを区別し、ワークフローそのものを知識として表現する「semantic persistence」の概念を提示した点。



**Tags:** LLM workflows, knowledge representation, semantic persistence, workflow systems, conceptual model
...
**日本語要約**
大規模な事前学習データの品質向上を目的として、UltraXは削除・修正に加えて挿入も扱える関数呼び出し型のデータ精製フレームワークを提案する。データセット適応型のプロンプト最適化と、整列・置換に基づくプログラム監督生成により、細粒度の編集を可能にしつつ、大規模処理に必要な効率性と信頼性を高める。実験では、複数コーパスで平均性能が最良となり、より少ない学習トークンでも同等以上の性能を示した。



**新規性**
削除・修正だけでなく挿入操作まで含めた編集空間を導入し、原文-精製文対から構造化されたプログラム監督を生成して大規模データ精製を実現した点が新規である。



**Tags:** 大規模言語モデル, 事前学習データ, データ精製, 関数呼び出し, プログラム監督
...
**日本語要約**
本論文は、科学的アイデアの系譜構造を扱うベンチマーク「IdeaGene-Bench（IG-Bench）」を提案し、論文や提案をIdea GenomeオブジェクトとGenomeDiffで表現して、継承・変異・喪失・外部導入・新規挿入を記録できる枠組みを示す。IG-Examでは系譜推論を、IG-Arenaでは系譜に条件づけたアイデア生成を評価し、10分野にわたるデータで14種類のLLM科学者を検証している。



**新規性**
科学的アイデアの「系譜」を明示的に表現し、推論と生成の両方を同一の lineage-grounded 枠組みで評価するベンチマークを構築した点が新しい。
...
**日本語要約**
本論文は、LLMの事後量子化評価が精度やperplexityだけでは不十分であり、量子化によって生じる振る舞いの変化を捉えられないことを示す。著者らは correctness agreement という決定レベルの指標を導入し、8-bitから2-bitまでの複数モデル・量子化手法で、見かけ上の性能が保たれていても予測の一致が崩れることを報告する。



**新規性**
量子化モデルと元モデルの正解予測の重なりを測る correctness agreement を提案し、注意重みの層別歪み解析により、低ビット幅での非線形な分岐と query/key 投影の高い感度を明らかにした。



**Tags:** LLM, quantization, evaluation, attention, statistical analysis
...
**日本語要約**
ポルトガルの国産LLM AMALIA-9Bが、道徳基盤の「権威」を人手コーダーと高い一致で符号化できるかを検証したが、一致は妥当性を保証しないことを示した。理論に基づく分解プロンプトで再評価すると性能は約半分まで低下し、表層的な手がかり、特に権威者周辺の道徳的怒りに依存している可能性が示唆された。



**新規性**
単なる人手一致ではなく、理論的な分解・再構成による「recovery gap」を用いて、LLMの注釈が構成概念を本当に捉えているかを検証した点が新しい。



**Tags:** LLM, data annotation, validity, moral foundations, multilingual
...
**日本語要約**
LLMベースのWeb検索エージェントは、深さと広さの両方を要する調査タスクに有効だが、単一エージェントは長い探索軌跡と文脈制約があり、既存のマルチエージェントも再帰的な深掘りや柔軟な協調に限界がある。WebSwarmは、推論時にタスク分解・再帰的拡張・エージェント協調を動的に行う枠組みで、各ノードが自己解決または子ノードへの委譲を行い、証拠を上位へ返すことで探索を段階的に拡張する。



**新規性**
Web上の情報構造を事前に探ってノード拡張を根拠づけ、同種の兄弟ノード間でプロセス経験を再利用する再帰的マルチエージェント編成を提案した。



**Tags:** LLM, web search, multi-agent, recursive delegation, information retrieval
...
**日本語要約**
UniClawBenchは、実世界の動的な環境で能動的エージェントを評価するための、能力駆動型ベンチマークです。5つの基礎能力に基づく400件のバイリンガル課題を用い、Docker上でのライブ実行と段階的な完了チェックポイントにより評価します。さらに、実行エージェント・隠れた監督エージェント・ユーザーエージェントからなる閉ループ評価で、現実的な多ターンフィードバックを再現します。



**新規性**
シナリオ中心ではなく能力中心の設計を採用し、ライブ環境と閉ループ評価を組み合わせて、モデル能力とエージェントフレームワーク設計の影響を切り分けて評価できる点が新しいです。



**Tags:** benchmark, proactive agents, real-world tasks, multimodal, evaluation
...
**日本語要約**
本研究は、deep-research 系システムにおける引用品質の判定を対象に、LLM judge の能力とバイアスを評価したベンチマーク研究である。8種類の既製LLM judgeを1,248件のルーブリック判定で比較し、source relevance と factual support の両面で、必ずしも高価な最先端モデルが最良とは限らないことを示した。



**新規性**
引用品質のルーブリック判定において、F1だけでは見えない pass-rate drift や false positive/negative の偏りを明示的に比較し、報酬信号として使う前に judge の較正が必要であることを示した点が新しい。



**Tags:** LLM judge, citation quality, benchmark, reinforcement learning, source attribution
...
**日本語要約**
消費者向けの健康チャットボットにおける症状評価では、従来の開発・評価が協力的で明瞭な模擬患者に依存してきたが、本研究は2,053件の実際の患者-チャットボット対話を分析し、ユーザごとにコミュニケーション様式や感情表現が大きく異なることを示した。さらに、臨床内容・感情状態・会話戦略・コミュニケーションスタイルを別々にモデル化する患者シミュレータを構築し、リアルさ評価では人間判定者の正解率が55%と、実対話とほぼ区別できない水準を達成した。



**新規性**
臨床内容だけでなく感情、会話戦略、コミュニケーションスタイルを分離してモデル化する患者シミュレータを提案し、対話スタイルがトリアージ結果を有意に変えうることを示した。



**Tags:** health chatbot, patient simulation, LLM evaluation, triage, conversational AI
...
**日本語要約**
本研究は、LLMで重要とされるSuper Weightsを削除すると性能が大きく落ちる一方で、それらの重みだけを選択的に学習しても有効に適応できないことを示した。OLMo-1B/7Bでは、Super Weights周辺の少数パラメータのみを更新すると精度がランダムレベルまで低下し、同数のランダム位置の更新や通常のLoRAは成功することから、重要性と学習可能性は一致しないと結論づけている。



**新規性**
Super Weightsの「重要性」は、そこだけを学習すればよいことを意味しないことを実験的に示し、効果的な微調整には個別座標ではなく層全体にまたがる構造化された更新が必要だと明らかにした。



**Tags:** LLM, fine-tuning, parameter importance, LoRA, sparsity
...
**日本語要約**
本研究は、連続制御における推論を、自己回帰的な潜在空間上での反復的・適応的な情報検索として定式化する Latent Memory Palace（LMP）を提案する。変分推論として推論を扱い、その下界を最適化する潜在空間強化学習手法を導出することで、シミュレーションと実機の両方で高い性能と、テスト時計算の適応的な配分を示した。



**新規性**
推論を自己回帰的潜在分布を用いた変分推論として制御に導入し、さらに可変長の行動トークナイザ LMP-tok も同一枠組みから得られる点が新規である。
...
**日本語要約**
本論文は、ダッシュカム動画における安全クリティカルな事故・ニアミスの理解を評価するための incident-centric VQA ベンチマーク「AUTOPILOT-VQA」を提案する。天候、照明、交通環境、道路状況、標識、関与物体、事故発生、衝突位置、回避可能性などを含む構造化質問により、文脈理解とイベントレベルの推論を評価する。



**新規性**
物体認識中心の評価ではなく、実世界の運転インシデントに基づく時間的に根拠づけられた安全志向の推論を測る点が新しい。



**Tags:** Vision-Language Models, VQA, autonomous driving, dashcam video, benchmark
