...
Generated at: 2026-07-25 09:13 JST
Run ID: 6f7f838d-8921-47ed-be68-b8db4e186864
...
**日本語要約**
本研究は、静的な臨床症例を意思決定中心のストーリーテリングゲームへ変換するフレームワーク「MedGame」を提案する。MedGameは、臨床ストーリーと状態・分岐を生成する設計器と、それを依存関係を考慮した実行計画に変換するディレクタの二段構成で、医療教育向けの対話型学習体験を実現する。



**新規性**
臨床ケース全体を単発QAではなく、状態遷移と分岐を持つ実行可能な物語ゲームとして組織化する点が新しい。



**Tags:** 医療教育, 大規模言語モデル, ストーリーテリング, ゲーム化, 臨床ケース
...
**日本語要約**
本研究は、個人データを外部クラウドに送れない研究環境でも利用可能な、ローカル実行のopen-weight LLM/エージェントによるデータ準備支援の有効性を評価するための公開フレームワークを提案する。英国コホート研究の6波にわたる清掃スクリプトを用いた20件のデータ準備タスク（102変数生成）でベンチマークを行い、最新の31–35Bモデルが高い達成率（最大87.9%）を示した。



**新規性**
長期縦断データの準備という研究上のボトルネックに特化し、open-weight LLMの性能を自動評価できる公開ベンチマークと評価基盤を整備した点が新規である。



**Tags:** LLM, agent, data preparation, open-weight model, longitudinal study
...
**日本語要約**
大規模音声言語モデルは音声知覚では進展している一方、音声推論データの不足により深い論理推論が弱いという課題に対し、本研究はテキスト教師から音声学生へ推論能力を移すクロスモーダルな on-policy distillation 手法 X^3-OPD を提案する。学生は自身の音響認識に基づいて推論軌跡を生成し、教師は対応するテキスト入力と検証済み解答を用いてトークン単位で指導する。MMSU、MMAU、BIG Bench Audio、MMAR での実験により、音声に基づく推論と chain-of-thought の質が向上し、既存能力も概ね維持されることが示された。



**新規性**
音声言語モデルへの推論蒸留を、テキストに還元できない音声イベント・韻律・会話文脈まで含む三層の対称コーパスと on-policy alignment で拡張した点が新しい。
...
**日本語要約**
本研究は、音声と自動書き起こしテキストを組み合わせたオープンソースLLMベースのマルチモーダル枠組みにより、認知障害（CI）を非侵襲的に検出する手法を提案している。音響埋め込みとテキスト埋め込みを結合して分類に用い、ADReSS20およびADReSSo21で単一モダリティのベースラインを一貫して上回り、92.4%の分類精度と優れたクロスデータセット汎化を示した。



**新規性**
生音声と書き起こしを統合しつつ、患者の生データに依存しないプライバシー配慮型のLLMベース多モーダルCI検出枠組みを構築し、クロスデータセット汎化を改善した点。



**Tags:** cognitive impairment, speech analysis, multimodal learning, large language models, privacy-preserving
...
**日本語要約**
本研究は、LLMの道徳的判断における「迎合（sycophancy）」を単独の問題としてではなく、他者の意見に対する抵抗と受容の広い過程として捉え直している。3つの研究を通じて、判断修正は、意見の距離、発言の出所、支持する集団構造という3次元に沿って体系的に変化することを示した。



**新規性**
迎合を、社会的影響に基づく判断更新の一部として位置づけ、建設的な信念修正と迎合的な同調を区別するための枠組みを提示した点。



**Tags:** LLM, moral reasoning, sycophancy, social influence, judgment revision
...
**日本語要約**
大規模言語モデルが「This is not a course. It is a journey of transformation」のような自己修正表現（epanorthosis）を過剰に用いる現象を、訓練分布の偏りとRLHFによる強調的表現の選好で説明している。さらに、ジャンルごとの人間の使用率を基準にしたEpanorthosis Indexを提案し、複数の文体でモデルの過不足を測定している。



**新規性**
ジャンル別の人間基準に対するepanorthosisの密度を測るEpanorthosis Indexを導入し、LoRAや指示文・SFTで人間率へ較正する方針を示した点。



**Tags:** large language models, rhetoric, preference tuning, calibration, LoRA
...
**日本語要約**
視覚と言語の各ビューで異なる推論挙動が現れることに着目し、同一幾何問題の text-dominant / image-dominant / combined の対応データセット ODA-Data を構築した。さらに、各ビューでモデルを評価して最良ビューを教師とし、他ビューを逆KLで学習させる MIRROR を提案し、幾何推論ベンチマークで標準的なRLより精度と一貫性を改善した。



**新規性**
同一問題の複数モダリティ視点のうち最良のビューを教師として他ビューへ逆KLで知識を伝播する、モダリティ間の相互学習型RLを導入した点。



**Tags:** multimodal reasoning, vision-language models, reinforcement learning, geometry, self-supervision
...
**日本語要約**
本研究は、日常文化や長尾知識を含むクイズ形式の多言語ベンチマークTriviaRoomQAを提案し、6言語で3,300問、さらにフランス語のみの5,340問を用いて30個のオープンウェイトLLMを評価した。結果として、歴史・地理・数学のような知識集約的分野では強い一方、著名人・音楽・映画・ニュースなどの日常的なポピュラーカルチャー分野では性能が大きく低下し、同一内容でも言語によって成績が変わることが示された。



**新規性**
学術ベンチマークでは捉えにくい、日常文化に関する多言語LLMの知識ギャップを体系的に測定するTriviaRoomQAを新たに構築した。



**Tags:** multilingual LLM, benchmark, knowledge evaluation, trivia QA, cultural knowledge
...
**日本語要約**
本論文は、並行かつ状態依存のRustライブラリAPIに対して、Petriネットで表現した資源・ライフサイクル・因果関係を中間表現として用い、LLMに実行可能なテストを生成させる手法を提案する。これにより、API前提条件を満たしやすい深い状態や並行シナリオを、低コストで具体的なテストコードへ落とし込むことを目指す。さらに、局所忠実性契約、構造修復ループ、スケジュール調整、段階的なセマンティックオラクルを組み合わせて、合成失敗とAPIの期待動作違反を区別する。



**新規性**
PetriネットをLLMテスト生成の制約付き中間表現として導入し、並行状態依存API向けに合法な深い状態・近合法・部分順序並行シナリオを系統的に実行可能テストへ変換する点が新規である。



**Tags:** LLM, test generation, Petri net, concurrent APIs, Rust
...
**日本語要約**
Thinkinkは、手書きテキストやスケッチをLLMへのプロンプトとして使い、その応答をインク風の文字や図として同じ2Dキャンバス上に空間的に統合する対話ツールである。セマンティックツリーと状態機械ベースの軽量UIにより、インクの解釈と明示的な制御を支援する。形成的調査、診断的調査、最終調査の3段階を通じて設計され、発想支援への組み込み方を検討している。



**新規性**
LLMの入出力を手書き・スケッチ中心の2D共有キャンバスに統合し、インクネイティブな相互作用を実現した点が新しい。



**Tags:** HCI, LLM, 手書き入力, スケッチ, 創造支援
...
**日本語要約**
本研究は、構造化された音声キャプションの評価が従来の平坦なテキスト指標では難しいことに着目し、AudioCardsデータセット上で5つの軸（タグ集合、記述、論理推論、数値測定、スペクトルプロファイル）を評価する多軸フレームワークを提案する。評価にはLLM判定と決定的な計算指標を組み合わせ、意味を保つ言い換えと、意味的・音響的な破損を区別できることを示した。



**新規性**
制御された摂動を注入する検証プロトコルにより、構造化音声記述の評価フレームワークの信頼性を体系的に検証した点が新しい。
...
**日本語要約**
Chain-of-thought推論モデルでは、生成がトークン予算内で収束する場合と、結論に至らず予算を使い切る非収束の場合の二峰性が観察され、収束した生成は高い正解率を示す一方、非収束は極めて低い正解率でした。さらに、隠れ状態の内部表現に対する線形プローブにより、生成の早い段階から収束可否をある程度予測でき、トークンエントロピーや反復統計に基づく行動ベースラインよりも良好でした。



**新規性**
生成途中の内部表現から、最終的な推論の収束／非収束を早期に検出できる可能性を示し、早期終了推論や適応的計算配分への道を開いた点が新規です。



**Tags:** chain-of-thought, early detection, linear probe, reasoning convergence, hidden states
...
**日本語要約**
本研究は、推論時の計算量を増やして性能を高める既存手法が、トークン単位の寄与を扱わないため有効な推論前半を捨ててしまう点に着目し、誤り位置を特定してその直前までを再利用する Test-Time Scaling via Error Localization（TTEL）を提案する。実験では、逐次的推論タスクにおいて pass@k と生成トークンコストの両面で優れた Pareto frontier を示し、LiveCodeBench や数学ベンチマークで既存の test-time baseline を上回った。



**新規性**
固定または環境フィードバックと、情報付きフィードバックと無文脈ベースラインの条件付き確率比較により、誤りが起きたステップをトークン単位で局在化し、正しい接頭辞を最大限再利用しながら分岐生成する点が新しい。
...
**日本語要約**
人間とLLMの共著テキストにおいて、文書全体ではなくトークン単位でLLM生成部分を特定する検出手法を提案している。既存のトークンレベル検出スコアを近傍で平滑化し、局所的な著者構造に応じて帯域幅を適応的に選ぶことで、混在文書中の生成箇所の局在化を目指す。



**新規性**
トークンレベルの検出スコアを平滑化し、Lepski型の適応規則で帯域幅を選ぶことで、トークン単位のLLM生成部分の推定を可能にした点が新しい。



**Tags:** LLM検出, トークンレベル解析, 共同執筆, 適応平滑化, 著者識別
...
**日本語要約**
LLMの長期記憶能力を評価するために、英語中心で粗い集計指標に依存していた既存ベンチマークの限界を踏まえ、ロシア語向けの長期会話記憶ベンチマークRUMBAを提案している。RUMBAは、セマンティックな種類、セッション範囲、時間推論、時間表現の明示性を考慮した細かな質問分類と統一的な評価方法を備え、セッションをまたぐ検索・統合・推論を要するQA対で構成される。



**新規性**
長期会話記憶を、時間情報や推論を含む細粒度の観点から診断できるロシア語ベンチマークとして設計し、英語サブセットも同一手法で整備した点が新しい。



**Tags:** long-term memory, benchmark, conversational QA, Russian language, temporal reasoning
