...
Generated at: 2026-07-31 09:13 JST
Run ID: 65e4dd6c-532a-4f47-a723-4923b3ddb000
...
- PDF: [link](https://arxiv.org/pdf/2607.27081.pdf)
- Published: 2026-07-30 01:07 JST
- Updated: 2026-07-30 01:07 JST
...
**日本語要約**
大規模言語モデルの安全な再調整において、攻撃者のプロンプトテンプレートが見えない場合や、再ジャイルブレイクに対して既存手法が脆弱である問題を指摘しています。提案手法ROPDは、特定テンプレートへの適合ではなく、整合済みモデルと侵害済みモデルの出力分布の乖離をモデル化することで、能力保持と防御性能の両立を目指します。3つのデータセットと3つの基盤モデルでの実験により、テンプレート不一致時でも既存手法より頑健であることを示しています。



**新規性**
整合済み出力と侵害済み出力の確率分布の差に基づいて学習するRouting-based On-Policy Distillation（ROPD）を提案し、テンプレート依存を減らしながら安全再調整の頑健性を高めた点です。



**Tags:** LLM safety, distillation, realignment, robustness, prompt template
...
- PDF: [link](https://arxiv.org/pdf/2607.27134.pdf)
- Published: 2026-07-30 02:04 JST
- Updated: 2026-07-30 02:04 JST
...
**日本語要約**
LLMによる文章支援が広く使われると、文章の明瞭化や整形には役立つ一方で、言語表現の多様性が低下し「linguistic monoculture」を生む可能性がある。本論文は、著者とLLMの相互作用を数理モデル化し、共有モデル・再帰的更新・個人化モデルの3機構について平衡と収束を解析した。さらに、順応を戦略的選択として扱うことで、個人の合理的な順応が社会的には過剰になりうる負の外部性を示した。



**新規性**
LLM支援による言語多様性の低下を、著者とモデルの共進化・平衡解析・戦略的順応を含む数理枠組みで定式化し、共有・再帰・個人化の各機構が長期的多様性に与える差を比較した点が新しい。



**Tags:** LLM, language diversity, mathematical modeling, conformity, social externality
...
- PDF: [link](https://arxiv.org/pdf/2607.27155.pdf)
- Published: 2026-07-30 02:33 JST
- Updated: 2026-07-30 02:33 JST
...
**日本語要約**
OmegaUse-OfficeValは、LLMエージェントがオフィススイートの長期タスクをどの程度のコストで遂行できるかを評価するためのベンチマークです。100件の実務由来タスクに対して、人間の作業時間とタスク価格の代理指標を付与し、人間コストとLLM推論コストの比較や価値重み付き評価を可能にしています。



**新規性**
各タスクに人間労働時間と価格代理指標という2つの経済シグナルを対応付け、長期オフィス業務を経済的観点から評価できる点が新規です。



**Tags:** LLM agents, benchmark, office-suite tasks, economic grounding, evaluation
...
- PDF: [link](https://arxiv.org/pdf/2607.27022.pdf)
- Published: 2026-07-30 00:20 JST
- Updated: 2026-07-30 00:20 JST
...
**日本語要約**
本研究は、LLMにおける地域バイアスを、抽象的なステレオタイプ評価から教育・職業・社会的交流に関する具体的な意思決定まで一貫して測定する枠組みS2Dを提案した。中国の34省級行政区を対象に6つのLLMを評価した結果、地域ごとの差が大きく、特にCompetenceと職業判断でモデル間の一致が高かった。さらに、これらの傾向は地域の経済・デジタル発展指標と関連し、中国語・英語プロンプトの両方で概ね安定していた。



**新規性**
地域ステレオタイプと具体的な社会的意思決定を同一枠組みで結びつけ、LLMの地域バイアスを体系的に評価した点が新しい。



**Tags:** LLM, regional bias, stereotype, social decision-making, evaluation
...
- PDF: [link](https://arxiv.org/pdf/2607.27090.pdf)
- Published: 2026-07-30 01:18 JST
- Updated: 2026-07-30 01:18 JST
...
**日本語要約**
InferScaleは、パーソナライズされたLLM推論で毎回同じメモリ文脈をプロンプトに再投入する代わりに、事前計算したKV状態をGPU上で再利用することで、TTFTの増加を抑えるGPUネイティブなメモリシステムである。vLLMのpaged cacheへ直接KVを注入し、RoPE対応のChunked RoPEと、周辺文脈も使って各メモリ断片を符号化するContext-Window Encodingを組み合わせる。



**新規性**
各メモリ事実のKV表現をGPU上に保持し、検索時に直接注入することで、再prefillを不要にした点が新規である。



**Tags:** LLM serving, KV cache, personalized memory, GPU, latency
...
### 6. AgentMap: Joint Equivalence and Subsumption Discovery for Ontology Matching
- arXiv: [2607.27130](https://arxiv.org/abs/2607.27130)
- PDF: [link](https://arxiv.org/pdf/2607.27130.pdf)
- Published: 2026-07-30 01:58 JST
- Updated: 2026-07-30 01:58 JST
...
**日本語要約**
本論文は、オントロジーマッチングを同値対応の発見と上位概念への包含関係（subsumption）の発見を統合した新しい課題として定義し、Hybrid Ontology Matching (HOM) を提案する。あわせて、LLMベースのマルチエージェント枠組み AgentMap を提案し、意味検索・階層探索・協調推論を通じて、同値概念または最も細粒度の包含概念を順次探索する。



**新規性**
同値マッチングとsubsumptionマッチングを同時に扱うHOMを導入し、LLMマルチエージェントによる統合的なオントロジーマッチング手法を提案した点。



**Tags:** ontology matching, large language model, multi-agent, subsumption, equivalence
...
- PDF: [link](https://arxiv.org/pdf/2607.27083.pdf)
- Published: 2026-07-30 01:07 JST
- Updated: 2026-07-30 01:07 JST
...
**日本語要約**
LLMエージェントが外部ツールを取得する際、関連性の順位付けだけでは「何個まで選ぶべきか」を決められない問題に対し、コストを考慮した停止判断としてCAM-DFを提案している。5つのツール利用ドメイン・1,343タスクで評価し、特に異なるコスト条件下で既存の予測後しきい値法より高い報酬を示し、実行時には利用ツール数を37%削減しつつ同等の成功率を維持した。



**新規性**
順位スコアではなく、停止時点と最良継続の差分に基づくコスト-awareな停止目的を直接学習し、異種コスト下でのツール取得判断を最適化した点が新規である。



**Tags:** LLM agents, tool selection, cost-aware stopping, retrieval, decision making
...
- PDF: [link](https://arxiv.org/pdf/2607.27069.pdf)
- Published: 2026-07-30 00:55 JST
- Updated: 2026-07-30 00:55 JST
...
**日本語要約**
本論文は、閉じたyes/no型の空間推論ベンチマークにおいて、画像が実際にどれだけモデルの判断に寄与したかを評価するためのVisual Credit Audit (VCA)を提案する。VCAは、テキストのみ・空白・画像条件を比較して、正解であっても画像から十分な支持を受けていない予測を分離し、さらに関係反転に対する応答も検証する。4つのMLLMと2つの空間ベンチマークで、正解だが画像による寄与が認められない決定が一定割合存在することを示した。



**新規性**
正解率だけではなく、画像の追加的支持と関係整合的な応答を分解して測るVCAを導入し、空間推論ベンチマークの「画像への依存度」を監査可能にした点が新規である。



**Tags:** multimodal learning, spatial reasoning, benchmark evaluation, visual grounding, model auditing
...
- PDF: [link](https://arxiv.org/pdf/2607.27066.pdf)
- Published: 2026-07-30 00:54 JST
- Updated: 2026-07-30 00:54 JST
...
**日本語要約**
科学論文の図表評価は、見た目の品質だけでなく、本文の主張を正しく支え、明瞭に情報を伝えるかを判定する必要がある。本研究では、3,857枚の科学図表に対してClarity, Relevance, Informativeness, Structureの4観点で注釈したデータセットを構築し、図の切り出し・キャプション・引用段落・軽量な文脈を入力として評価するSciFigAlignを提案した。



**新規性**
図と論文本文の証拠を細かく対応付けるようにCLIPとSciBERTを微調整し、クロスアテンション融合と順位学習を組み合わせて科学図表の品質を評価する点が新しい。



**Tags:** scientific figure assessment, multimodal learning, CLIP, SciBERT, peer review
...
### 10. Improving Item Discoverability in e-Commerce Search via Related Intent Generation
- arXiv: [2607.27172](https://arxiv.org/abs/2607.27172)
- PDF: [link](https://arxiv.org/pdf/2607.27172.pdf)
- Published: 2026-07-30 02:46 JST
- Updated: 2026-07-30 02:46 JST
...
**日本語要約**
本研究は、e-commerce検索において厳密一致の検索だけでは捉えにくい代替品・補完品・関連商品の発見性を高めるため、意図条件付きの候補拡張を行う discovery-augmented search を提案する。大規模言語モデルで head query の意図生成を行い、さらに LoRA と教師生徒蒸留で学習した小規模言語モデルを用いて tail query へ拡張し、セマンティック品質とセッション単位の購買分析の両面で有効性を評価した。



**新規性**
閉じた重みのLLMと蒸留済みSLMを組み合わせた2段階ハイブリッド構成により、関連意図生成の品質と推論コストの両立を図り、検索トラフィックのより広い範囲に発見性拡張を適用できる点が新規である。



**Tags:** e-commerce search, information retrieval, intent generation, large language models, knowledge distillation
...
- PDF: [link](https://arxiv.org/pdf/2607.27109.pdf)
- Published: 2026-07-30 01:38 JST
- Updated: 2026-07-30 01:38 JST
...
**日本語要約**
AudioLLMの発展に伴い、音声キャプションは短い説明から、より自由形式で細かな記述へと移行している。本論文は、情報の網羅性と記述の信頼性を診断できる大規模ベンチマークMMACを提案し、5,638件の音声クリップを用いて複数の能力カテゴリと評価次元でモデルを評価する。



**新規性**
音声キャプションの評価を、生成品質だけでなく「対象情報を含むか」と「参照ラベルと整合的か」の2観点で多次元に診断できるベンチマークを構築した点が新規である。



**Tags:** audio captioning, benchmark, evaluation, AudioLLM
