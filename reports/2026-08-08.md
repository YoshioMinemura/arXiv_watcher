...
Generated at: 2026-08-08 08:44 JST
Run ID: 8ee30135-a939-4e45-8018-28cec2413edd
...
**日本語要約**
MetaboLLMは、代謝omicsの異種知識を統合するために継続事前学習、教師あり微調整、構造化検索を組み合わせた代謝omics特化LLMであり、MetaboLLM-GINは生成した生化学記述を代謝物グラフに変換して患者レベル予測に用いる手法である。4種類のバックボーンで既存の基盤モデルや医療適応モデルを上回り、外部ベンチマークにも転移可能だった。さらに、CABG後のストレス高血糖予測と閉経後ホルモン療法分類で高いAUCを示し、解釈可能な生物学的知見も得られた。



**新規性**
代謝omics特化LLMで生化学知識を予測可能な代謝物グラフ表現へ変換し、患者レベル予測と解釈性を両立した点が新規である。
...
**日本語要約**
本研究は、多言語における推論能力の転移を改善するために、on-policy self-distillation（OPSD）を拡張したRP-OPSDを提案する。提案手法は、推論を進めたり方向転換したりする「reasoning pivots」に着目し、英語参照解答の有無による教師分布の差を手がかりに、重要な推論信号へ優先的に蒸留を行う。17言語の数理推論ベンチマークで、強い多言語ベースラインやOPSD系手法を上回った。



**新規性**
英語参照解答の有無による教師分布の変化を用いて、推論の要所であるreasoning pivotsに蒸留を集中させる点が新しい。



**Tags:** 多言語推論, 知識蒸留, 大規模言語モデル, 自己蒸留, 機械翻訳
...
**日本語要約**
本研究は、国家標準文書のようなルール密度の高い専門文書レビューにおけるLLMの能力を評価するため、初のベンチマークGB/T-Benchを提案する。GB/T Review Taxonomyに基づき、文書構造、適用範囲、規範表現、用語整合性、参照関係など25種類の誤りを診断可能にし、488文書から7,306件のレビュー誤り事例を構築した。



**新規性**
ルール重視の標準文書レビューに特化した初のベンチマークと、誤り位置・観点・種類まで厳密に一致を求める診断評価プロトコルを導入し、さらに多エージェント型のGB/T-Reviewerで性能向上を示した。



**Tags:** LLM評価, 文書レビュー, ベンチマーク, 標準文書, マルチエージェント
...
**日本語要約**
RLVRにおけるon-policy self-distillation（OPSD）は、教師を用いたトークンレベルの密な監督で報酬の疎さを補うが、従来法は生成中の時間的なずれの履歴を十分に反映できない。DASHは、各局所蒸留信号と系列平均との差をもとに適応的な伝播ゲートを作り、バックワードの多段集約を制御することで、生成過程に応じた監督重み付けを実現する。



**新規性**
局所的な分岐の大きさだけでなく、その分岐がどのようなずれの履歴の中で生じたかに応じて、トークンレベルの監督重みを適応的に変える点が新しい。



**Tags:** reinforcement learning, self-distillation, reasoning models, token-level supervision, adaptive weighting
...
**日本語要約**
NeSy-RAGは、検索したテキスト断片からPrologモジュールを生成し、LLMの質問応答を説明可能にするモジュール型のneuro-symbolic RAGフレームワークである。各断片から意味的な述語を作成してPrologクエリとして組み立て、実行トレースにより各推論ステップを元ソースに対応づける。さらに、ユーザー文脈の不足を検出して追質問を行う仕組みも導入している。



**新規性**
検索文書から属性可能なPrologモジュールを合成し、知識ギャップ検出まで含めて説明可能なRAG推論を実現した点が新しい。



**Tags:** RAG, 説明可能AI, 神経記号的推論, 質問応答, Prolog
...
**日本語要約**
本研究は、心不全のEHR特徴量エンジニアリングを自動化するための、証拠リンク付き・ルーブリック基盤のマルチエージェントパイプライン（nMAS）を提案し、9つのEHRソース表から500件のダミー患者記録で評価した。nMASは132の構造化特徴量と70のルーブリック採点付き集約特徴量を生成し、HFrEF/HFpEFの表現型分類でAUROCの改善を示した。



**新規性**
既存のルールベース手法やLLMベース手法の限界に対し、証拠の追跡可能性と監査可能性を備えた自動特徴量エンジニアリングのパイプラインを提示した点が新規である。
...
**日本語要約**
LLMをエージェントシステムに組み込む際の「ハーネス」（プロンプト、ツール、制御フロー、メモリ、オーケストレーション）を、評価フィードバックに基づいて反復的に改善する能力を測るベンチマークHarnessOpt-Benchを提案している。提案ベンチマークでは、固定された評価予算のもとでハーネスを編集し、秘匿されたテスト分割での正規化改善量により最終候補を評価する。5つのフロンティアLLMを4タスク・111回の実験で評価し、ハーネス最適化が測定可能で識別的な能力であることを示した。



**新規性**
評価境界を厳密に管理する実行環境の下で、エンドツーエンドのハーネス最適化能力を測る共通ベンチマークを初めて提示した点が新規である。
...
**日本語要約**
近年の生成的reward modelは応答の順位付けに強みを持つ一方、既存のRL手法が想定するスカラー報酬との不一致により、その性能を十分に活かせていないことを示した。著者らは、相対的な選好順位から報酬を構成するRanking-based Reward Construction（RRC）を提案し、サンプル間比較を用いるself-competitive rankingと、少数の参照応答を用いるanchor-guided rankingの2手法を導入した。



**新規性**
生成的reward modelの比較的な性質に合わせて、順位情報からRL用の報酬を構成するRRCを提案した点が新規である。
...
**日本語要約**
本研究は、外部の正解ラベルや環境フィードバック、大規模モデルの指導を使わずに、モデル自身の生成結果だけから on-policy self-distillation を行う U-OPSD を提案する。複数ロールアウトの多数決で擬似解を作り、その一貫性に基づいて教師分布を構成し、誤った長い出力の接頭辞へ蒸留することで自己修正を促す。



**新規性**
自己生成のみを用いた「真の」on-policy self-distillation を、内部整合性と多数決に基づく擬似解構築で実現した点が新規である。
...
**日本語要約**
本論文は、ポストトレーニング適応技術（再学習、微調整、検索拡張、モデル編集、アンラーニングなど）を対象に、文献を整理するための包括的なサーベイを提示する。機構、目的、データ要件、永続性、構造的範囲、モデル種別の6次元からなる分類体系を提案し、技術間の関係や用語の混同を整理することで、技術文書化やガバナンス分析に役立つ語彙を提供する。



**新規性**
6次元（機構・目的・データ要件・永続性・構造的範囲・モデル種別）からなる分類体系を導入し、ポストトレーニング適応技術を横断的に整理した点が新しい。



**Tags:** post-training adaptation, taxonomy, survey, AI governance, model editing
...
**日本語要約**
本研究は、ICUの心停止患者における死亡予測のために、agentic LLMによる臨床的に整合した特徴選択と、コンパクトな量子リカレントネットワークを組み合わせたQuanTiMedAIを提案している。MIMIC-IVコホートでの実験では、入院初期の静的要約に依存する従来法の限界を補い、時系列的な生理変化を考慮した予測でAUROC 0.852を達成した。



**新規性**
agentic LLMによる特徴選択と量子強化時系列モデルを統合し、少ないパラメータ数で心停止死亡予測性能を向上させた点が新規である。



**Tags:** 心停止, 死亡予測, 時系列解析, 量子機械学習, LLM
...
**日本語要約**
EnvACEは、長期的なツール使用を行うLLMエージェントの学習において、外部環境とのやり取りの代わりに「world rehearsal」を用いる手法です。ポリシーが行動と環境応答の両方を交互に生成し、その関係をパラメータ内に内在化することで、意思決定を支えるエージェントの世界モデルを学習します。BFCL-v4、tau^2-Bench、VitaBench、FinMCP-Benchで強い性能を示し、テスト時の私的なリハーサルでも追加の外部相互作用なしに性能向上が得られます。



**新規性**
外部環境に依存せず、行動と環境応答を交互に生成するworld rehearsalをエンドツーエンドで最適化し、エージェント内部に世界モデルを内在化する点が新規です。
...
**日本語要約**
LLMベンチマーク評価は安全性や信頼性の根拠として使われる一方、実運用でのアクセス形態やWeb検索の有無、複数回実行時のばらつきは十分に考慮されていない。本研究はChatGPTのUIとAPIを、Web検索の有無を含めてBBQとSafetyBenchの401プロンプトで比較し、精度だけでなく応答の一貫性、類似性、引用の根拠付け、棄権行動も分析した。その結果、UIはAPIより低精度で、Web検索は精度を最大8ポイント低下させ、同一プロンプトでも最大21%で応答が不一致だった。



**新規性**
単純な精度指標だけでなく、モダリティ・Web検索・反復実行・引用/棄権行動まで含めてLLM安全性評価の見落としを実証的に示した点が新しい。



**Tags:** LLMベンチマーク, 安全性評価, Web検索, 応答の一貫性, 引用
...
**日本語要約**
深層学習、特にTransformer系モデルは時系列予測で高い性能を示している一方、RAGの適用はまだ限定的である。本研究では、類似する時系列を検索して参照情報として利用するTS-RAGを提案し、入力系列と検索系列の情報を統合することで予測性能の向上を図る。実験では、複数の実世界ベンチマークで一貫して最先端性能を達成した。



**新規性**
時系列予測にRAGを適用し、単純な連結ではなく専用のreference tokensで入力系列と検索系列を効果的に融合する点が新しい。
...
**日本語要約**
外部コンテキストに依存する言語モデルにおいて、誤った信号への耐性だけを高めると、正しい文脈まで無視してしまう問題があると指摘し、選択的に信頼する能力として再定式化している。人手注釈付きベンチマークMISTと、誤った信号が正解を誤答に変える頻度を測るSC2Wを提案し、複数モデルでこの脆弱性が普遍的であることを示した。



**新規性**
誤誘導への耐性ではなく「文脈を選択的に信頼する能力」を評価・最適化する枠組みとして、MISTとSC2W、および四条件を均衡させたDPOベースのSCOPEを提案した。



**Tags:** language models, context trust, preference optimization, benchmark, robustness
