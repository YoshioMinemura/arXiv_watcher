...
Generated at: 2026-09-01 10:48 JST
Run ID: 6ee7ea23-eccf-4c34-9777-c20bfd5a379e

## Summary
- Queries executed: 1
- New matched papers: 25
...
- arXiv: [2608.29613](https://arxiv.org/abs/2608.29613)
- PDF: [link](https://arxiv.org/pdf/2608.29613.pdf)
- Published: 2026-08-30 16:08 JST
- Updated: 2026-08-30 16:08 JST
...
**日本語要約**
本研究は、LLMの挙動を制御するfunction vectors（FVs）が、感情認識という意味的に複雑な多ラベル分類タスクにおいて、言語をまたいで転移可能かを検証している。ソース言語で抽出したFVsを別言語のゼロショット設定に適用すると性能が大きく向上し、FVsが言語固有の表層パターンではなく、タスクに関係する言語非依存の信号を捉えていることが示唆された。



**新規性**
感情認識を用いて、FVsのクロスリンガル転移性を体系的に評価し、少数ショット推論に近い効果を追加デモなしで実現できることを示した点が新規である。



**Tags:** large language models, function vectors, cross-lingual transfer, emotion recognition, multilingual NLP
...
### 2. Beyond Surface Alignment: Grounding the Dynamics of Situational Understanding and Generative Control in LLMs
- arXiv: [2608.29610](https://arxiv.org/abs/2608.29610)
- PDF: [link](https://arxiv.org/pdf/2608.29610.pdf)
- Published: 2026-08-30 16:04 JST
- Updated: 2026-08-30 16:04 JST
...
**日本語要約**
本論文は、LLMの現行のアライメント調整が流暢さや安全性などの表層的な振る舞いに偏っており、状況理解や生成過程の基盤が十分に整っていないと主張する。SitTestやReCodeで状況的グラウンディングの失敗を、Branching FactorやHindsightで生成のグラウンディング不足を評価し、動的制御の枠組みとしてcontext engineeringや協調的生成手法を提案する。



**新規性**
状況理解と生成制御をそれぞれグラウンディングの観点から分解して評価し、表層的アライメントを超える「Grounded Alignment」の枠組みを提示した点が新しい。



**Tags:** LLM, alignment, grounding, generation control, situational understanding
...
### 3. SUP-MIMIC: A Multi-Task Clinical Diagnosis Benchmark for Evaluating LLMs' Robustness to Contradictory Evidence
- arXiv: [2608.29582](https://arxiv.org/abs/2608.29582)
- PDF: [link](https://arxiv.org/pdf/2608.29582.pdf)
- Published: 2026-08-30 14:59 JST
- Updated: 2026-08-30 14:59 JST
...
**日本語要約**
本研究は、MIMIC-IV-v3.1を用いた多タスク臨床診断ベンチマークSUP-MIMICを提案し、LLMが診断の曖昧性や異なる症状から同一疾患に至る状況にどれだけ頑健に対応できるかを評価する。Basic Assessmentに加え、類似症例の識別を行うDiagnostic Divergence Taskと、異なる病態経路から同一診断を見つけるDiagnostic Convergence Taskを導入している。評価の結果、最先端LLMはこれらのタスクで性能が大きく低下し、臨床推論における脆弱性と「健康」予測への保守的バイアスが示された。



**新規性**
診断の一対多・多対一の両方の曖昧性を評価するために、MIMIC-IV上でDDTとDCTを含む多タスクベンチマークを構築した点が新しい。



**Tags:** LLM, clinical diagnosis, benchmark, robustness, MIMIC-IV
...
### 4. Integrating adaptive human behavior into epidemic models with large language models
- arXiv: [2608.29535](https://arxiv.org/abs/2608.29535)
- PDF: [link](https://arxiv.org/pdf/2608.29535.pdf)
- Published: 2026-08-30 12:46 JST
- Updated: 2026-08-30 12:46 JST
...
**日本語要約**
感染症の伝播は人々の行動変化に強く依存するが、その文脈依存的な行動を疫学モデルに組み込むことは難しい。本研究は、LLMを用いて行動応答を推定し、それを年齢構造化接触行列として機械論的疫学モデルに結合するGABLEを提案した。フランスのCOVID-19に適用した結果、接触構造の再現と短期予測で有効であり、将来の政策評価にも利用できた。



**新規性**
LLMを「行動層」として疫学モデルに組み込み、感染状況や政策条件に応じた適応的な接触行列を生成して予測と政策評価を可能にした点が新しい。



**Tags:** 感染症モデル, 大規模言語モデル, 行動モデリング, COVID-19, 政策評価
...
### 5. LLMODE: Aligning ODEs with LLMs via Gated Token Injection for Irregular Spatio-Temporal Forecasting
- arXiv: [2608.29640](https://arxiv.org/abs/2608.29640)
- PDF: [link](https://arxiv.org/pdf/2608.29640.pdf)
- Published: 2026-08-30 17:02 JST
- Updated: 2026-08-30 17:02 JST
...
**日本語要約**
LLMODEは、不規則に観測される時空間データに対して、凍結したLLMを用いた予測を行うためのトークン効率の高い枠組みです。グラフ対応のODEエンコーダで不規則観測を連続時間の潜在軌道に復元し、それを固定数の動的メモリトークンと文脈メモリトークンに圧縮してLLMへ注入します。



**新規性**
不規則時空間予測のために、ODE表現と固定予算のリサンプリング、さらにゲート付きクロスアテンションを組み合わせて、凍結LLMへ外部時空間情報を制御的に注入する点が新規です。



**Tags:** 時空間予測, 不規則観測, 大規模言語モデル, ODE, クロスアテンション
...
### 6. Evaluating LLMs on Conversational Text-to-SQL under Chain Ambiguity and Intent Drift
- arXiv: [2608.29543](https://arxiv.org/abs/2608.29543)
- PDF: [link](https://arxiv.org/pdf/2608.29543.pdf)
- Published: 2026-08-30 13:24 JST
- Updated: 2026-08-30 13:24 JST
...
**日本語要約**
本論文は、会話型Text-to-SQLにおけるユーザ意図の変化を評価するため、chain ambiguity と intent drift に着目したベンチマーク TIDE-Bench を提案する。BIRD由来の514本のanchor SQLを基に1,542サンプルを構築し、実行精度だけでなく chain identification と drift recognition-resolution を測る専用指標を導入した。12種類のLLM評価では、chain identification のボトルネックや intent drift の認識・解決の難しさが確認された。



**新規性**
会話中の曖昧さの連鎖と意図の撤回・置換を対象に、実行精度以外の専用評価指標を備えたベンチマーク TIDE-Bench を新たに構築した点。



**Tags:** text-to-SQL, conversational AI, LLM evaluation, benchmark, intent drift
...
### 7. AgenticRag-R1: Agentic Reinforcement Learning with Stack Memory for Multi-Step Reasoning, Retrieval and Memorizing
- arXiv: [2608.29622](https://arxiv.org/abs/2608.29622)
- PDF: [link](https://arxiv.org/pdf/2608.29622.pdf)
- Published: 2026-08-30 16:31 JST
- Updated: 2026-08-30 16:31 JST
...
**日本語要約**
Retrieval-Augmented Generation（RAG）の既存手法は、複雑な多段推論において適応的な検索や中間文脈の更新が難しいという課題がある。本論文は、推論・検索・記憶をメモリスタックと細粒度の行動空間で統合し、階層的な報酬設計と情報量に基づく軌跡選別によって長期的学習を促すAgenticRag-R1を提案する。多様な多段・オープンドメイン・エージェント推論ベンチマークで、強いベースラインを一貫して上回ることを示した。



**新規性**
メモリスタックと細粒度アクション空間を用いて、推論・検索・記憶を深く統合し、長期的なRL学習を可能にした点が新しい。



**Tags:** RAG, reinforcement learning, multi-step reasoning, memory, agentic reasoning
...
### 8. Agent Zero Memory: Provenance-Aware Long-Term Memory for LLM Agents
- arXiv: [2608.29606](https://arxiv.org/abs/2608.29606)
- PDF: [link](https://arxiv.org/pdf/2608.29606.pdf)
- Published: 2026-08-30 15:55 JST
- Updated: 2026-08-30 15:55 JST
...
**日本語要約**
LLMエージェント向けに、会話・ファイル・接続ソースから得た情報を3つの並列メモリ（エピソード、知識グラフ、引用固定の階層文書メモリ）として保持する、出典追跡可能な長期記憶システムAgent Zero Memoryを提案している。各学習項目に由来・時刻・証拠を持たせ、回答時は開いた証拠にのみ基づいて引用することで、幻覚を構造的に抑え、根拠がない場合は推測せず棄却する。



**新規性**
単一の記憶構造に依存せず、3種類のメモリを並列に用いるうえ、証拠を開いたものだけ引用できるcitation lockを導入して、出典付きの忠実な長期記憶を実現した点が新しい。



**Tags:** LLM agents, long-term memory, provenance, knowledge graph, retrieval
...
### 9. LLM Judges as Raters: A Pre-Registered Audit of Severity, Halo, Reliability, and Version Instability in LLM Essay Scoring on Public Corpora
- arXiv: [2608.29517](https://arxiv.org/abs/2608.29517)
- PDF: [link](https://arxiv.org/pdf/2608.29517.pdf)
- Published: 2026-08-30 11:45 JST
- Updated: 2026-08-30 11:45 JST
...
**日本語要約**
LLMをエッセイ採点者として扱い、事前登録した評価で、厳しさ、ハロー効果、信頼性、バージョン間の不安定性を公開コーパス上で検証した研究である。ENEMとASAPの2言語・計2,377件のエッセイ、12のLLM judge、4社のモデルを用い、LLM間で採点の厳しさや版差が大きく、自己整合性は高い一方で人間採点者並みの精度には達しないことを示した。



**新規性**
LLM judgeを「採点者」とみなし、many-facet Raschやgeneralizability analysisなどの測定論的手法で、厳しさ・ハロー・版不安定性を事前登録ベースで体系的に監査した点が新規である。



**Tags:** LLM evaluation, essay scoring, rater effects, educational measurement, reliability
...
### 10. Predicting the Unpredictable: LLM-powered Long-term Chaotic Time Series Forecasting under Short-term Observations
- arXiv: [2608.29579](https://arxiv.org/abs/2608.29579)
- PDF: [link](https://arxiv.org/pdf/2608.29579.pdf)
- Published: 2026-08-30 14:56 JST
- Updated: 2026-08-30 14:56 JST
...
**日本語要約**
本論文は、短期観測しか得られない状況でのカオス時系列の長期予測に取り組み、LLMを用いた予測枠組みPAC-LLMを提案する。提案手法は、位相空間特徴とテキスト情報を融合し、補助特徴モジュールとゲート付き重み付け機構により多変量の結合情報を選択的に統合する。代表的なカオス系での実験では、既存のfine-tunedおよびzero-shot手法より短期・長期予測の両方で優れた性能を示した。



**新規性**
LLMの時系列予測能力を、カオス系の位相空間構造に合わせて拡張するために、位相空間認識型の適応的融合フレームワークPAC-LLMを導入した点が新規である。



**Tags:** chaotic time series, long-term forecasting, LLM, phase-space, time series prediction
...
### 11. Which LLM for Which Work? Budgeted Model Allocation under Uncertain Evaluation
- arXiv: [2608.29560](https://arxiv.org/abs/2608.29560)
- PDF: [link](https://arxiv.org/pdf/2608.29560.pdf)
- Published: 2026-08-30 14:00 JST
- Updated: 2026-08-30 14:00 JST
...
**日本語要約**
固定AI予算の下で、各LLMをどの反復業務に割り当てるべきかを考える問題を扱い、性能表が不確実でも最適割り当てが一意に定まるかを検証する枠組みを提案する。推定された性能表と最悪ケースの性能表の2回の最適化結果が一致すれば割り当てを証明でき、不一致なら追加評価が有効な組合せを特定できる。実データと実験では、評価情報の不足が割り当て最適化より大きな損失要因であることが示された。



**新規性**
不確実な性能表の下で、最適割り当ての頑健性を「2回解く」ことで証明する厳密な判定法と、必要な箇所に評価を集中するCASEを提案した。



**Tags:** LLM割り当て, 予算制約最適化, 不確実性下の意思決定, 因果推論, アクティブ評価
...
### 12. Argument-Aware Semantic Alignment of Normative Texts: A Toulmin-Based Neuro-Symbolic Approach
- arXiv: [2608.29529](https://arxiv.org/abs/2608.29529)
- PDF: [link](https://arxiv.org/pdf/2608.29529.pdf)
- Published: 2026-08-30 12:30 JST
- Updated: 2026-08-30 12:30 JST
...
**日本語要約**
専門的な規範テキストの意味対応付けでは、用語や構文が異なるため、単純な語彙的・分散表現ベースの類似度だけでは不十分である。本研究は、Toulminの議論構造（claim, grounds, warrant, qualifier, backing）をLLMで明示化し、神経記号的なアラインメントモデルに組み込むことで、要求事項の対応付けを行う手法を提案する。NERC-CIPとNIST-CSFのマッピングベンチマークでは、議論由来の特徴量が既存の神経記号的ベースラインを上回り、特にwarrant関連特徴が有効だった。



**新規性**
規範テキストのアラインメントにToulmin型の議論構造を中間表現として導入し、LLMによる議論明示化と神経記号的特徴統合で性能向上を示した点が新規である。



**Tags:** semantic alignment, normative texts, Toulmin model, neuro-symbolic, cybersecurity standards
...
### 13. MI-Distillation: Selecting from Model-Interpolated Instruct-Reasoning Data Spectrum for Chain-of-Thought Distillation
- arXiv: [2608.29623](https://arxiv.org/abs/2608.29623)
- PDF: [link](https://arxiv.org/pdf/2608.29623.pdf)
- Published: 2026-08-30 16:32 JST
- Updated: 2026-08-30 16:32 JST
...
**日本語要約**
大規模推論モデルの長いChain-of-Thoughtを小型学生モデルへ蒸留する際、直接の長文監督は短い推論より効果が低い場合があることを、勾配の大きさと更新方向の観点から分析している。これに基づき、推論情報量と学生モデルへの分布整合性の両立を目指すMI-Distillationを提案し、実験で長文CoT蒸留の性能向上を示した。



**新規性**
モデル補間によりInstruct-Reasoningデータの連続スペクトルを構成し、SeqLSSで学生にとって情報的かつ学習可能な推論経路を選択する点が新しい。



**Tags:** chain-of-thought, distillation, reasoning models, model interpolation, small models
...
### 14. JPO: Juris Policy Optimization for Structured Legal Reasoning in Criminal Judgment Prediction
- arXiv: [2608.29616](https://arxiv.org/abs/2608.29616)
- PDF: [link](https://arxiv.org/pdf/2608.29616.pdf)
- Published: 2026-08-30 16:14 JST
- Updated: 2026-08-30 16:14 JST
...
**日本語要約**
本研究は、中国の刑事判決予測において、法条文・罪名・量刑を単なる最終ラベルではなく、法的に整合した段階的推論として扱うための後学習フレームワークJPOを提案する。教師生成の根拠文で4段階の標準化推論を学習させた後、予測品質、推論構造の完全性、段階間整合性を組み合わせた報酬で強化学習を行う。複数のオープンソースモデルと3つの中国法ベンチマークで、判決予測性能と推論品質の両方が改善した。



**新規性**
法的推論を4段階の標準化プロセスとして明示的に学習し、構造・整合性を含む複合報酬で最適化する点が新しい。



**Tags:** 刑事判決予測, 法的推論, 強化学習, 中国法, 大規模言語モデル
...
### 15. Call Neighbours Yourself: Graph Walks with Destination-Conditioned On-Policy Self-Distillation
- arXiv: [2608.29588](https://arxiv.org/abs/2608.29588)
- PDF: [link](https://arxiv.org/pdf/2608.29588.pdf)
- Published: 2026-08-30 15:19 JST
- Updated: 2026-08-30 15:19 JST
...
**日本語要約**
テキスト属性付きグラフ（TAG）上の推論では、ノードのテキストと近傍から得られる証拠を組み合わせる必要がありますが、既存手法は事前に選んだ固定近傍に基づいて推論するため、推論中に不足情報を補うことができません。本論文は、LLMがトポロジー制約付きのグラフウォーク行動を通じて近傍探索を能動的に行うフレームワークCall Neighbours Yourself（CNY）を提案し、標準TAGベンチマークで固定コンテキストの後処理ベースラインより一貫して高い性能を示しました。



**新規性**
近傍選択自体を推論過程に組み込み、destination-conditioned on-policy self-distillation により、探索後の選好変化を行動レベルの学習信号として利用した点が新しいです。



**Tags:** text-attributed graphs, LLM reasoning, graph walks, self-distillation, neighbour exploration
...
### 16. SemTrace: Source-Grounded Semantic Signatures for Tracing LLM Exposure to Protected Documents
- arXiv: [2608.29575](https://arxiv.org/abs/2608.29575)
- PDF: [link](https://arxiv.org/pdf/2608.29575.pdf)
- Published: 2026-08-30 14:41 JST
- Updated: 2026-08-30 14:41 JST
...
**日本語要約**
SemTraceは、保護された文書コピーに対してLLMが影響を受けたかどうかを検出するための、ソースに根ざした意味的ウォーターマーク手法です。文書内で直接支持される事実命題から文書固有の二値シグネチャを作り、レビュー生成後に自然言語推論モデルでそのビットを復号して照合します。



**新規性**
表層的なトークン操作ではなく、文書由来の事実命題に基づく意味的シグネチャを用いて、モデル非依存の露出検出を行う点が新規です。



**Tags:** LLM, watermarking, provenance, semantic signature, document tracing
...
### 17. Conducting Stylistic Analysis of Paintings through an Art-History Agent
- arXiv: [2608.29644](https://arxiv.org/abs/2608.29644)
- PDF: [link](https://arxiv.org/pdf/2608.29644.pdf)
- Published: 2026-08-30 17:10 JST
- Updated: 2026-08-30 17:10 JST
...
**日本語要約**
本研究は、絵画の様式分析を自動化するAIフレームワークを提案する。大規模な絵画データで学習したViTの埋め込みを疎辞書学習で共通特徴に分解し、LLMが関連作品とキュレーター文を参照して特徴を解釈し、最終的に作品の記述や比較を生成する。



**新規性**
視覚特徴をそのまま分類するのではなく、疎辞書学習とLLMを組み合わせて、絵画の様式を人文学的な記述へ変換する点が新しい。



**Tags:** 絵画分析, 様式分析, Vision Transformer, 大規模言語モデル, 計算美術史
...
### 18. Detect Before You Attribute: Cascade Failure Attribution for Multi-Agent Systems
- arXiv: [2608.29646](https://arxiv.org/abs/2608.29646)
- PDF: [link](https://arxiv.org/pdf/2608.29646.pdf)
- Published: 2026-08-30 17:17 JST
- Updated: 2026-08-30 17:17 JST
...
**日本語要約**
LLMベースのエージェントは複雑なタスクを解ける一方で実行失敗に弱く、失敗原因の特定が重要です。本研究は、まず異常な実行を検出してから、その結果を基に下流の属性推定を行うプラグイン型フィルタDUOTRACEを提案し、長い軌跡での文脈劣化を抑えつつ失敗属性推定を支援します。



**新規性**
検出先行の枠組みを導入し、VAEベースの異常検出に双視点の意味・構造表現、Tree-LSTM、データ拡張を組み合わせて、長いエージェント軌跡から有効な証拠を絞り込む点が新規です。



**Tags:** LLMエージェント, 失敗原因推定, 異常検出, マルチエージェント, 軌跡解析
...
### 19. The Emergent Symbolic Structure of Artificial Neural Networks
- arXiv: [2608.29530](https://arxiv.org/abs/2608.29530)
- PDF: [link](https://arxiv.org/pdf/2608.29530.pdf)
- Published: 2026-08-30 12:32 JST
- Updated: 2026-08-30 12:32 JST
...
**日本語要約**
本研究は、ニューラルネットワークの内部表現が見かけ上は連続ベクトルであっても、実際には記号的な構造で近似できる可能性を示す。小規模なリスト操作ネットワークから大規模言語モデルまで、算術・論理・コード・言語の各領域で、閉形式の記号構造による近似に置き換えても挙動が大きく変わらないことを示した。



**新規性**
ニューラルネットワークの表現生成過程全体を、閉形式の記号構造を与える式で置き換えられることを示し、さらに内部表現への精密な介入でLLMの挙動を局所的に変更できる点が新規である。



**Tags:** neural networks, symbolic structure, large language models, representation learning, interpretability
...
### 20. Towards a Systems Foundation for Agentic Skills: Architecture, Lifecycle, and Security
- arXiv: [2608.29596](https://arxiv.org/abs/2608.29596)
- PDF: [link](https://arxiv.org/pdf/2608.29596.pdf)
- Published: 2026-08-30 15:36 JST
- Updated: 2026-08-30 15:36 JST
...
**日本語要約**
本論文は、長期的で複雑なタスクにおけるLLMエージェントの信頼性・文脈消費・実行安定性の課題に対し、「agentic skills」を外部化された手続き知識として捉える統一的なシステム基盤と参照アーキテクチャを提案する。さらに、発見、記述、保存、検索、合成、実行、適応、評価、セキュリティまでを含む9段階のライフサイクルを整理し、実装例や公開レジストリ、脅威と防御、未解決課題を体系的に論じる。



**新規性**
agentic skills を、計画と決定的実行環境をつなぐ再利用可能・可搬な実行可能アーティファクトとして定式化し、そのライフサイクル全体を含む統一的なシステム基盤を提示した点が新規である。



**Tags:** LLMエージェント, システムアーキテクチャ, 手続き知識, セキュリティ, 継続学習
