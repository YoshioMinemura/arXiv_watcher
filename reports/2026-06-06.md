...
Generated at: 2026-06-06 09:18 JST
Run ID: a679fce1-6ba4-4b09-8f21-cfa19b349950
...
**日本語要約**
本研究は、感染症流行時の個人の意思決定を大規模言語モデルで生成し、国勢調査ベースの合成人口に組み込んだ空間的に基盤のあるエージェントベースシミュレーションを提案する。サンフランシスコとアトランタで自己申告のインフルエンザ様症状の報告行動を比較し、所得と教育が報告率の主な要因であり、地理やモデル選択、メッセージの枠組みも小さいながら一貫した影響を持つことを示した。



**新規性**
LLMによる意思決定を、都市内の空間分布を反映した合成人口に統合し、社会的・地理的異質性を同時に扱う感染症行動シミュレーション枠組みを構築した点が新しい。
...
**日本語要約**
大規模言語モデルを用いたマルチエージェントシステムにおける「協働能力」を、CSCWの知見に基づいて体系的に調べるためのシミュレーション枠組みCollabSimを提案している。タスク結果だけでなく、相互作用条件の制御や行動レベルのプロービングを通じて、共通理解の形成や不一致修復などの側面を分析できる。4つのLLMでの実験により、条件の影響やモデル間の性能差、タスク依存の設計効果を捉えられることを示した。



**新規性**
協働能力を理論に基づいて定義し、相互作用条件の制御と内部状態の行動レベル解析を組み合わせて評価する、CSCW基盤のMAS評価フレームワークを提示した点。



**Tags:** multi-agent systems, LLM agents, collaborative competence, CSCW, evaluation framework
...
**日本語要約**
大規模言語モデルの推論を、離散的なテキストのCoTではなく、連続潜在状態で行う「latent reasoning」によって効率化する手法を提案している。提案手法NF-CoTは、normalizing flowを用いて連続的な思考を確率的にモデル化し、左から右への生成、KV-cacheの利用、尤度計算を可能にする。コード生成ベンチマークでは、explicit CoTや既存のlatent reasoning手法より高い性能と低い中間推論コストを示した。



**新規性**
normalizing flowsをLLM内部の連続思考表現に組み込み、latent reasoningでありながら自回帰生成・KV-cache・厳密尤度を維持した点が新規である。
...
**日本語要約**
本論文は、消滅危機にある極低資源言語であるKomi-Yazva語とロシア語の初の並列コーパスを提示し、LLMによる翻訳を評価するための明示的な評価プロトコルも併せて提案する。データセットは457組の文対応と74の物語テキストからなり、リークを避けるための出典情報や物語ID、story-level cross-validation、厳密な出力検証、参照ベースと判定器ベースの両方の指標を含む。ゼロショットと検索ベースのfew-shot設定で比較した結果、LLMは一定の翻訳能力を示す一方、モデル系列やプロンプト条件で性能差が大きく、少数の検索文脈を用いるfew-shotが一貫して改善をもたらした。



**新規性**
Komi-Yazva--Russianの初の並列コーパスと、リーク耐性を意識した再現可能な評価プロトコルを同時に提供し、極低資源翻訳の評価基盤として位置づけた点が新規である。
...
**日本語要約**
本研究は、成人が因果学習で苦手とされる連言的因果規則について、受動的観察ではなく能動的探索を許した場合にその偏りがどう変わるかを調べた。改変したblicket detector課題で、能動的探索は連言条件の推論成績を大きく改善したが、それでも選言条件より多くの試行を要した。



**新規性**
成人とLLMを同一の能動探索課題で比較し、推論精度だけでなく探索効率まで含めて、連言・選言条件の差を評価した点が新規である。



**Tags:** 因果学習, 能動探索, 成人, 大規模言語モデル, blicket detector
...
**日本語要約**
本研究は、オンライン議論におけるLLMベースのスタンスシミュレーションが、ユーザー固有の信念をどの程度反映するのかを調べるため、反実仮想的な文脈修正による監査枠組みを提案する。元の会話文脈を制御的に修正し、テキストのみとミームを含むマルチモーダルの両方で再シミュレーションすることで、スタンスの方向変化量と遷移率を評価した。



**新規性**
文脈を反実仮想的に修正してLLMのスタンスシミュレーションの文脈感度を監査する評価枠組みを提示し、テキスト・マルチモーダル双方で有効なスタンス遷移を示した点。



**Tags:** LLM, stance simulation, online discussions, context sensitivity, multimodal
...
**日本語要約**
MLEvolveは、LLMベースの自己進化型マルチエージェント枠組みとして、機械学習アルゴリズムのエンドツーエンドな発見を行う手法です。Progressive MCGSによる探索の拡張、Retrospective Memoryによる経験の再利用、適応的なコーディングモードの分離により、長期的な最適化を安定化します。MLE-Benchでは12時間予算下で複数指標のSOTAを達成し、数学的アルゴリズム最適化でもAlphaEvolveを上回りました。



**新規性**
探索木をProgressive MCGSへ拡張して枝間の情報共有を可能にし、さらに記憶機構と戦略計画・コード生成の分離を組み合わせて長期自己進化を実現した点が新規です。



**Tags:** LLM, multi-agent, machine learning engineering, search, self-evolution
...
**日本語要約**
本研究は、豊富な言語コンテキストを与えたうえで、未見の言語翻訳を強化学習で学習させる手法を提案する。報酬には表層的な翻訳指標 chrF を用い、モデルが文脈中の言語情報を抽出して活用できることを示し、完全に未見の言語に対して in-context learning や教師あり微調整より良い翻訳性能を得た。



**新規性**
未見言語の翻訳に対して、特定言語の暗記ではなく「文脈中の言語知識を利用するメタスキル」を強化学習で獲得させる点が新しい。



**Tags:** 機械翻訳, 大規模言語モデル, 強化学習, 低資源言語, 文脈学習
...
**日本語要約**
本研究は、推論言語モデルのRL微調整における遅延報酬問題に対し、Chain-of-Thoughtの各セグメントへ報酬を再配分する手法を提案する。提案手法RREDCoTは、追加生成を行わずにモデル自身を用いて最適な報酬再配分を近似し、長い文脈での高粒度な信用割当を可能にする。



**新規性**
追加のモンテカルロ生成を使わず、モデル自身でCoTのセグメント単位の報酬再配分を近似する点が新規である。



**Tags:** reinforcement learning, reasoning models, chain-of-thought, reward redistribution, credit assignment
...
**日本語要約**
本論文は、LLMの事前学習を安定化するために、重みを多項式前処理で再パラメータ化するPC layerを提案する。PC moduleは重み行列の特異値スペクトルを整形し、学習後は元のアーキテクチャに統合できるため推論時の追加コストはない。Llama-1Bの事前学習で、AdamWとMuonの両最適化手法に対して標準Transformerより有効であることを示している。



**新規性**
低次数の多項式前処理によって重み行列の特異値分布を制御し、学習安定性と収束性を改善するPC layerを提案した点。
...
**日本語要約**
長文コンテキスト推論におけるデコード効率の課題に対し、KV共有アーキテクチャ上で動作するcross-layer sparse attention（CLSA）を提案している。トークン単位のtop-k選択を各層で共有し、ルーティング計算を一度にまとめることで、細粒度の選択性を保ちながら推論時のオーバーヘッドを削減する。



**新規性**
KVキャッシュだけでなくルーティングインデックスも層間で共有し、token sparse attentionの精度と効率を両立させた点が新規である。



**Tags:** long-context inference, sparse attention, KV sharing, LLM efficiency
...
**日本語要約**
大規模言語モデルに対する「ポパー流の反証主義者として振る舞う」コード生成スキルについて、著者らは長さを揃えたプラセボ、ラベルのみのスキャフォールド、実行オラクル、自己評価監査を含む事前登録済みの二層アブレーションで検証した。フロンティアモデルでは条件間の差は見られず、小型モデルでは構造化されたプロンプトが正答率を押し上げた一方で、スキル本体はラベルのみのスキャフォールドを超える分離可能な利益を示さなかった。



**新規性**
ポパー的な手続き内容そのものではなく、プロンプトのスキャフォールド構造が性能向上を説明していることを、事前登録済みの対照実験で切り分けて示した点が新しい。



**Tags:** large language models, code generation, prompt engineering, ablation study, evaluation bias
...
**日本語要約**
Vortexは、ページ中心のテンソル抽象化とPython埋め込みのフロントエンドを備えた、疎な注意機構を表現・実装しやすいLLMサービング用システムです。これにより、新しい疎注意アルゴリズムの試作、デプロイ、評価を高速化し、理論上の効率向上を実運用のスループット改善につなげます。



**新規性**
疎注意アルゴリズムを広く記述できるフロントエンドと、現代的なLLMサービングスタックに統合された高効率バックエンドを組み合わせ、AIエージェントによる自動探索・改良まで支援する点が新規です。
...
**日本語要約**
大規模言語モデルが多様な音声入力を扱うための単一エンコーダとして、SSLと教師あり基盤モデルの知識を統合した汎用音声エンコーダUSAD 2.0を提案している。ドメイン間の教師不一致に対応するdomain-aware distillation、音楽ドメインへの拡張、下流用途向けの第二段階の教師あり蒸留を導入し、さらに深さ方向のスケーリングで10億パラメータまで拡張した。



**新規性**
SSLと教師あり基盤モデルを統合し、ドメイン認識型蒸留と二段階蒸留、さらに1B規模へのスケーリングを組み合わせて汎用音声理解を強化した点が新しい。



**Tags:** audio encoder, knowledge distillation, foundation models, universal audio understanding, music domain
...
**日本語要約**
医用画像AIは単独画像の読影では高性能だが、臨床で重要な過去画像や類似症例との比較推論には十分対応していない。本研究は、比較読影をentity-awareなクロス画像推論として定式化し、参照症例検索と経時的な比較解釈の両方を支える枠組みを提案する。



**新規性**
通常診療の画像・レポート対から大規模な比較画像資源MedReCo-DBを構築し、entity条件付き検索と比較VQAの監督信号を用いて、症例検索用のMedReCoと経時変化生成用のMedReCo-VLMを学習可能にした点。



**Tags:** medical imaging, vision-language, comparative reasoning, case retrieval, longitudinal analysis
...
**日本語要約**
本研究は、LLMエージェントに対して「この資源には入らないでほしい」と伝えるための、既存プロトコル上で送るインバンドの拒否シグナル「Recuse Signal」を提案し、その有効性を実運用ホスト上で検証した。SSHとPostgreSQL向けの軽量アダプタを実装し、ベンチマークではシグナルありでエージェントが自発的に離脱するかを観察した。



**新規性**
認証やアクセス制御の代替ではなく、協調的な拒否意思表示として機能するインバンド拒否シグナルを定義し、LLMエージェントが実際にそれに従うかを初めて実験的に測定した点が新規である。



**Tags:** LLMエージェント, アクセス制御, SSH, PostgreSQL, セキュリティ
...
**日本語要約**
本論文は、LLMエージェントが人間の協働相手として機能するために必要な、自己推論・相手の意図・共有目標のメンタルモデルを扱う重要性を指摘しています。そのために、社会科学のMap Taskに基づく人間同士の協働データセットALMANACを構築し、2,987件の協働行動に対して行動レベルのメンタルモデル注釈を付与しました。



**新規性**
人間の協働過程における行動単位のメンタルモデル注釈を持つ実データセットを提示し、LLMの次行動予測とメンタルモデル推定を評価できる点が新しいです。
...
**日本語要約**
離散拡散言語モデルは、応答全体を並列に反復的に復元する過程で、各ステップの低信頼トークンを破棄します。本研究は、その破棄されるトークンが早期の実体名などを含む有用な先読み信号になることに着目し、復元途中のトークンを使って検索を動的に行うRAG手法を提案します。5つのマルチホップQAベンチマークで、訓練不要かつ検索器非依存のまま既存の拡散系・自己回帰系の訓練不要ベースラインを上回りました。



**新規性**
拡散言語モデルの低信頼な先読みトークンを検索誘導に利用する、訓練不要・検索器非依存の動的RAG枠組みSARDIを提案した点。
