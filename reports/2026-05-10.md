...
Generated at: 2026-05-10 09:00 JST
Run ID: fed9999c-77d2-421c-97d8-c9609209d55b
...
- arXiv: [2605.06660](https://arxiv.org/abs/2605.06660)
- PDF: [link](https://arxiv.org/pdf/2605.06660.pdf)
- Published: 2026-05-08 02:58 JST
- Updated: 2026-05-08 02:58 JST
...
**日本語要約**
大規模言語モデルは科学・数学問題の解答には強い一方で、妥当で難しく新規性のある問題を生成することは苦手であり、従来法では人手コストや報酬ハックによる無効問題が課題だった。本研究は、setter-solver に独立した verifier を加えた三者自己対戦に基づく VHG を提案し、問題の妥当性と難易度の両方で setter の報酬を決めることで、より良い難問生成を実現する。



**新規性**
独立した verifier を導入して、問題の妥当性と難易度を同時に制約する三者自己対戦型の hard problem generation 枠組み VHG を提案した。



**Tags:** 大規模言語モデル, 数学的推論, 問題生成, 自己対戦, 検証器
...
- PDF: [link](https://arxiv.org/pdf/2605.06597.pdf)
- Published: 2026-05-08 02:22 JST
- Updated: 2026-05-08 02:22 JST
...
**日本語要約**
本論文は、強い外部教師を使わずに大規模言語モデルを適応させる自己蒸留（self-distillation）を体系的に調べるための統一フレームワークUniSDを提案する。UniSDは、複数教師の合意、EMAによる教師安定化、トークンレベルの対照学習、特徴マッチング、divergence clippingを統合し、6ベンチマーク・6モデルで自己蒸留の有効性や各要素の役割を分析する。



**新規性**
自己蒸留の個別手法を統合的に比較・組み合わせる枠組みを提示し、どの条件で性能向上するかと各要素の相互作用を明らかにした点。



**Tags:** self-distillation, large language models, knowledge distillation, training stability, representation learning
...
- PDF: [link](https://arxiv.org/pdf/2605.06638.pdf)
- Published: 2026-05-08 02:48 JST
- Updated: 2026-05-08 02:48 JST
...
**日本語要約**
本研究は、LLMの長期的な推論能力に対するRLの学習スケーリングを調べるため、推論の深さ（horizon）と論理の表現力を独立に制御できる合成環境ScaleLogicを提案する。実験の結果、RLの必要計算量は推論深さに対してべき乗則に従い、さらに論理がより表現豊かになるほどスケーリング指数が大きくなることを示した。加えて、より表現力の高い設定で学習したモデルは、数学や一般推論ベンチマークへの転移性能が高く、計算効率も良い。



**新規性**
推論深さと論理表現力を独立に制御できる環境を用いて、RLのスケーリング則が表現力に強く依存することを体系的に示した点が新規である。



**Tags:** reinforcement learning, LLM reasoning, scaling law, logical reasoning, transfer learning
...
- PDF: [link](https://arxiv.org/pdf/2605.06635.pdf)
- Published: 2026-05-08 02:46 JST
- Updated: 2026-05-08 02:46 JST
...
**日本語要約**
LLMによる深層リサーチエージェントが生成する引用付きレポートについて、引用の実在性・関連性・事実整合性を評価するための再現可能なソースアトリビューション評価フレームワークを提案している。Markdownレポートからインライン引用をASTパーサで抽出し、実際の参照先コンテンツを取得して人手またはLLM評価で検証することで、引用品質と事実的信頼性の乖離を明らかにした。



**新規性**
再現可能なASTパーサを用いてLLM生成レポート中の引用を大規模に抽出・評価し、実際の引用先コンテンツまで取得して検証する初のソースアトリビューション評価枠組みを提示した。



**Tags:** LLM, source attribution, citation evaluation, deep research agents, fact checking
...
- PDF: [link](https://arxiv.org/pdf/2605.06623.pdf)
- Published: 2026-05-08 02:35 JST
- Updated: 2026-05-08 02:35 JST
...
**日本語要約**
LLMベースのマルチエージェントシステムにおいて、役割別プロンプトを自動的かつ反復的に最適化するフレームワークMASPOを提案している。局所的な妥当性だけでなく、後続エージェントの成功にどれだけ寄与するかを評価する仕組みにより、ラベルなしで局所目標と全体目標のずれを埋める。6つのタスクで既存手法を上回り、平均精度を2.9向上させた。



**新規性**
後続エージェントの成功を考慮した共同評価機構により、マルチエージェント全体の目的に沿ってプロンプトを最適化する点が新規である。



**Tags:** multi-agent systems, prompt optimization, LLM, evolutionary beam search, automatic evaluation
...
### 6. Crafting Reversible SFT Behaviors in Large Language Models
- arXiv: [2605.06632](https://arxiv.org/abs/2605.06632)
- PDF: [link](https://arxiv.org/pdf/2605.06632.pdf)
- Published: 2026-05-08 02:44 JST
- Updated: 2026-05-08 02:44 JST
...
**日本語要約**
本研究は、SFTで獲得された振る舞いを、推論時に重みを変更せず制御可能な疎なサブネットワーク（carrier）へ意図的に圧縮できるかを検討している。提案手法として、routing mask と重みを効用予算の下で同時最適化する LCDD と、抽出した carrier チャネルに対する activation matching により振る舞いを反転させる SFT-Eraser を示し、複数のモデル群で安全性・固定応答・文体の振る舞いに対して有効性を確認した。



**新規性**
SFT由来の振る舞いを、後付けの相関的解析ではなく、因果的に必要な疎な carrier として構成し、その構造を利用して推論時に振る舞いを反転・抑制できる点が新規である。



**Tags:** large language models, supervised fine-tuning, mechanistic interpretability, sparse subnetwork, behavior control
...
### 7. Beyond Negative Rollouts: Positive-Only Policy Optimization with Implicit Negative Gradients
- arXiv: [2605.06650](https://arxiv.org/abs/2605.06650)
- PDF: [link](https://arxiv.org/pdf/2605.06650.pdf)
- Published: 2026-05-08 02:55 JST
- Updated: 2026-05-08 02:55 JST
...
**日本語要約**
RLVRにおける従来のGRPOは正例・負例ロールアウトを用いますが、本論文は正例ロールアウトのみで学習するPositive-Only Policy Optimization（POPO）を提案しています。POPOは正例集合への重み付き重要度サンプリングにより、明示的な負例を使わずに学習を進め、LLMの数学ベンチマークでGRPOと同等以上の性能を示しました。



**新規性**
負例ロールアウトを使わず、正例の再配分から暗黙的な負勾配を生み出すRLVRフレームワークを提案した点が新規です。



**Tags:** reinforcement learning, LLM, policy optimization, math reasoning, RLVR
...
- arXiv: [2605.06605](https://arxiv.org/abs/2605.06605)
- PDF: [link](https://arxiv.org/pdf/2605.06605.pdf)
- Published: 2026-05-08 02:25 JST
- Updated: 2026-05-08 02:25 JST
...
**日本語要約**
多ターン対話におけるLLMの評価では、ジャイルブレイクやタスク成功のような事象が反復後に現れるため、計算資源が限られると観測が難しい。本論文は、時間経過での事象発生を下界付きで推定するための動的予算配分手法DAPROを提案し、予算制約を満たしつつ有限標本でのカバレッジ保証を与える。さらに、少ない計算資源下でジャイルブレイク率などの集団評価指標を低分散・不偏に推定できることを示す。



**新規性**
多ターンLLM評価に対して、静的配分ではなく理論的に妥当な動的予算配分を導入し、打ち切りと事象時刻の条件付き独立を仮定せずに有限標本保証を与えた点が新しい。



**Tags:** LLM評価, マルチターン対話, 動的予算配分, ジャイルブレイク, 生存解析
...
### 9. FedAttr: Towards Privacy-preserving Client-Level Attribution in Federated LLM Fine-tuning
- arXiv: [2605.06596](https://arxiv.org/abs/2605.06596)
- PDF: [link](https://arxiv.org/pdf/2605.06596.pdf)
- Published: 2026-05-08 02:21 JST
- Updated: 2026-05-08 02:21 JST
...
**日本語要約**
本論文は、連合学習によるLLM微調整において、セキュア集約のプライバシー保証を保ちながら、どのクライアントがウォーターマーク付き文書で学習したかを特定するためのクライアントレベル属性手法FedAttrを提案する。FedAttrは、2回のSA問い合わせの差分から各クライアント更新を推定し、ウォーターマーク検出器でスコアリングした後、複数ラウンドの結果を統合する。



**新規性**
セキュア集約の下でクライアント更新を差分推定し、ウォーターマーク検出を用いてクライアント単位の属性を可能にする点が新規である。



**Tags:** federated learning, LLM fine-tuning, privacy-preserving attribution, watermark detection, secure aggregation
...
- PDF: [link](https://arxiv.org/pdf/2605.06654.pdf)
- Published: 2026-05-08 02:57 JST
- Updated: 2026-05-08 02:57 JST
...
**日本語要約**
本論文は、LLMの事前学習とSFTで同じ最適化手法を使うと、下流タスク性能を維持しつつ事前学習知識の忘却をより抑えられることを観察し、これを optimizer-model consistency と呼ぶ。制御実験と理論解析により、最適化手法が活性に対する正則化効果を通じて事前学習済みチェックポイント周辺の地形を変え、それに対応した更新構造が忘却低減に重要であることを示す。



**新規性**
事前学習とSFTで同一の最適化手法を用いることが、忘却を抑える上で有利であるという optimizer-model consistency を提案し、その背景を活性への正則化効果として説明した。



**Tags:** LLM, fine-tuning, optimizer, catastrophic forgetting, theory
...
### 11. PairAlign: A Framework for Sequence Tokenization via Self-Alignment with Applications to Audio Tokenization
- arXiv: [2605.06582](https://arxiv.org/abs/2605.06582)
- PDF: [link](https://arxiv.org/pdf/2605.06582.pdf)
- Published: 2026-05-08 02:11 JST
- Updated: 2026-05-08 02:11 JST
...
**日本語要約**
PairAlignは、音声を離散トークン列として表現するために、自己整合に基づくシーケンスレベルのトークナイゼーションを学習する枠組みである。2つの内容保持ビュー間で相互にトークン列を予測させることで、長さ制御、終了位置、編集距離の保存、トークンの非退化性を同時に改善する。3秒音声で広い語彙利用と高いビュー間整合性を示し、TIMIT検索では編集距離ベースの検索性能を保ちながらアーカイブのトークン数を55%削減した。



**新規性**
音声トークナイゼーションを局所的な量子化ではなく、自己整合による条件付き系列生成として定式化し、編集距離保存と長さ制御を直接狙う点が新しい。



**Tags:** audio tokenization, self-alignment, sequence generation, edit distance, speech representation
...
- PDF: [link](https://arxiv.org/pdf/2605.06642.pdf)
- Published: 2026-05-08 02:51 JST
- Updated: 2026-05-08 02:51 JST
...
**日本語要約**
LLMを対話的エージェントとして長期的な意思決定に適応させるため、StraTAは初期状態からコンパクトな戦略を抽出し、その戦略に条件づけて後続の行動を生成する階層的な強化学習枠組みを提案する。ALFWorld、WebShop、SciWorldでの実験では、強いベースラインよりもサンプル効率と最終性能の両方が一貫して向上した。



**新規性**
軌跡全体の方針を明示的な戦略として抽象化し、戦略生成と行動実行を同時に学習する点が新しい。



**Tags:** LLMエージェント, 強化学習, 長期計画, 階層学習, サンプル効率
...
### 13. Algospeak, Hiding in the Open: The Trade-off Between Legible Meaning and Detection Avoidance
- arXiv: [2605.06619](https://arxiv.org/abs/2605.06619)
- PDF: [link](https://arxiv.org/pdf/2605.06619.pdf)
- Published: 2026-05-08 02:34 JST
- Updated: 2026-05-08 02:34 JST
...
**日本語要約**
本研究は、LLMがコンテンツ生成とモデレーションの両方に関与する状況で、Algospeakによる回避表現が「検出回避」と「意味の理解可能性」の間にトレードオフを生むことを定式化した。COVID-19誤情報を用いた実験では、意味回復と誤情報検出の2つの評価を行い、モジュレーション強度に応じた理解可能性の変化を分析した。



**新規性**
Majority Understandable Modulation (MUM) という、回避性を高めると多数の受け手の理解が失われる閾値概念を導入し、調整可能なAlgospeak変種生成フレームワークと700件のデータセットを提示した。



**Tags:** Algospeak, LLM, 誤情報検出, 自然言語処理, モデレーション
...
### 14. The Structural Origin of Attention Sink: Variance Discrepancy, Super Neurons, and Dimension Disparity
- arXiv: [2605.06611](https://arxiv.org/abs/2605.06611)
- PDF: [link](https://arxiv.org/pdf/2605.06611.pdf)
- Published: 2026-05-08 02:28 JST
- Updated: 2026-05-08 02:28 JST
...
**日本語要約**
本論文は、LLMにおけるattention sinkの構造的起源を、自己注意のvalue aggregationが生む分散の不一致として説明し、さらにFFN層のsuper neuronの活性化がその不一致を増幅することを示す。加えて、チャネル疎なdown-projectionが先頭トークン表現の次元不均衡を引き起こし、attention sinkが構造的なアンカーとして形成されると論じる。



**新規性**
attention sinkの発生を、分散不一致→super neuronによる増幅→次元不均衡という因果連鎖として機械論的に説明し、head-wise RMSNormによる安定化で収束改善を示した点。



**Tags:** attention sink, LLM, mechanistic interpretability, self-attention, RMSNorm
...
### 15. Automated Clinical Report Generation for Remote Cognitive Remediation: Comparing Knowledge-Engineered Templates and LLMs in Low-Resource Settings
- arXiv: [2605.06594](https://arxiv.org/abs/2605.06594)
- PDF: [link](https://arxiv.org/pdf/2605.06594.pdf)
- Published: 2026-05-08 02:20 JST
- Updated: 2026-05-08 02:20 JST
...
**日本語要約**
本研究は、低資源環境における遠隔認知リハビリテーションの臨床報告を自動生成するため、知識工学に基づくテンプレート方式とゼロショットLLM（GPT-4）を比較した。両手法は同一の構造化変数を用い、8名の評価者による多面的評価の結果、臨床的信頼性と文章の流暢さ・簡潔さの間にトレードオフがあることが示された。



**新規性**
専門家の知見を反映したテンプレート生成とLLM生成を、参照報告のない低資源環境で同一入力条件のもと比較し、臨床報告システム設計のための8つの提言を導いた点が新規である。



**Tags:** clinical NLG, report generation, LLM comparison, remote rehabilitation, low-resource settings
...
- arXiv: [2605.06599](https://arxiv.org/abs/2605.06599)
- PDF: [link](https://arxiv.org/pdf/2605.06599.pdf)
- Published: 2026-05-08 02:22 JST
- Updated: 2026-05-08 02:22 JST
...
**日本語要約**
本論文は、Transformerのクロスエントロピー損失にL2正則化（weight decay）を加えた目的関数が、Villaniの可縮エネルギー関数の条件を満たすことを示し、その結果として対数ソボレフ不等式やポアンカレ不等式の定数を明示的に導出しています。さらに、これによりノイズ付きSGDの有限時間収束保証やPAC-Bayesian一般化境界との関係を理論的に与え、100M超パラメータ規模のモデルで診断量を用いた実験的検証も行っています。



**新規性**
Transformerの標準的なweight decay付き損失をVillaniの枠組みで厳密に特徴づけ、正則化強度とモデル次元に依存する収束・一般化の理論的保証を導出した点が新規です。



**Tags:** Transformer, weight decay, functional analysis, generalization, optimization
...
### 17. Why Global LLM Leaderboards Are Misleading: Small Portfolios for Heterogeneous Supervised ML
- arXiv: [2605.06656](https://arxiv.org/abs/2605.06656)
- PDF: [link](https://arxiv.org/pdf/2605.06656.pdf)
- Published: 2026-05-08 02:57 JST
- Updated: 2026-05-08 02:57 JST
...
**日本語要約**
本研究は、Arenaの約8.9万件・116言語・52モデルの比較データを分析し、グローバルなBradley-Terry順位付けが強い異質性のために誤解を招くことを示した。特に言語が重要な要因であり、言語や言語族ごとに分けると投票の一致度が大きく高まり、グローバル順位では見えない一貫した部分集団が存在することが分かった。



**新規性**
異質な利用者集団に対して、少数のモデル集合で一定の誤差以内に予測できるユーザー割合を最大化する(λ, ν)-ポートフォリオを提案し、集合被覆問題として定式化して保証を与えた点が新しい。



**Tags:** LLM leaderboard, Bradley-Terry, heterogeneous data, set cover, portfolio
...
### 18. SoftSAE: Dynamic Top-K Selection for Adaptive Sparse Autoencoders
- arXiv: [2605.06610](https://arxiv.org/abs/2605.06610)
- PDF: [link](https://arxiv.org/pdf/2605.06610.pdf)
- Published: 2026-05-08 02:28 JST
- Updated: 2026-05-08 02:28 JST
...
**日本語要約**
Sparse Autoencoders（SAE）はLLMやViTの内部表現を解釈するために用いられますが、従来のTopK SAEは全入力に対して固定の疎性Kを課していました。本論文は、入力ごとの複雑さに応じて活性化数を変えられるDynamic Top-K選択を備えたSoftSAEを提案し、よりデータ構造に合った表現を学習できることを示します。実験では、意味のある特徴を見つけるだけでなく、概念ごとに適切な特徴数を選択できることが確認されています。



**新規性**
微分可能なSoft Top-K演算により、入力依存の疎性レベルkを学習して固定Kの制約を解消した点が新規です。



**Tags:** sparse autoencoder, mechanistic interpretability, dynamic sparsity, top-k selection, LLM/ViT
...
### 19. When No Benchmark Exists: Validating Comparative LLM Safety Scoring Without Ground-Truth Labels
- arXiv: [2605.06652](https://arxiv.org/abs/2605.06652)
- PDF: [link](https://arxiv.org/pdf/2605.06652.pdf)
- Published: 2026-05-08 02:56 JST
- Updated: 2026-05-08 02:56 JST
...
**日本語要約**
本研究は、ラベル付きベンチマークが存在しない状況で言語モデルの安全性を比較評価するための「benchmarkless comparative safety scoring」を定式化し、シナリオパック、ルーブリック、監査者、判定器、サンプリング設定、再実行回数を固定したときのみスコアを有効とみなす枠組みを提案する。さらに、正解ラベルとの一致の代わりに、安全/劣化対照への反応、監査者・判定器由来のアーティファクトよりも対象差が支配的であること、再実行間での安定性という因果的検証連鎖を用いて評価する。



**新規性**
グラウンドトゥルースなしで比較的安全性スコアを妥当化するための検証連鎖を明示し、実運用での証拠として解釈可能な条件を定義した点が新規である。



**Tags:** LLM安全性, ベンチマークなし評価, 比較評価, 監査, 妥当性検証
...
### 20. NeuroAgent: LLM Agents for Multimodal Neuroimaging Analysis and Research
- arXiv: [2605.06584](https://arxiv.org/abs/2605.06584)
- PDF: [link](https://arxiv.org/pdf/2605.06584.pdf)
- Published: 2026-05-08 02:13 JST
- Updated: 2026-05-08 02:13 JST
...
**日本語要約**
NeuroAgentは、sMRI・fMRI・dMRI・PETなどの異種脳画像データに対して、前処理から下流の解析までを自然言語で支援するLLM駆動のエージェントフレームワークである。階層的なマルチエージェント構成とGenerate-Execute-Validate機構により、コード生成、実行時エラー回復、出力検証を自動化し、ADNIデータで有効性を評価している。



**新規性**
複数モダリティの神経画像解析に対し、前処理コードの自動生成・エラー回復・検証を組み込んだ階層型マルチエージェント枠組みを提案し、自然言語クエリによる下流解析も可能にした点が新規である。



**Tags:** neuroimaging, LLM agents, multimodal analysis, preprocessing automation, Alzheimer's disease
