...
Generated at: 2026-05-20 09:17 JST
Run ID: ba42e402-7ff9-4953-b276-42d01970593b
...
### 1. CrossView Suite: Harnessing Cross-view Spatial Intelligence of MLLMs with Dataset, Model and Benchmark
- arXiv: [2605.18621](https://arxiv.org/abs/2605.18621)
- PDF: [link](https://arxiv.org/pdf/2605.18621.pdf)
- Published: 2026-05-19 01:31 JST
- Updated: 2026-05-19 01:31 JST
...
**日本語要約**
本研究は、MLLMのクロスビュー空間推論を強化するために、データセット・ベンチマーク・モデルからなるCrossView Suiteを提案する。具体的には、17種類のタスクを含む160万件のCrossViewSet、シーン分離型の評価基盤CrossViewBench、そしてPerception→Alignment→Reasoningの3段階で推論するCrossViewerを構築した。



**新規性**
大規模なクロスビュー指示データ、体系的な評価ベンチマーク、明示的な物体レベル整合を同時に整備し、MLLMのクロスビュー空間知能を包括的に扱った点が新規である。



**Tags:** multimodal large language models, cross-view reasoning, spatial intelligence, dataset, benchmark
...
- PDF: [link](https://arxiv.org/pdf/2605.18740.pdf)
- Published: 2026-05-19 02:57 JST
- Updated: 2026-05-19 02:57 JST
...
**日本語要約**
MLLMは、画像全体では細部の手がかりを見落としてしまい、細粒度の視覚理解で性能が伸びにくいという課題がある。本論文は、同じモデルを用いて「切り出し画像を条件にした教師」と「全体画像を条件にした生徒」を作り、オンポリシーの自己蒸留によって教師の局所的な見え方を生徒へ منتقلするVision-OPDを提案する。外部教師やラベル、推論時ツールを使わずに、複数の細粒度ベンチマークで競合的またはそれ以上の性能を示した。



**新規性**
外部モデルや追加ラベルに頼らず、同一MLLM内のcrop-conditionedな局所知覚をfull-image policyへオンポリシー自己蒸留で移す枠組みを提案した点が新しい。



**Tags:** multimodal LLM, self-distillation, fine-grained visual understanding, on-policy learning, computer vision
...
- PDF: [link](https://arxiv.org/pdf/2605.18721.pdf)
- Published: 2026-05-19 02:50 JST
- Updated: 2026-05-19 02:50 JST
...
**日本語要約**
本論文は、オンラインRLの探索性と、選好最適化のオープンエンド生成能力の間にあるギャップを埋めるため、General Preference Model（GPM）の多次元な選好表現を政策更新まで一貫して用いるGeneral Preference Reinforcement Learning（GPRL）を提案する。GPRLは各次元ごとの優位度を別々に正規化して集約し、単一軸への偏りや報酬ハッキングを抑えるよう設計されている。Llama-3-8B-Instructを用いた実験では、AlpacaEval 2.0やArena-Hard、MT-Bench、WildBenchで既存手法を上回った。



**新規性**
スカラー報酬ではなく、k次元の構造化された選好表現をそのまま方策更新とドリフト監視に組み込み、単一軸の最適化を抑制する点が新しい。



**Tags:** preference learning, reinforcement learning, LLM alignment, reward hacking, open-ended generation
...
- PDF: [link](https://arxiv.org/pdf/2605.18732.pdf)
- Published: 2026-05-19 02:53 JST
- Updated: 2026-05-19 02:53 JST
...
**日本語要約**
本研究は、38モデルを対象に8,900件超の学術参考文献を自動検証し、LLMの事実想起性能がモデル規模と学習データ中のトピック頻度の両方に依存することを示した。想起品質は、これら2変数の対数線形結合に対してシグモイド状のスケーリングを示し、16の密なモデルでは分散の60%、同一ファミリ内では74〜94%を説明した。



**新規性**
事実想起に関するスケーリング則を、モデルサイズと訓練データ中のトピック頻度を同時に用いて定式化し、両者だけで性能変動の大部分を説明した点が新しい。



**Tags:** large language models, scaling law, factual recall, training data composition, cs.CL
...
- PDF: [link](https://arxiv.org/pdf/2605.18692.pdf)
- Published: 2026-05-19 02:28 JST
- Updated: 2026-05-19 02:28 JST
...
**日本語要約**
本論文は、運用最適化モデルの再最適化を、LLMが自然言語対話を通じて支援するエージェント型フレームワークを提案する。LLMはユーザ指示をモデル更新に変換し、最適化ツールボックスから適切な再最適化手法を選んで解を返すことで、実運用環境での迅速な再最適化を可能にする。



**新規性**
LLMを「OR専門家」として用い、構造化されたモデルパッチ更新と再最適化手法の選択を統合した点が新しい。



**Tags:** 最適化, 再最適化, 大規模問題, LLM, 意思決定支援
...
- PDF: [link](https://arxiv.org/pdf/2605.18747.pdf)
- Published: 2026-05-19 02:59 JST
- Updated: 2026-05-19 02:59 JST
...
**日本語要約**
本論文は、エージェント型LLMにおいてコードが単なる出力対象ではなく、推論・行動・環境モデル化・実行検証を支える「ハーネス」として機能するという視点を提案し、関連研究を体系的に整理したサーベイである。特に、ハーネスのインターフェース、計画・記憶・ツール利用などの機構、さらに単一エージェントからマルチエージェントへの拡張という3層で整理している。



**新規性**
コードをエージェント基盤として統一的に捉える「code as agent harness」という枠組みを提示し、実行可能で検証可能なエージェントシステムの設計指針をまとめた点。



**Tags:** LLM, agent systems, code generation, survey, multi-agent
...
### 7. DashAttention: Differentiable and Adaptive Sparse Hierarchical Attention
- arXiv: [2605.18753](https://arxiv.org/abs/2605.18753)
- PDF: [link](https://arxiv.org/pdf/2605.18753.pdf)
- Published: 2026-05-19 02:59 JST
- Updated: 2026-05-19 02:59 JST
...
**日本語要約**
DashAttentionは、従来の階層的注意機構が用いるtop-k選択の代わりに、適応的に疎なα-entmax変換でクエリごとに可変数のKVブロックを選択し、その結果を第2段のsoftmax注意の事前分布として用いる手法です。これにより階層全体が微分可能になり、長文脈モデリングに有利な非分散性を持つことが示されています。LLM実験では75%の疎性でもfull attentionに近い精度を達成し、NSAやInfLLMv2より良いPareto frontierを示しました。



**新規性**
top-kによる固定個数選択をやめ、α-entmaxで可変個数のブロック選択を行うことで、疎な段階と密な段階を完全に微分可能にした点が新規です。



**Tags:** hierarchical attention, sparse attention, long-context modeling, entmax, LLM
...
- PDF: [link](https://arxiv.org/pdf/2605.18738.pdf)
- Published: 2026-05-19 02:56 JST
- Updated: 2026-05-19 02:56 JST
...
**日本語要約**
本研究は、医療用大規模言語モデルが臨床倫理においてどのような価値を重視するかを、医師が検証した倫理的ジレンマのベンチマークと価値優先度を復元する手法で監査した。評価の結果、最先端モデル群は医師間の価値の多様性に近い範囲を示す一方、個々の判断は反復や表現の違いに対してほぼ決定的で、医師集団の分布的な多様性は再現しなかった。特に一部のモデルは患者の自律性を相対的に低く評価しており、価値配分を意識せずに単一モデルを展開すると臨床の多元性を損なう可能性がある。



**新規性**
医療AIの価値多元性を監査するためのベンチマークと、意思決定から価値優先度を復元する属性手法を提示した点が新規である。



**Tags:** 医療AI, 大規模言語モデル, 倫理, 価値多元性, 監査
...
- PDF: [link](https://arxiv.org/pdf/2605.18693.pdf)
- Published: 2026-05-19 02:28 JST
- Updated: 2026-05-19 02:28 JST
...
**日本語要約**
LLMエージェントにおけるスキル生成を独立した対象として評価するため、SkillGenBenchというベンチマークを提案している。生のコーパスから標準化されたスキル成果物を生成し、固定された実行環境と統一評価手順で正しさ・再利用性・実行可能性を検証する。タスク条件付き/非条件付き生成や、リポジトリ由来/文書由来の手続きソースを含む点も特徴である。



**新規性**
既存研究が主に与えられたスキルの利用や下流タスク性能を評価していたのに対し、スキル生成そのものを分離して評価する統一ベンチマークを構築した点。



**Tags:** LLM agents, benchmark, skill generation, evaluation protocol, reproducibility
...
### 10. Generative AI Advertising as a Problem of Trustworthy Commercial Intervention
- arXiv: [2605.18673](https://arxiv.org/abs/2605.18673)
- PDF: [link](https://arxiv.org/pdf/2605.18673.pdf)
- Published: 2026-05-19 02:15 JST
- Updated: 2026-05-19 02:15 JST
...
**日本語要約**
本論文は、生成AI広告を従来の「コンテンツ配置」ではなく、生成過程への介入がユーザに見えにくい形で商業的影響を与える「信頼できる介入」の問題として捉え直す。著者らは、商品言及、情報のフレーミング、行動の誘導、長期的な嗜好形成という影響の階層からなる分類を提案し、RAGやエージェント型パイプラインを含む複数のシステムでその現れ方を整理する。



**新規性**
生成AI広告を、可視的な広告挿入ではなく生成プロセスへの介入として再定義し、影響の強さに応じた階層的タクソノミーを提示した点が新規である。



**Tags:** generative AI, advertising, trustworthiness, user autonomy, LLM
...
### 11. Position: A Three-Layer Probabilistic Assume-Guarantee Architecture Is Structurally Required for Safe LLM Agent Deployment
- arXiv: [2605.18672](https://arxiv.org/abs/2605.18672)
- PDF: [link](https://arxiv.org/pdf/2605.18672.pdf)
- Published: 2026-05-19 02:13 JST
- Updated: 2026-05-19 02:13 JST
...
**日本語要約**
本論文は、LLMエージェントの安全性を単一の抽象層で保証するのは構造的に不十分であり、意味的意図・環境的妥当性・動的実行可能性の3次元をそれぞれ異なる段階で検証する必要があると主張する。さらに、各安全次元を独立に認証された層で扱い、次層の前提を確率的保証で支える契約ベースの3層アーキテクチャを提案し、その合成安全境界を確率の連鎖律で導く。



**新規性**
単一のガードレールではなく、3つの安全次元を別層で確率的に接続するassume-guarantee型アーキテクチャが、LLMエージェント安全の構造的要請であると位置づけた点が新規である。



**Tags:** LLMエージェント, 安全性, assume-guarantee, 確率的保証, アーキテクチャ
...
- PDF: [link](https://arxiv.org/pdf/2605.18703.pdf)
- Published: 2026-05-19 02:37 JST
- Updated: 2026-05-19 02:37 JST
...
**日本語要約**
EnvFactoryは、LLMのツール利用能力をAgentic RLで高めるために、実行可能で状態を持つツール環境を自動探索・検証し、自然な多ターン軌跡を合成する枠組みです。85個の検証済み環境から2,575件のSFT/RL軌跡を生成し、Qwen3系モデルで複数ベンチマークの性能向上を示しました。



**新規性**
実世界由来の資源から実行可能な環境を自動構築し、さらに人間の暗黙的意図を含む自然な多ターン軌跡まで一体的に自動生成する点が新規です。



**Tags:** Agentic RL, tool-use, environment synthesis, LLM training, reinforcement learning
...
### 13. GIM: Evaluating models via tasks that integrate multiple cognitive domains
- arXiv: [2605.18663](https://arxiv.org/abs/2605.18663)
- PDF: [link](https://arxiv.org/pdf/2605.18663.pdf)
- Published: 2026-05-19 02:09 JST
- Updated: 2026-05-19 02:09 JST
...
**日本語要約**
LLMベンチマークの飽和に対し、本論文は知識量ではなく複数の認知操作の統合の難しさに着目したベンチマークGIMを提案する。GIMは820問の原題からなり、制約充足、状態追跡、認識的注意、読者調整などを組み合わせた現実的だが専門知識に依存しない課題でモデルを評価する。さらに、公開・非公開分割と2PL IRTモデルにより、汚染検出と頑健な能力推定を可能にしている。



**新規性**
知識の暗記や抽象推論のみに寄らず、複数の認知ドメインを統合する課題でLLMを評価する新しいベンチマーク設計と、IRTによる頑健な能力推定を組み合わせた点が新規である。



**Tags:** LLM評価, ベンチマーク, 認知推論, IRT, 能力推定
...
- PDF: [link](https://arxiv.org/pdf/2605.18746.pdf)
- Published: 2026-05-19 02:59 JST
- Updated: 2026-05-19 02:59 JST
...
**日本語要約**
ESI-BENCHは、観測を受動的に処理するのではなく、行動を通じて必要な観測を獲得する「知覚-行動ループ」に基づく embodied spatial intelligence のベンチマークです。OmniGibson 上で10カテゴリ・29サブカテゴリの課題を構成し、MLLM の評価では受動的な多視点よりも能動的探索が有効である一方、行動選択の失敗が観測の質を下げて誤りを連鎖させることが示されました。



**新規性**
空間知能を「観測する主体」ではなく「行動する主体」として再定式化し、知覚・移動・操作を組み合わせた能動探索を評価する包括的ベンチマークを提示した点が新規です。



**Tags:** embodied AI, spatial intelligence, benchmark, multimodal LLM, active exploration
