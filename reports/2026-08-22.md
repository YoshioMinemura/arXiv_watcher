...
Generated at: 2026-08-22 08:34 JST
Run ID: c8e641f1-9843-4787-8554-9c2c3220aed7
...
### 1. Rule-Compliant Visual Spatial Planning for Multimodal Large Language Models
- arXiv: [2608.20237](https://arxiv.org/abs/2608.20237)
- PDF: [link](https://arxiv.org/pdf/2608.20237.pdf)
- Published: 2026-08-21 01:28 JST
- Updated: 2026-08-21 01:28 JST
...
**日本語要約**
本論文は、マルチモーダル大規模言語モデルが、明示的または未見のルール制約の下で視覚的な空間計画を行う能力を調べるためのベンチマークRuleMazeを提案する。RuleMazeでは、迷路ナビゲーションに自然言語ルールを課し、知覚・ルール解釈・制約付き行動計画を同時に評価する。



**新規性**
自然言語ルールを自動生成し、論理表現と実行可能な検証器へ変換するLanguage-Logic-Function Hybridizationと、知覚・実行・ルール検証を分離するDMPを提案した点が新規である。



**Tags:** multimodal large language models, visual spatial planning, rule compliance, benchmark, reasoning
...
- PDF: [link](https://arxiv.org/pdf/2608.20129.pdf)
- Published: 2026-08-20 23:56 JST
- Updated: 2026-08-20 23:56 JST
...
**日本語要約**
本研究は、自律走行における文脈依存の判断が必要な場面に対して、LLMの常識推論を活用しつつ、PPOベースの強化学習とPID制御をオーケストレータで統合するハイブリッド枠組みを提案する。LLMを直接車両制御に使うのではなく、推論を通じて制御系を補助し、動的な走行環境に合わせてRLの報酬関数を反復的に改善する。CARLAの高ランダム化シナリオで評価し、従来手法の構造化された制御と安全機構を保ちながらLLM推論を組み込める可能性を示した。



**新規性**
LLMの常識推論を直接制御ではなくオーケストレーションと報酬設計の改善に用い、強化学習とPID制御を統合した点が新規である。



**Tags:** autonomous driving, large language models, reinforcement learning, control, CARLA
...
- PDF: [link](https://arxiv.org/pdf/2608.20338.pdf)
- Published: 2026-08-21 02:59 JST
- Updated: 2026-08-21 02:59 JST
...
**日本語要約**
本論文は、LLMのunlearningを単なる事実の削除ではなく、危険な文脈での利用を除去しつつ有益な文脈での利用を保持する「概念」レベルの問題として捉え直しています。そのために、善悪両方に使える dual-use concepts に基づくベンチマーク ConceptGuard を提案し、既存手法が文脈分離や概念レベルの制御で十分でないことを示しています。



**新規性**
forget/retain を独立した事実ではなく相補的な概念使用として設計した、文脈感度・意図感度を評価できる unlearning ベンチマーク ConceptGuard を新たに構築した点です。



**Tags:** LLM, unlearning, benchmark, context-sensitivity, safety
...
### 4. FormalTCS: Benchmarking End-to-End Frontier Formal Theoretical Computer Science Research of Large Language Models
- arXiv: [2608.20153](https://arxiv.org/abs/2608.20153)
- PDF: [link](https://arxiv.org/pdf/2608.20153.pdf)
- Published: 2026-08-21 00:13 JST
- Updated: 2026-08-21 00:13 JST
...
**日本語要約**
本論文は、LLMによる理論計算機科学（TCS）研究を現実的な設定で評価するため、2025–2026年のSTOC/FOCS/SODA/COLT論文に基づく175件のベンチマークFormalTCSを提案する。各事例は定義・仮定・証明依存関係を保持し、Leanによる形式化と証明が専門家により検証されている。



**新規性**
自然言語の主張を形式定理へ変換するautoformalizationを含む、端から端までのTCS研究能力を評価する専門家検証済みベンチマークを構築した点が新規である。



**Tags:** large language models, theoretical computer science, benchmark, formalization, Lean
...
- PDF: [link](https://arxiv.org/pdf/2608.20116.pdf)
- Published: 2026-08-20 23:48 JST
- Updated: 2026-08-20 23:48 JST
...
**日本語要約**
本研究は、テキスト要約・数値時系列・外部ツール出力が互いに矛盾する状況で、LLMがどの証拠を採用するかを調べた。潜在的なリスク軌道から生成した合成ベンチマークを用い、モダリティ、時間的近接性、信頼性、証拠の由来を独立に操作して評価している。



**新規性**
テキストと数値が対立する状況を制御可能な合成ベンチマークとして構築し、LLMの証拠仲裁がランダムではなく、モダリティ嗜好や時間的近接性への依存などのヒューリスティックに基づくことを示した点。



**Tags:** large language models, evidence arbitration, benchmark, multimodal reasoning, tool-augmented decision systems
...
- PDF: [link](https://arxiv.org/pdf/2608.20202.pdf)
- Published: 2026-08-21 01:00 JST
- Updated: 2026-08-21 01:00 JST
...
**日本語要約**
大規模言語モデルのメモリ利用において、正しく保存・検索された記憶であっても、現在のタスクに対する推論や信念を歪めて性能を下げる「認知トラップ」があることを指摘し、その評価のためのベンチマークMemTrapBenchを提案している。実験では、2つのモデル परिवारと5種類の代表的なメモリ枠組みで、評価した全てのメモリ戦略がメモリなし設定を下回り、強い手法でも10%以上性能が低下した。



**新規性**
記憶の抽出・保存・検索ではなく、検索された記憶が推論に与える悪影響に着目し、Reasoning FixationとBelief Distortionの2種類の認知トラップを体系的に評価するMemTrapBenchを導入した点が新規である。



**Tags:** LLM memory, benchmark, reasoning, cognitive traps, inference-time method
...
### 7. OenoBench: A Wine-Domain Benchmark for Knowledge-Grounded Evaluation of Large Language Models
- arXiv: [2608.20106](https://arxiv.org/abs/2608.20106)
- PDF: [link](https://arxiv.org/pdf/2608.20106.pdf)
- Published: 2026-08-20 23:37 JST
- Updated: 2026-08-20 23:37 JST
...
**日本語要約**
OenoBenchは、ワイン分野に関する3,266問の多肢選択式ベンチマークで、地域、品種、栽培、醸造、生産者、ビジネスの6領域と4段階の難易度を含みます。政府登録、査読論文、Wikipedia/Wikidataなどに基づく38,104件の事実から構築され、各設問はURLに紐づく根拠付きで生成・監査されています。



**新規性**
LLMを真実の स्रोतではなく整形・監査役として用いる構築パイプラインと、9エージェント監査による大規模な知識グラウンド評価手法を提示した点が新規です。



**Tags:** benchmark, large language models, wine domain, knowledge-grounded evaluation, question answering
...
- PDF: [link](https://arxiv.org/pdf/2608.20274.pdf)
- Published: 2026-08-21 02:12 JST
- Updated: 2026-08-21 02:12 JST
...
**日本語要約**
LLMエージェントが過去タスクから獲得したスキルを別タスクへ転移する際の有効性を、スキルの誘導方法の違いに着目して体系的に比較した研究です。タスク単位のスキルはベースラインを下回ることが多い一方、サブタスク単位のスキルは平均的に性能を向上させ、テキスト形式のスキルはコード形式より転移しやすいことを示しています。



**新規性**
スキルの具体性と抽象性を組み合わせた「skill utility score」を提案し、実行前にスキルメモリの有用性を診断できる指標として、タスク成功との一貫した相関を示した点が新規です。



**Tags:** LLM agents, skill transfer, memory, task decomposition, evaluation
...
### 9. Inject, Align, Recover: Staged Post-Training for Retrieval-Free Document Knowledge Internalization
- arXiv: [2608.20281](https://arxiv.org/abs/2608.20281)
- PDF: [link](https://arxiv.org/pdf/2608.20281.pdf)
- Published: 2026-08-21 02:14 JST
- Updated: 2026-08-21 02:14 JST
...
**日本語要約**
本研究は、検索を行わずに固定文書集合の知識をLLMに内在化させる設定を扱い、文書知識の注入・QAへの適応・一般能力の回復を分離した3段階の事後学習フレームワークIARを提案する。Common CorpusとCCI、複数のモデル परिवारにおいて、IARはretrieval-freeな文書QA性能と一般性能の両立を改善し、Vanilla SFTより多くの設定で優位だった。



**新規性**
文書を continuation・rewrite・instruction-conditioned reconstruction に変換して注入し、その後にQA適応と一般能力回復を段階的に分けて行う点が新しい。



**Tags:** document knowledge internalization, retrieval-free QA, post-training, large language models, instruction tuning
...
- PDF: [link](https://arxiv.org/pdf/2608.20314.pdf)
- Published: 2026-08-21 02:53 JST
- Updated: 2026-08-21 02:53 JST
...
**日本語要約**
MidToolは、一般的なツール利用能力に向けたmid-training用のオープンなデータ構築パイプラインであり、Web・PDF・コードデータに加えて、実世界のツールAPIやMCPスキル、文書に基づくワークフローから合成した教師データを組み合わせています。Qwen3-4B-BaseとQwen3-8B-BaseをMidTool-Mixでmid-trainし、その後SFTとRLで追加学習すると、BFCL、tau2-Bench、MCP Universeでベースラインより一貫して性能が向上しました。



**新規性**
一般的なツール利用を、post-trainingだけでなく専用のmid-trainingで強化するためのデータ合成パイプラインMidToolを提案し、その有効性を示した点です。



**Tags:** mid-training, tool use, data synthesis, large language models, agentic AI
...
- PDF: [link](https://arxiv.org/pdf/2608.20318.pdf)
- Published: 2026-08-21 02:56 JST
- Updated: 2026-08-21 02:56 JST
...
**日本語要約**
Recursive self-improvement（RSI）の可否に関わる「学習アルゴリズムを設計できるか」という能力を測るため、10個の研究リポジトリを用いたベンチマークAI4AI-Benchを提案している。各タスクではエージェントが学習アルゴリズムを書き換え、そのコードを再実行して固定評価器で採点する。29設定・6システムの評価では平均スコアは0.166、最良でも0.250で、既存アルゴリズムから最適解への距離の2割未満しか埋められなかった。



**新規性**
データ収集やハイパーパラメータ調整ではなく、訓練アルゴリズムそのものの改変能力に焦点を当てたRSI向けベンチマークを初めて体系的に提示した点。



**Tags:** benchmark, recursive self-improvement, LLM agents, training algorithms, algorithmic design
...
### 12. An Agentic Approach for Active Data Collection, Travel Behavior Modeling, and Weather-Sensitive Demand Prediction
- arXiv: [2608.20320](https://arxiv.org/abs/2608.20320)
- PDF: [link](https://arxiv.org/pdf/2608.20320.pdf)
- Published: 2026-08-21 02:57 JST
- Updated: 2026-08-21 02:57 JST
...
**日本語要約**
本研究は、会話型データ収集、構造化データ処理、行動予測を統合する3エージェントのワークフローを提案し、学生通勤者を対象に天候シナリオ付きの画像補助型選好調査を実施した。天候と交通手段選択の関係を多項ロジットモデルで分析し、機械学習およびLLMによる予測性能も比較した結果、最良の視覚入力付きLLMは5クラス分類で71.5%の精度を示した。



**新規性**
対話型調査から行動モデリング、天候感応型需要予測までを、監査可能なマルチエージェント・ワークフローとして一体化し、テキスト・画像・ペルソナ・few-shot条件を含むLLM予測を体系的に評価した点が新しい。



**Tags:** travel behavior, multi-agent workflow, large language models, weather-sensitive prediction, survey modeling
...
### 13. Learning When to Think: Adaptive Reasoning for Test-Time Compute Allocation
- arXiv: [2608.20256](https://arxiv.org/abs/2608.20256)
- PDF: [link](https://arxiv.org/pdf/2608.20256.pdf)
- Published: 2026-08-21 01:54 JST
- Updated: 2026-08-21 01:54 JST
...
**日本語要約**
本研究は、推論言語モデルが固定的なトークン予算ではなく、問題ごとに推論量を自ら選べるかを調べた。GRPOの学習中に最初のトークンとして 「NoThink」「Short」「Long」 の3モードを選ばせることで、難易度に応じた計算配分を学習させ、MATH500で精度を大きく落とさず平均応答長を41%削減した。さらに、この方策は再学習なしで他ベンチマークにも移り、特に易しい問題で大きなトークン削減を示した。



**新規性**
別個のルータを用いず、GRPO内で3つの推論モードを直接学習させることで、問題難易度に応じた適応的なテスト時計算配分を実現した点が新しい。



**Tags:** reasoning LLM, test-time compute, reinforcement learning, adaptive inference, token efficiency
...
- PDF: [link](https://arxiv.org/pdf/2608.20161.pdf)
- Published: 2026-08-21 00:16 JST
- Updated: 2026-08-21 00:16 JST
...
**日本語要約**
本論文は、指示ベース画像編集における planner-renderer の2段階パイプラインに対し、最終画像報酬だけではどちらのモジュールを重点的に改善すべきか分かりにくいという問題を扱う。DARSは、モジュール間・モジュール内の両方で報酬変動を用いる強化学習枠組みを提案し、planner には構造化された推論出力を導入して局所的な学習信号を与える。



**新規性**
planner と renderer の両方に対して二重レベルの credit assignment を行い、さらに planner 内では構造化推論を用いて outcome-level の報酬を token-level に局所化した点が新規である。



**Tags:** instruction-based image editing, reinforcement learning, credit assignment, structured reasoning, vision-language model
...
- PDF: [link](https://arxiv.org/pdf/2608.20280.pdf)
- Published: 2026-08-21 02:14 JST
- Updated: 2026-08-21 02:14 JST
...
**日本語要約**
本研究は、LLMのセマンティックキャッシュにおける複数のエビクションポリシーを、ワークロード・容量・エンコーダを揃えた同一プロトコルで比較した。18条件のいずれでもLFUを上回るポリシーはほぼ見られず、FIFOやstreaming SISOは特に小容量で大きく劣った。一方で、実運用上の閾値ではヒットの多くが回答置換可能ではなく、単純なヒット率だけでは有効性を過大評価しうることも示した。



**新規性**
CLEVERを用いて、複数のエビクションポリシーを横断比較し、LFUが最も強い単純な既定値であることと、幾何学的に賢い削除戦略の上積みがほとんど出ない理由を条件付きpackingの観点から説明した。



**Tags:** LLM cache, eviction policy, semantic cache, LFU, benchmark
...
- PDF: [link](https://arxiv.org/pdf/2608.20204.pdf)
- Published: 2026-08-21 01:01 JST
- Updated: 2026-08-21 01:01 JST
...
**日本語要約**
契約書の最終レビューである「contract scrubbing」は、長文の整合性確認や定義語の誤用検出などを含むため、LLMの適用が期待される一方、これまで正式な評価ベンチマークがありませんでした。本論文は、経験豊富な弁護士が作成した多様なエラーカテゴリを含むベンチマーク「ContractScrub」を提案し、最先端モデルでも性能が低いことを示しています。



**新規性**
契約書の最終レビュー能力を評価する初のベンチマークContractScrubを構築し、実務に近い法務タスクに対するLLMの限界を明らかにした点が新規です。



**Tags:** legal NLP, benchmark, LLM evaluation, contract analysis
...
### 17. Reward-Guided Autoregressive Graph Generation for Efficient Multi-Agent Communication Topology Design
- arXiv: [2608.20099](https://arxiv.org/abs/2608.20099)
- PDF: [link](https://arxiv.org/pdf/2608.20099.pdf)
- Published: 2026-08-20 23:32 JST
- Updated: 2026-08-20 23:32 JST
...
**日本語要約**
LLMベースのマルチエージェントシステムでは高い推論性能が得られる一方、トークン消費が大きいという課題がある。本研究は、マルチエージェント通信トポロジー設計を自己回帰的グラフ生成として扱い、報酬モデルで正解性と構造のコンパクトさを同時に評価して学習する手法を提案する。



**新規性**
タスク正解性と構造の簡潔さを組み合わせた報酬モデルを用いて、自己回帰グラフ生成器を微調整し、精度を保ちながら疎で効率的なトポロジー生成を促した点。



**Tags:** multi-agent systems, graph generation, reinforcement learning, communication topology, token efficiency
...
- PDF: [link](https://arxiv.org/pdf/2608.20186.pdf)
- Published: 2026-08-21 00:41 JST
- Updated: 2026-08-21 00:41 JST
...
**日本語要約**
非侵襲EEGを用いて、黙読中の脳活動から語彙レベルの情報を復号できるかを検討した研究である。約24万語・49時間分の単一被験者データに対し、EEGエンコーダと大規模言語モデル埋め込みを対比学習で整列させ、単語群ごとのtop-10 retrievalが偶然水準を有意に上回ることを示した。さらに、復号性能は訓練データ量に対して対数線形に向上し、飽和は見られなかった。



**新規性**
黙読を、内言の代替となるスケーラブルなプロキシ課題として位置づけ、オープンボキャブラリの単語レベル情報を非侵襲EEGから復号できることを示した点が新しい。



**Tags:** EEG, silent reading, contrastive learning, language model, brain decoding
...
### 19. Task-CoEvolve: Efficient Harness Optimization via Adaptive Validation Task Selection
- arXiv: [2608.20169](https://arxiv.org/abs/2608.20169)
- PDF: [link](https://arxiv.org/pdf/2608.20169.pdf)
- Published: 2026-08-21 00:24 JST
- Updated: 2026-08-21 00:24 JST
...
**日本語要約**
本論文は、LLMエージェントのハーネス最適化において、各反復で固定の検証集合全体を評価する代わりに、適応的に検証タスクを選ぶことで評価コストを削減する手法を提案する。Task-CoEvolveは、候補ハーネス間で差が出やすいタスクを重視して部分評価を行い、そのサンプリング確率を考慮して全体性能を推定することで、反復間の比較可能性を保つ。



**新規性**
検証タスクをハーネスとともに共進化させ、分散重み付きサンプリングと確率補正付きの全体性能推定を組み合わせて、少ない評価回数で効率的な最適化を実現した点が新しい。



**Tags:** LLMエージェント, ハーネス最適化, 適応的評価, タスク選択, 効率化
