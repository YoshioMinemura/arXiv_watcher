...
Generated at: 2026-08-14 08:53 JST
Run ID: 4ef25c8e-8b7a-48c9-8c03-c83363a551c2
...
- PDF: [link](https://arxiv.org/pdf/2608.12304.pdf)
- Published: 2026-08-13 02:50 JST
- Updated: 2026-08-13 02:50 JST
...
**日本語要約**
本研究は、システム記述からDynamic Master Logic（DML）モデルを自動構築し、Knowledge Graphとして表現するKG-DMLフレームワークを提案する。Retrieval-Augmented GenerationとLarge Language Modelsを用いて、機能的依存関係と論理関係を保ちながら階層的にモデルを生成し、診断推論や安全性評価、故障伝播解析に利用できることを示した。



**新規性**
大規模で複雑なシステムに対して、RAGとLLMを用いたDMLの自動構築と多段階検証を可能にし、技術文書を実行可能な機能モデルへ変換する枠組みを拡張した点が新規である。



**Tags:** Knowledge Graph, Large Language Models, Retrieval-Augmented Generation, System Diagnostics, Reliability Analysis
...
### 2. A corpus-specific clinical RAG system matches or outperforms newer frontier LLMs on HealthBench
- arXiv: [2608.12138](https://arxiv.org/abs/2608.12138)
- PDF: [link](https://arxiv.org/pdf/2608.12138.pdf)
- Published: 2026-08-12 23:55 JST
- Updated: 2026-08-12 23:55 JST
...
**日本語要約**
インドやLMIC向けに設計された臨床RAGシステムVITAを、HealthBench上で最新の汎用LLMと比較評価した。VITAは4,023問の評価でGPT-5.4などを上回って首位となり、500問の再評価でもGPT-5.5と同等水準の成績を示した一方、コミュニケーション面のスコアは低かった。



**新規性**
疾患別ガイドラインやインド特有の抗菌薬耐性データなど、コーパス特異的な知識を用いる臨床RAGが、公開ベンチマークでフロンティアLLMに匹敵しうることを示した点。



**Tags:** clinical RAG, LLM evaluation, HealthBench, retrieval-augmented generation, LMIC
...
- PDF: [link](https://arxiv.org/pdf/2608.12283.pdf)
- Published: 2026-08-13 02:28 JST
- Updated: 2026-08-13 02:28 JST
...
**日本語要約**
本研究は、金融ニュースの感情情報に加えて、マクロ経済指標やテクニカルシグナルを用い、小型株（Russell 2000）向けの売買戦略を評価した。特に、予測されたリスクをアレアトリック／エピステミックに分解してポートフォリオの共分散行列へ直接組み込む不確実性対応の手法を検討し、純粋なalpha・betaトリガーとその組み合わせを比較した。結果として、純alphaと純betaの分離した戦略が、しばしばbetaの交差条件よりもSharpe比とリターンで優れていた。



**新規性**
予測リスクを分解して共分散行列に直接反映する点と、alpha/betaのトリガーを分離して評価する設計が新規である。



**Tags:** large language model, financial sentiment, portfolio optimization, small-cap stocks, risk modeling
...
### 4. NetlistBench: Evaluating LLM Reliability in SPICE Netlist Recognition and Manipulation
- arXiv: [2608.12197](https://arxiv.org/abs/2608.12197)
- PDF: [link](https://arxiv.org/pdf/2608.12197.pdf)
- Published: 2026-08-13 00:51 JST
- Updated: 2026-08-13 00:51 JST
...
**日本語要約**
本論文は、LLMの回路設計ワークフローにおけるSPICE netlistの認識・編集能力を評価するための、構造検証付きベンチマークNetlistBenchを提案する。NetlistBenchは2,342件・24タスク परिवारを含み、構造を考慮した決定的オラクルで評価することで、単純な局所編集は高精度でも、デバイス追加や等価性判定、長い編集連鎖では性能が大きく低下することを示した。



**新規性**
netlistを高レベル設計推論から切り分け、構造保存性に着目したSPICE netlist専用のベンチマークを構築し、LLMの信頼性上のボトルネックを定量化した点が新しい。



**Tags:** LLM, SPICE netlist, benchmark, circuit design, reliability
...
- PDF: [link](https://arxiv.org/pdf/2608.12150.pdf)
- Published: 2026-08-13 00:11 JST
- Updated: 2026-08-13 00:11 JST
...
**日本語要約**
本研究は、LLM評価において推論時の生成トークン予算を64〜4,096の7段階で変化させると、モデル順位が安定せずベンチマーク間でも逆転することを、4モデル・3つの推論ベンチマーク・56,476回の推論で示した。さらに、一部の問題では予算を増やしても精度が単調に改善せず、モデル間の補完性や予算を考慮したルーティングの有効性も確認し、評価プロトコルに予算条件を組み込む必要性を主張している。



**新規性**
生成トークン予算を評価条件として系統的に変化させ、LLMの順位が予算依存で逆転しうることと、予算条件付き評価の必要性を実証した点が新規である。



**Tags:** LLM評価, 推論予算, モデル順位, ベンチマーク, ルーティング
...
- PDF: [link](https://arxiv.org/pdf/2608.12149.pdf)
- Published: 2026-08-13 00:11 JST
- Updated: 2026-08-13 00:11 JST
...
**日本語要約**
本研究は、層間に全注意を挟むハイブリッド線形注意LLMにおけるMassive activations（MA）を体系的に調べ、全注意層の直前に現れるpre-attention spikes（PAS）と、線形注意層をまたいで持続するinter-spike plateaus（ISP）という2つの形態を発見した。これらの構造は5種類の線形注意アーキテクチャ、6種類のハイブリッド化設定、5つのデータ領域、1.2B〜397B規模のモデルで再現され、全注意が密になるほどPASとISPが連結して全注意LLMに近い安定した形態へ移行することが示された。



**新規性**
ハイブリッド線形注意LLMにおけるMAの形態を、PASとISPという2つのアーキテクチャ整合的パターンとして初めて体系的に記述し、その再現性と生成・消失の機構を示した点が新規である。



**Tags:** large language models, linear attention, hybrid architectures, massive activations, mechanistic analysis
...
### 7. Do LLMs Take Care of Their Own? Similarity Signals Can Induce Cooperation
- arXiv: [2608.12125](https://arxiv.org/abs/2608.12125)
- PDF: [link](https://arxiv.org/pdf/2608.12125.pdf)
- Published: 2026-08-12 23:47 JST
- Updated: 2026-08-12 23:47 JST
...
**日本語要約**
本論文は、LLMエージェント同士の戦略的相互作用において、段階的な「類似性シグナル」を与えたときに協力行動がどう変化するかを評価する初の枠組みを提案する。実験の結果、モデルによって類似性シグナルへの反応は大きく異なる一方で、いくつかの最新モデルでは協力問題や利得構造、プロンプト表現をまたいで一貫した挙動が見られた。



**新規性**
類似性シグナルがLLM間の協力を誘発しうることを、実験枠組みと行動ゲーム理論モデルの両面から示した点が新しい。



**Tags:** LLM, 協力ゲーム, 戦略的相互作用, 類似性シグナル, ゲーム理論
...
- PDF: [link](https://arxiv.org/pdf/2608.12253.pdf)
- Published: 2026-08-13 01:55 JST
- Updated: 2026-08-13 01:55 JST
...
**日本語要約**
マルチエージェントRLで単一の大規模言語モデルをユーザシミュレータとして使うと、シミュレータのモード崩壊により方策がその偏った挙動に過適合し、未知のシミュレータや実ユーザへの汎化が悪化することを示している。これに対し、推論時のVerbalized Samplingと学習時のCo-Trainingという2つの手法を提案し、複数ベンチマークと人手評価で性能改善を確認した。



**新規性**
シミュレータ側の多様性不足を「simulator collapse」として理論化し、推論時・学習時の両面から対処する枠組みを提案した点が新しい。



**Tags:** multi-agent RL, LLM simulator, generalization, mode collapse, human-AI interaction
...
- PDF: [link](https://arxiv.org/pdf/2608.12282.pdf)
- Published: 2026-08-13 02:27 JST
- Updated: 2026-08-13 02:27 JST
...
**日本語要約**
VAKRAは、企業環境でのエージェントが構造化APIと文書コレクションをまたいで推論する能力を評価するためのベンチマークで、8,000以上の実行可能なAPIを62分野にわたって収録しています。タスクは単一APIの利用から多段推論、さらに自然言語のツール利用ポリシー制約を含むマルチソース推論まで段階的に難しく設計されており、正解は実際のAPI再実行で検証されます。



**新規性**
API呼び出しと検索を別々ではなく、ツール利用ポリシー制約を含む多段・多 स्रोत推論として統合評価し、実行可能なAPI再実行で正解を検証する点が新しいです。



**Tags:** benchmark, multi-hop reasoning, API, retrieval, tool use
...
### 10. Information Abundance Paradox: Long-Context Training Undermines Parametric Knowledge
- arXiv: [2608.12218](https://arxiv.org/abs/2608.12218)
- PDF: [link](https://arxiv.org/pdf/2608.12218.pdf)
- Published: 2026-08-13 01:13 JST
- Updated: 2026-08-13 01:13 JST
...
**日本語要約**
本論文は、長い文脈を用いた学習が常にモデル性能を向上させるわけではなく、むしろ十分に関連情報が与えられると、モデルがその情報をパラメトリックに内部化する必要性が下がり、文脈依存が強まることを示す。事前学習では文脈長の増加により性能が中間点まで改善した後に低下し、SFTでは訓練時文脈が豊富だと文脈ありでは良くなる一方、テスト時に文脈がない・誤っている場合の頑健性が下がる。



**新規性**
長文脈が情報を豊富に与えるほど、モデルが知識を内部化せず文脈に頼るようになるという「Information Abundance Paradox」を提案し、その性能低下とメカニズムを実証的に示した。



**Tags:** 長文脈学習, 大規模言語モデル, パラメトリック知識, 文脈依存, 頑健性
...
- PDF: [link](https://arxiv.org/pdf/2608.12290.pdf)
- Published: 2026-08-13 02:35 JST
- Updated: 2026-08-13 02:35 JST
...
**日本語要約**
本研究は、ブラックボックスなImage-to-Videoモデルにおける試行錯誤的な生成を、目標指向の閉ループ最適化として扱う「Agentic Self-Improvement」フレームワークを提案する。具体的には、mLLMによるプロンプト反復最適化と、Bayesian optimizationによるseed・CFG scaleの共同最適化を組み合わせ、生成結果の整合性と品質向上を図る。



**新規性**
DSGクエリとCMQによる自動評価を用いてプロンプトを改善し、さらにVTAスコアを導入して動画生成の最適化を誘導する二段階手法を提案した点が新しい。



**Tags:** Image-to-Video, prompt optimization, Bayesian optimization, multimodal LLM, video generation
...
### 12. Convergent Detour Hijacking: Task-Preserving Resource Amplification in Skill-Based LLM Agents
- arXiv: [2608.12273](https://arxiv.org/abs/2608.12273)
- PDF: [link](https://arxiv.org/pdf/2608.12273.pdf)
- Published: 2026-08-13 02:12 JST
- Updated: 2026-08-13 02:12 JST
...
**日本語要約**
LLMエージェントが第三者スキルを選択・計画に利用する際、説明文と本文を組み合わせて、正しいタスクを不必要に高コストな経路へ誘導する攻撃「Convergent Detour Hijacking (CDH)」を提案している。複数のLLMバックエンドと491件のタスクで評価し、タスク完了率を大きく損なわずにトークン消費と実行時間を増加させられることを示した。



**新規性**
選択段階の誘導と計画段階の誘導を一体化し、正しい完了を保ちながら経路とコストを悪化させるエンドツーエンドの攻撃としてCDHを定式化した点。



**Tags:** LLM agents, security, prompt injection, resource amplification, tool use
...
- PDF: [link](https://arxiv.org/pdf/2608.12262.pdf)
- Published: 2026-08-13 02:04 JST
- Updated: 2026-08-13 02:04 JST
...
**日本語要約**
Diagram-MMUは、科学図の解析と理解におけるMLLMの能力を評価するために作られたマルチモーダルベンチマークで、3.7kの図と18.3kの人手検証済み質問を含みます。6分野にわたり、diagram-to-code parsing、diagram-to-code editing、diagram question answeringの3課題と、それぞれのagentic設定を評価します。12のMLLMを評価した結果、図に対する推論は比較的できても、図からコードへの解析・編集は難しいことが示されました。



**新規性**
科学図の理解を、図からコード生成・編集・QAまで含む3課題とagentic設定で体系的に評価するベンチマークを新たに構築した点です。



**Tags:** multimodal large language models, scientific diagrams, benchmark, diagram-to-code, question answering
...
- PDF: [link](https://arxiv.org/pdf/2608.12220.pdf)
- Published: 2026-08-13 01:14 JST
- Updated: 2026-08-13 01:14 JST
...
**日本語要約**
既存のVLMにおける空間推論の弱点に対し、本研究は3D環境認識を明示的に組み込んだ構造化Chain-of-Thoughtと、途中推論段階への信用割当を改善する多目的プロセス報酬付きRLを提案する。さらに、専用パイプラインで合成したSCOUT-24kデータセットを用いて学習し、空間ベンチマークや複雑な空間推論タスクで性能向上を示した。単一画像のみで学習したにもかかわらず、マルチ画像や動画への外部領域汎化も確認された。



**新規性**
3D空間認識を明示した構造化CoTと、多目的プロセス報酬・改良advantage推定を組み合わせて、空間推論における中間ステップの学習を細かく最適化した点。



**Tags:** Vision-Language Model, spatial reasoning, chain-of-thought, reinforcement learning, 3D perception
...
### 15. Ready Cohorts: Bounding GPU Opportunity and Avoiding Host Round Trips in LLM-Agent Control
- arXiv: [2608.12123](https://arxiv.org/abs/2608.12123)
- PDF: [link](https://arxiv.org/pdf/2608.12123.pdf)
- Published: 2026-08-12 23:42 JST
- Updated: 2026-08-12 23:42 JST
...
**日本語要約**
LLMエージェントの制御経路において、GPU実行に十分な並行ワークがどの条件で得られるかを定式化し、固定窓境界で失われる機会を正確なパッキングでどれだけ回収できるかを評価している。さらに、GPUで計算した分岐結果をホストに戻さずデバイス上に保持する方式を調べ、複数のGPU配置で一貫して高速化することを示している。



**新規性**
ready-cohortの境界を用いてGPU実行可能な制御ワークの供給量を定量化し、加えて観測結果をデバイス上に留めることでホスト往復を避ける設計の有効性を実証した点が新しい。



**Tags:** LLMエージェント, GPU実行, スケジューリング, ホスト往復削減, 並列処理
...
### 16. SAG: SQL-Retrieval Augmented Generation with Query-Time Dynamic Hyperedges
- arXiv: [2608.12129](https://arxiv.org/abs/2608.12129)
- PDF: [link](https://arxiv.org/pdf/2608.12129.pdf)
- Published: 2026-08-12 23:49 JST
- Updated: 2026-08-12 23:49 JST
...
**日本語要約**
SAGは、グローバルな知識グラフを構築せずに、文書をイベントとエンティティの組として整理する構造化RAGです。クエリ時には共有エンティティを結合キーとして関連チャンクを動的に接続し、元のチャンクを証拠として保持したまま多段推論に対応します。HotpotQA、2WikiMultiHopQA、MuSiQueで検索性能とQA性能の両方で最良の結果を示しました。



**新規性**
チャンクを「イベント＋エンティティ」の潜在的ハイパーエッジとして表現し、クエリ時に動的な近傍を生成する点が新規です。



**Tags:** RAG, 情報検索, 多段推論, 構造化検索, LLM
