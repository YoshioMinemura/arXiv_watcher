...
Generated at: 2026-05-23 09:15 JST
Run ID: 0a92fc36-17f5-4acf-b88b-c98ed8da4f08
...
**日本語要約**
本研究は、LLMの事前学習におけるデータ順序が、時間に依存する事実知識の獲得に与える影響を調べた。7,000件超の時系列に基づくベンチマークを用い、Common Crawlの時系列順学習とシャッフル学習を比較した結果、時系列順モデルは一般的な言語理解では同等でありつつ、より新しく時間的に正確な知識を示した。



**新規性**
7,000件超の時系列ベンチマークと評価手法を新たに構築し、データ順序の違いが知識の新鮮さと時間的精度に与える影響を実証的に示した。



**Tags:** large language models, pre-training, temporal knowledge, data ordering, benchmark
...
**日本語要約**
大規模言語モデルを自動評価器として用いる際、直前までの会話履歴の極性が後続の判断を偏らせる現象（AMEL）を検証した。11モデル・75,898回のAPI呼び出しにより、モデルは履歴の支配的な極性に引き寄せられ、特にベースラインで不確実な項目ほど影響が大きく、負の履歴の方が正の履歴より強い偏りを生むことが示された。



**新規性**
会話履歴の極性がLLMの判断を系統的に歪めることを大規模実験で定量化し、効果が文脈長ではなく履歴の偏りに依存し、負の非対称性を持つことを示した点。



**Tags:** LLM評価, 会話履歴, 判断バイアス, 自動評価, 負の非対称性
...
**日本語要約**
本研究は、政治演説におけるPathos分析に対して、音響感情認識モデルが代理指標として使えるかを、LLMベースのマルチモーダル分析と比較して検証した。Bundestag演説の事例では、GeminiのValenceはTRUST-Pathosと強い相関を示した一方、音響モデルemotion2vecのValenceは相関しなかった。さらに、EMO-DBの評価を通じて、従来のSERベンチマークは演技音声や文化的バイアス、カテゴリ不一致の問題を抱えることを示した。



**新規性**
LLMが音声と書き起こしを統合して政治的感情の意味的側面を捉え、従来の音響モデルよりPathos推定に適していることを実証的に示した点が新規である。



**Tags:** 政治演説分析, マルチモーダルLLM, 感情認識, Pathos, 音声感情認識
...
**日本語要約**
ChronoMedKGは、疾患関連知識を静的な事実として扱う従来の生物医学KGの限界に対し、発症時期や進行段階などの時間情報を付与した時系列対応の知識グラフです。PubMed/PMCから抽出した13M件の候補から、PMID追跡可能な証拠と信頼度評価、オントロジー整合性を通じて460,497件の三つ組を構築し、13,431疾患をカバーします。さらに、時系列推論を評価するChronoTQAベンチマークを提案し、静的質問よりも時系列質問でLLM性能が大きく低下することを示しています。



**新規性**
疾患知識に「いつ臨床的に重要になるか」という時間軸を明示的に組み込んだ、証拠リンク付きの時系列生物医学KGと、その有効性を測るベンチマークを同時に提示した点が新規です。
...
**日本語要約**
大規模言語モデルの不確実性を補う方法として、Lean などの形式証明をLLMに生成させるアプローチを用い、未解決問題への解決能力を大規模に評価した。最も高性能なエージェントは Erdős の未解決問題353件のうち9件、OEIS の予想492件のうち44件を証明し、いくつかの数学研究分野で実運用されている。



**新規性**
未解決の数学問題に対するAI駆動の形式証明探索を初めて大規模に評価し、どのようなエージェント設計が有効かを示した。
...
**日本語要約**
LLMベースのマルチエージェントシステムにおけるKVキャッシュ共有は効率的ですが、入力や推論過程などの機微情報が他エージェントへ漏れる可能性があります。LCGuardは、共有前にKVキャッシュへ表現レベルの変換を学習的に適用し、タスクに必要な意味を保ちながら再構成可能な敏感情報を減らす枠組みです。複数のモデルとベンチマークで、再構成ベースの漏えいと攻撃成功率を下げつつ、性能を競争的に維持することを示しています。



**新規性**
共有KVキャッシュを「潜在的な作業記憶」とみなし、敵対的再構成に基づいて安全性を定式化し、表現変換を学習することで安全な潜在通信を実現した点が新規です。
...
**日本語要約**
本研究は、紛争下の社会で利用されるLLMが、出力によって対立を悪化させうるかを検証した。OpenAI、Anthropic、DeepSeek、xAIの9つのモデル設定を90のマルチターンシナリオで評価し、虚偽の対称化、ジェノサイド否認、民族的スラ―の認識失敗などの不整合な挙動を確認した。



**新規性**
紛争文脈におけるLLMの安全性を評価する初の評価フレームワークを提示し、モデル選択自体が安全性上の重要な判断であることを示した。



**Tags:** LLM安全性, 紛争文脈, アライメント評価, モデル評価
...
**日本語要約**
本論文は、LLMが政治的に対立する話題を扱う際に、左右の立場で非対称な応答を示す「covert political bias」を指摘し、その操作手法を7カテゴリに整理している。さらに、レトリックとフレーミングの対称性を測るSentiment Consistencyと、応答の深さ・関与の対称性を測るHelpfulness Consistencyという2つの指標を提案している。



**新規性**
政治的バイアスの抑制に向けて、Sentiment Consistency TrainingとHelpfulness Consistency TrainingからなるPolitical Consistency Training（PCT）を導入し、ヘルプフルネスを保ちながら covert political bias を低減する点が新規である。



**Tags:** LLM, political bias, consistency training, reinforcement learning
...
**日本語要約**
本研究は、従来のセッション単位・ウィンドウ単位のログ異常検知では特定しにくかった、メッセージ単位の異常検知を扱う。FAMEは、各テンプレートから少数のラベル付き例を用いて学習し、LLMはオフラインで一度だけ利用することで、軽量なルータとドメイン別エキスパートによるオンプレミス推論を可能にする。



**新規性**
LLMを常時適用せずに、テンプレート分割の提案と検証を通じて failure domain を構成し、少量ラベルでメッセージレベル異常検知を実現した点が新規である。



**Tags:** ログ異常検知, メッセージレベル, Mixture-of-Experts, 大規模言語モデル, ラベル効率
...
**日本語要約**
本論文は、LLMのポストトレーニングを損失関数ではなく「どの状態（prompt＋生成prefix）に対して学習するか」という状態分布の観点から捉え直している。Qwen3-0.6B-Baseを用いた小規模実験では、SFTの強さによって性能向上と保持性能の低下が変わること、またon-policy distillationや軽量なon-policy RLがGSM8Kを改善しつつ保持性能を保てることを示した。



**新規性**
SFT、RL、OPDを状態分布のシェイピングとして統一的に扱い、学習信号の形式よりも学習状態の由来と局所性が重要であることを実験的に示した。
...
**日本語要約**
RLVRにおけるclippingベースのGRPO系目的関数では、hard clippingにより境界付近の有用な信号が捨てられ、学習不安定性や収束不良の一因になることを分析した。著者らは、境界近傍のわずかに範囲外のトークンを確率的に保持するNear-boundary Stochastic Rescue (NSR) を提案し、7B〜30Bの複数モデルとdense/MoEで安定性と性能の改善を示した。



**新規性**
hard clippingで失われる境界近傍の信号を、局所的な確率的復元によって回収するNSRを提案した点が新規である。



**Tags:** RLVR, LLM reasoning, gradient clipping, training stability, stochastic rescue
