...
Generated at: 2026-07-30 09:07 JST
Run ID: dd783456-7d2f-4b79-8012-899c32d83bc9
...
- PDF: [link](https://arxiv.org/pdf/2607.25947.pdf)
- Published: 2026-07-29 01:33 JST
- Updated: 2026-07-29 01:33 JST
...
**日本語要約**
不規則な臨床時系列（ICTS）に対する質問応答のために、ClinPRISMというコスト効率の高いマルチモーダルLLM推論フレームワークを提案している。疎・非同期・不規則な観測に対応するため、マルチスケールエンコーダ、時系列証拠の圧縮器、段階的アライメント戦略を組み合わせ、4BパラメータのLLMで高性能かつ低遅延を実現した。



**新規性**
不規則な臨床時系列の疎性と非同期性を扱うために、マルチスケール表現の抽出とLLM向けの少数トークン圧縮、さらに段階的なテキスト空間への整列を組み合わせた点が新規である。



**Tags:** clinical time series, question answering, multimodal LLM, irregular sampling, representation learning
...
- PDF: [link](https://arxiv.org/pdf/2607.25956.pdf)
- Published: 2026-07-29 01:41 JST
- Updated: 2026-07-29 01:41 JST
...
**日本語要約**
本研究は、マルチ倉庫在庫配分を対象に、インスタンスごとに適切なMIP定式化を選ぶ「OR定式化選択」をLLMで行う枠組みを提案する。提案法は、SFTでスキーマ学習を行った後、ソルバ評価に基づくIPOとGRPOで選択器を学習し、JD.comの実データで既存の固定定式化やSFT+IPOより高い配分品質を示した。



**新規性**
ソルバ評価に基づく損失差を用いてIPO/GRPOの報酬設計を行い、LLMをOR定式化選択に適用した点が新しい。



**Tags:** large language model, operations research, mixed-integer programming, inventory allocation, formulation selection
...
- arXiv: [2607.25933](https://arxiv.org/abs/2607.25933)
- PDF: [link](https://arxiv.org/pdf/2607.25933.pdf)
- Published: 2026-07-29 01:19 JST
- Updated: 2026-07-29 01:19 JST
...
**日本語要約**
本研究は、実臨床に近い段階的な情報開示や診断仮説の更新を含む多ターン・マルチモーダル診断評価の必要性を示し、そのためのベンチマークClinMM-Benchを提案する。ClinMM-Benchは1,089件の実症例と3,760枚の医用画像からなり、15の代表的MLLMを診断精度と推論品質の両面から評価した結果、最良モデルでも完全正答は限定的で、信頼できる診断推論にはなお課題があることを示した。



**新規性**
多ターンの情報更新を含む実世界臨床診断を評価する、これまでで最大規模のマルチモーダルベンチマークClinMM-Benchを構築し、精度だけでなく推論品質も評価した点が新規である。



**Tags:** multimodal LLM, clinical diagnosis, benchmark, multi-turn reasoning, medical imaging
...
- arXiv: [2607.25907](https://arxiv.org/abs/2607.25907)
- PDF: [link](https://arxiv.org/pdf/2607.25907.pdf)
- Published: 2026-07-29 01:01 JST
- Updated: 2026-07-29 01:01 JST
...
**日本語要約**
本研究は、推論時に内部活性を直接編集する代わりに、流暢な入力プロンプトを最適化して特定の内部潜在表現をゼロ方向へ抑制する「入力側」の手法を検討した。Llama-3.2-3B と Llama-3.1-8B で、評価意識に関わるとされる潜在を複数の定義で抑制できる一方、CA A方向についてはランダムな偽陽性対照でも同程度に抑制・行動変化が起き、可読性と行動制御は一致しないことが示された。



**新規性**
推論時の介入なしに、入力最適化だけで内部潜在の抑制を試み、評価意識ラテントの可読性と行動的因果性が必ずしも一致しないことを対照実験で示した点が新規である。



**Tags:** large language models, activation steering, prompt optimization, latent suppression, safety evaluation
...
### 5. Does Runtime Topology Context Improve LLM-Generated Kubernetes Security Patches?
- arXiv: [2607.25995](https://arxiv.org/abs/2607.25995)
- PDF: [link](https://arxiv.org/pdf/2607.25995.pdf)
- Published: 2026-07-29 02:12 JST
- Updated: 2026-07-29 02:12 JST
...
**日本語要約**
本論文は、KubernetesのKSPM指摘に対してLLMが生成する修正パッチに、実行時のサービス依存関係を含むクラスタトポロジー文脈を与えると、機能を壊さない修正の正確性が向上するかを検証している。Istioの呼び出し関係、Trivyの指摘、サービスアカウント結合から文脈を構築するKuTIEを提案し、36デプロイメントの医療クラスタVulnCareで248試行を行った結果、トポロジー依存の修正正確性が11.1%から78.0%に改善した。



**新規性**
ライブのサービス呼び出しグラフとサービスアカウント結合をLLMのパッチ生成に条件付けし、依存関係のあるKubernetes脆弱性修正における有効性を複数クラスで定量評価した点が新規である。



**Tags:** Kubernetes, LLM, security patching, runtime topology, cloud-native security
...
### 6. Knowledge-Guided Multimodal Reasoning over Interacting Streams for Video-Level Ambivalence and Hesitancy Recognition
- arXiv: [2607.25961](https://arxiv.org/abs/2607.25961)
- PDF: [link](https://arxiv.org/pdf/2607.25961.pdf)
- Published: 2026-07-29 01:44 JST
- Updated: 2026-07-29 01:44 JST
...
**日本語要約**
本研究は、動画レベルのambivalence/hesitancy（A/H）を、顔・音声・言語・身体モダリティ間の不一致として時間的に捉えるPRISM-AHを提案する。短時間窓ごとのクロスモーダルな不協和、次窓予測によるhesitation surprise、行動プロトタイプ、参加者メタデータを用いて判定し、さらに知識誘導型LLMで専門家キューに基づく推論を行う。公開テスト525本でmacro F1 0.6133を達成し、zero-shot baseline 0.2827を上回った。



**新規性**
複数モダリティの時間的な不一致を中核に据えたストリーミング推論と、データセットの専門家キュー分類に基づく知識誘導LLM推論を組み合わせ、A/H認識を改善した点が新規である。



**Tags:** multimodal learning, video understanding, affective computing, reasoning, large language model
...
### 7. MemLens: A Value-Aware Memory Management System with Interactive Analytics for LLM-based Agents
- arXiv: [2607.25992](https://arxiv.org/abs/2607.25992)
- PDF: [link](https://arxiv.org/pdf/2607.25992.pdf)
- Published: 2026-07-29 02:08 JST
- Updated: 2026-07-29 02:08 JST
...
**日本語要約**
LLMベースのエージェントにおける長期メモリ管理に対し、MemLensはユーザーとLLMの対話記録を一律に扱うのではなく、価値を考慮して管理するシステムを提案する。Shapley風のメモリ評価、価値に基づく保存、メモリ支援応答を含む対話的分析ダッシュボードを備え、メモリのライフサイクル全体を可視化できる。study-copilotアプリを通じて、メモリ価値の確認、階層構造の可視化、応答品質・検索遅延・トークン消費の比較が可能である。



**新規性**
メモリ記録を第一級データとして扱い、Shapley風評価を用いた価値認識型のメモリ管理と対話的分析を統合した点が新しい。



**Tags:** LLMエージェント, メモリ管理, 対話的分析, Shapley値, 長期記憶
...
### 8. Polistemics: Evaluating LLMs as Information Mediators in Politics & Elections
- arXiv: [2607.25953](https://arxiv.org/abs/2607.25953)
- PDF: [link](https://arxiv.org/pdf/2607.25953.pdf)
- Published: 2026-07-29 01:40 JST
- Updated: 2026-07-29 01:40 JST
...
**日本語要約**
LLMが選挙に関する政治情報を市民に媒介する際に、責任ある振る舞いをしているかを評価するための理論ベースのベンチマーク「Polistemics」を提案している。2025年のドイツ・オランダ選挙を対象に3つの最先端LLMを評価し、明確な証拠がある場合は比較的安定する一方、情報が欠落・曖昧・矛盾している場合に失敗しやすく、政治的言語の強度も平坦化することを示した。



**新規性**
政治情報の「再現」ではなく「媒介」としてLLMを評価する点に新規性があり、Epistemic Modestyに基づく理論的ベンチマークを導入した。



**Tags:** LLM評価, 政治情報, 選挙, ベンチマーク, 情報媒介
...
- arXiv: [2607.25881](https://arxiv.org/abs/2607.25881)
- PDF: [link](https://arxiv.org/pdf/2607.25881.pdf)
- Published: 2026-07-29 00:43 JST
- Updated: 2026-07-29 00:43 JST
...
**日本語要約**
本研究は、物理学・天体物理学・宇宙論の8件の研究課題について、人間研究者と3種類のLLMが作成した1ページのプロジェクト計画を、4人の人間査読者と2つの新しいLLM査読者が盲検評価することで、LLMの研究計画支援能力を調べた。人間査読者は人間作成とAI作成の提案を全体として同程度に評価した一方、AI査読者はAI作成の提案を一貫して高く評価し、AI査読者は全32件を正しく分類した。



**新規性**
人間作成とAI作成の研究提案を、複数の人間査読者と新しいLLM査読者で盲検比較し、AI査読者にAI生成提案への系統的な好みがあることを示した点が新規である。



**Tags:** 大規模言語モデル, 研究計画, 提案評価, 盲検評価, 科学研究支援
...
- PDF: [link](https://arxiv.org/pdf/2607.26057.pdf)
- Published: 2026-07-29 02:59 JST
- Updated: 2026-07-29 02:59 JST
...
**日本語要約**
On-policy distillationでは学生自身の軌跡に基づいてトークン単位の教師信号を与える一方、誤った推論方向に入ると以降の生成がずれてしまい、信頼できない監督と計算浪費が生じる。本研究は、失敗した接頭辞における教師と学生の継続の非対称性を利用して、ラベル不要のハンドオフトリガーを導入するRelay-OPDを提案し、教師が一時的に介入した後に学生へ戻すことで学習する。数学推論8ベンチマークで、標準OPDや強力なベースラインを上回り、学習軌跡長も50%以上削減した。



**新規性**
失敗した接頭辞での教師・学生の継続非対称性を、ラベル不要のトリガーとして用い、限定的な教師介入で軌跡を中継するRelay-OPDを提案した。



**Tags:** on-policy distillation, knowledge distillation, math reasoning, trajectory relaying, language model training
...
- PDF: [link](https://arxiv.org/pdf/2607.26023.pdf)
- Published: 2026-07-29 02:35 JST
- Updated: 2026-07-29 02:35 JST
...
**日本語要約**
本論文は、マルチモーダルグラフにおけるゼロショット転移を対象とするグラフ基盤モデルCHARMを提案する。CHARMは、階層的なグラフコンテキストを用いてモダリティ間の関係と共有概念を捉え、ターゲットドメインでの微調整なしに未知ドメインへ転移しやすくする。実験では、ゼロショットのマルチモーダルグラフタスクで一貫した性能向上を示した。



**新規性**
生のノード表現ではなく階層的なグラフコンテキストを導入し、マルチモーダル情報とグラフ構造を統合してゼロショット転移を可能にした点が新しい。



**Tags:** graph foundation model, multimodal graph, zero-shot transfer, hierarchical context modeling, large language model
...
### 12. Penelope: Localized Latent Recurrence for Efficient Structured Reasoning
- arXiv: [2607.25915](https://arxiv.org/abs/2607.25915)
- PDF: [link](https://arxiv.org/pdf/2607.25915.pdf)
- Published: 2026-07-29 01:06 JST
- Updated: 2026-07-29 01:06 JST
...
**日本語要約**
Penelopeは、事前学習済みのdecoder-only Transformerに対して、推論計算をデコーダの一部区間に局所化する効率的な潜在推論フレームワークです。下位層のprefixを一度だけ評価して境界メモリを作り、その後はGRUベースの反復更新で潜在空間内に追加計算を行い、長いCoTトークン列を生成せずに回答を得ます。



**新規性**
推論計算をデコーダ全体の再実行や長い可視CoTではなく、選択した狭いデコーダ区間の潜在反復更新に局所化した点です。



**Tags:** latent reasoning, decoder-only Transformer, structured reasoning, efficiency, chain-of-thought
...
- arXiv: [2607.25959](https://arxiv.org/abs/2607.25959)
- PDF: [link](https://arxiv.org/pdf/2607.25959.pdf)
- Published: 2026-07-29 01:43 JST
- Updated: 2026-07-29 01:43 JST
...
**日本語要約**
Wikipedia、Wikidata、表形式データの間で知識が食い違う場合に、その不整合を検出・説明する問題を扱っている。著者らは、粒度の違い、直接的な矛盾、時系列変化、KGの不完全性を含む不整合の分類体系を提案し、Text-to-SPARQLとLLM推論を用いる自動フレームワークKontrastを提示した。実験では、クロスモーダルな不整合が実際に多く存在し、知識の矛盾だけでなく欠損や時間的不一致も明らかになることを示した。



**新規性**
テキスト・表・知識グラフ間の不整合を対象に、分類体系と自動検出フレームワークKontrastを組み合わせて大規模な知識監査を可能にした点が新しい。



**Tags:** knowledge inconsistency, cross-modal, knowledge graph, table QA, Wikipedia
...
### 14. Instruction-Tuned Models Locally Reuse Human Syntax More Than Humans Do
- arXiv: [2607.26015](https://arxiv.org/abs/2607.26015)
- PDF: [link](https://arxiv.org/pdf/2607.26015.pdf)
- Published: 2026-07-29 02:27 JST
- Updated: 2026-07-29 02:27 JST
...
**日本語要約**
本研究は、置換パラダイムを用いて16のLlama/Gemma系モデルにおける構文的収束を調べ、全モデルで直前の人間発話に対するCFG規則の再利用が、無関係な人間プライムより大きいことを示した。さらに、低頻度の規則ほどこの差が大きく、探索的分析では語彙・意味的類似性も人間応答より高い傾向が見られた。



**新規性**
Instruction tuning により直前発話との実際の再利用は増える一方、無関係プライムへの重なりも増え、条件付き規則再利用の優位は小さくなることを、複数モデルで比較して示した点が新規である。



**Tags:** 構文的収束, 大規模言語モデル, instruction tuning, 構文再利用, 対話分析
...
### 15. SAM3D-Guided Object-Centric Representation Alignment for Vision-Language-Action Models
- arXiv: [2607.25912](https://arxiv.org/abs/2607.25912)
- PDF: [link](https://arxiv.org/pdf/2607.25912.pdf)
- Published: 2026-07-29 01:05 JST
- Updated: 2026-07-29 01:05 JST
...
**日本語要約**
本論文は、Vision-Language-Actionモデルに対して、対象物中心の3D表現整合を学習時に導入する手法を提案する。SAM3Dを凍結した3D教師として用い、対象物の密な3D表現をπ_0の中間特徴に整合させることで、遮蔽や姿勢変化、スケール変化に対する対象物理解を強化する。



**新規性**
学習時のみSAM3D由来の3D priorsを用いて対象物中心の3D情報を内部表現に取り込み、テスト時には深度・点群・マスク・追加3Dモジュールを不要にした点が新規である。



**Tags:** Vision-Language-Action, 3D表現学習, ロボット操作, SAM3D, 対象物中心
