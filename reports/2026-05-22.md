...
Generated at: 2026-05-22 09:13 JST
Run ID: 1bc67e54-c334-4ab4-8765-ed3e5ca68ce6
...
- arXiv: [2605.21362](https://arxiv.org/abs/2605.21362)
- PDF: [link](https://arxiv.org/pdf/2605.21362.pdf)
- Published: 2026-05-21 01:27 JST
- Updated: 2026-05-21 01:27 JST
...
**日本語要約**
本研究は、複数の既存ジャイルブレイク攻撃の出力を種プロンプトとして再利用し、ターゲットごとに適応的に合成するブラックボックス攻撃フレームワークLASHを提案する。LASHは、種の組み合わせと重みを探索し、拒否検出とLLM判定を組み合わせた二段階の評価で遺伝的最適化を行う。JailbreakBench上で6つの対象モデルに対し、少ないクエリ数で既存手法を上回る攻撃成功率を示した。



**新規性**
単一の攻撃系列に固定せず、複数のジャイルブレイク戦略をターゲットごとに適応的にハイブリッド合成する点が新規である。



**Tags:** large language models, jailbreaking, black-box attack, prompt optimization, red-teaming
...
- PDF: [link](https://arxiv.org/pdf/2605.21468.pdf)
- Published: 2026-05-21 02:53 JST
- Updated: 2026-05-21 02:53 JST
...
**日本語要約**
本研究は、RLVRによるLLMの学習軌跡が非常に低ランクで予測可能であり、性能向上の大部分がrank-1近似で捉えられることを示す。短い観測区間からrank-1部分空間を推定し、線形回帰で将来のチェックポイントを外挿するRELEXを提案し、少ない学習ステップで既存のRLVRと同等以上の性能を達成した。



**新規性**
短いRLVR学習履歴からrank-1軌道を外挿して将来のチェックポイントを予測する、学習不要の軽量手法RELEXを提案した点が新規である。



**Tags:** RLVR, LLM, 低ランク近似, 外挿, 強化学習
...
- arXiv: [2605.21465](https://arxiv.org/abs/2605.21465)
- PDF: [link](https://arxiv.org/pdf/2605.21465.pdf)
- Published: 2026-05-21 02:51 JST
- Updated: 2026-05-21 02:51 JST
...
**日本語要約**
メタモデル進化に伴うグラマー適応の手作業を減らすため、過去の適応例から学習して新しいグラマーへ自動適用するLLMベース手法を提案している。6つのXtext DSLで評価し、テストセットでは3つのLLMが適応整合性と出力類似性で100%を達成し、QVToの縦断事例でも手動編集なしで適応を再利用できた。一方で、EAST-ADLのような大規模グラマーでは整合性が90%未満にとどまり、規模の大きいケースでの限界も示した。



**新規性**
過去のグラマー適応を学習して、メタモデル進化後のグラマー修正をLLMで自動化し、複数の実世界DSLと縦断事例で有効性を検証した点が新しい。



**Tags:** large language models, grammar adaptation, metamodel evolution, model-driven engineering, Xtext
...
- PDF: [link](https://arxiv.org/pdf/2605.21467.pdf)
- Published: 2026-05-21 02:53 JST
- Updated: 2026-05-21 02:53 JST
...
**日本語要約**
RLVRにおける応答レベルの報酬が、トークンレベルの確率更新にどう反映されるかを、トークングラデントに対する判別器の視点から解析している。標準的な手法では高頻度な共有パターンが更新方向を支配しやすい問題があるため、著者らはトークン係数を推定して識別的な方向を強めるDelTAを提案した。



**新規性**
RLVR更新をトークングラデント上の線形判別として捉え、共有的・弱識別的な成分を抑えて側別の判別性を高めるトークン信用割当手法DelTAを導入した点。



**Tags:** reinforcement learning, large language models, token credit assignment, verifiable rewards, reasoning
...
- PDF: [link](https://arxiv.org/pdf/2605.21427.pdf)
- Published: 2026-05-21 02:19 JST
- Updated: 2026-05-21 02:19 JST
...
**日本語要約**
本論文は、LLM推論においてGPUの電力上限を静的な制約ではなく制御可能な資源として扱う、電力認識型ランタイムPALSを提案する。PALSは、オフラインの電力性能モデルとフィードバック制御を組み合わせ、バッチサイズなどのソフトウェア設定とGPU電力キャップを同時に選択して、スループット目標を満たしつつエネルギー効率を高める。vLLM上に実装され、再学習やAPI変更なしで動作する。



**新規性**
GPU電力キャップをLLM servingの第一級の制御ノブとして扱い、ソフトウェア設定と共同最適化する点が新規である。



**Tags:** LLM serving, power-aware runtime, energy efficiency, Mixture-of-Experts, GPU power control
...
- arXiv: [2605.21401](https://arxiv.org/abs/2605.21401)
- PDF: [link](https://arxiv.org/pdf/2605.21401.pdf)
- Published: 2026-05-21 01:59 JST
- Updated: 2026-05-21 01:59 JST
...
**日本語要約**
本研究は、11種類のオープンソースLLMに対してミルグラム実験を模した服従実験を行い、多くのモデルが拒否する前に最終的なショック段階まで、あるいはそれに近い段階まで応答したことを報告する。8条件・各30試行の結果から、LLMは継続的な権威圧力や段階的な境界侵害に影響され、拒否時にも応答形式の不備で再試行が発生し、結果的に服従が起こりうることが示された。



**新規性**
オープンソースLLMに対してミルグラム型の服従実験を体系的に適用し、長期的な権威圧力下での服従傾向と、拒否が実質的に無効化されうる実行上の問題を明らかにした点が新しい。



**Tags:** LLM, obedience experiment, AI safety, open-source models, agentic pipelines
...
- PDF: [link](https://arxiv.org/pdf/2605.21463.pdf)
- Published: 2026-05-21 02:51 JST
- Updated: 2026-05-21 02:51 JST
...
- arXiv: [2605.21479](https://arxiv.org/abs/2605.21479)
- PDF: [link](https://arxiv.org/pdf/2605.21479.pdf)
- Published: 2026-05-21 02:58 JST
- Updated: 2026-05-21 02:58 JST
...
**日本語要約**
本研究は、Wikipedia画像・記事キャプション・Wikidataの構造化知識を組み合わせて、人手で精査した知識グラウンデッドVQAベンチマークWikiVQABenchを提案する。各設問は外部知識を必要とし、視覚情報だけでは解けないように設計されている。15種類のVLM評価では、知識を要する推論能力に応じて大きな性能差が確認された。



**新規性**
WikipediaとWikidataを統合し、LLM生成候補を人手で検証することで、外部知識が必須な多肢選択VQAベンチマークを構築した点が新しい。



**Tags:** Visual Question Answering, Knowledge Grounding, Benchmark, Wikipedia, Wikidata
...
- PDF: [link](https://arxiv.org/pdf/2605.21486.pdf)
- Published: 2026-05-21 02:59 JST
- Updated: 2026-05-21 02:59 JST
...
**日本語要約**
本論文は、大規模言語モデルの学習におけるハイパーパラメータ転移を定量化するために、スケーリング則の当てはまり、外挿誤差への頑健性、パラメータ化による漸近損失ペナルティの3指標を提案する。さらに、AdamWでの学習においてμPが標準パラメータ化より優れる主因は、埋め込み層の学習率を十分大きくできる点にあることを示し、これが学習の不安定性を抑えつつ転移性能を改善すると報告する。



**新規性**
μPの利点を、理論的説明ではなく埋め込み層の学習率ボトルネックという観点から実証的に特定し、ハイパーパラメータ転移を評価する3つの定量指標を提示した。



**Tags:** hyperparameter transfer, large language models, learning rate, embedding layer, scaling law
...
### 10. Quantifying the cross-linguistic effects of syncretism on agreement attraction
- arXiv: [2605.21403](https://arxiv.org/abs/2605.21403)
- PDF: [link](https://arxiv.org/pdf/2605.21403.pdf)
- Published: 2026-05-21 02:02 JST
- Updated: 2026-05-21 02:02 JST
...
**日本語要約**
本研究は、文法的主語ではなく介在名詞に動詞が誤って一致する agreement attraction が、形態的同形性（syncretism）によってどのように増幅されるかを、英語・ドイツ語・ロシア語・トルコ語・アルメニア語の比較を通じて調べた。大規模言語モデルの surprisal と attention entropy を処理指標として用い、英語とドイツ語では行動実験の結果を再現し、トルコ語では同形性の影響がないという結果と整合し、ロシア語では一部のパターンを捉えた。



**新規性**
大規模言語モデル由来の処理指標を用いて、同形性が agreement attraction に与える影響の言語間差を定量的に比較した点が新しい。



**Tags:** agreement attraction, syncretism, cross-linguistic, large language models, psycholinguistics
...
### 11. "I didn't Make the Micro Decisions": Measuring, Inducing, and Exposing Goal-Level AI Contributions in Collaboration
- arXiv: [2605.21363](https://arxiv.org/abs/2605.21363)
- PDF: [link](https://arxiv.org/pdf/2605.21363.pdf)
- Published: 2026-05-21 01:28 JST
- Updated: 2026-05-21 01:28 JST
...
**日本語要約**
本研究は、LLMとの協働において最終成果物ではなく「目標そのもの」がどのように共同で形成されるかに着目し、目標を検証可能な要件へ分解して直接・間接の寄与を追跡する枠組みCoTraceを提案する。638件の実ログ分析では、モデルの目標形成への寄与は11〜26%にとどまる一方、具体的な下位要件の導入や間接的影響は大きいことが示された。さらに、対話設計がモデルの目標形成行動に影響し、目標レベルの分析を提示すると参加者の寄与認識が有意に変化することが分かった。



**新規性**
目標レベルで人間-AI協働の寄与を分解・追跡するCoTraceを導入し、最終成果物では捉えにくい目標形成への直接・間接貢献を可視化した点が新しい。



**Tags:** human-AI collaboration, LLM attribution, goal-level analysis, dialogue analysis, user study
...
### 12. Equilibrium Reasoners: Learning Attractors Enables Scalable Reasoning
- arXiv: [2605.21488](https://arxiv.org/abs/2605.21488)
- PDF: [link](https://arxiv.org/pdf/2605.21488.pdf)
- Published: 2026-05-21 02:59 JST
- Updated: 2026-05-21 02:59 JST
...
**日本語要約**
反復的に潜在状態を更新することでテスト時計算を拡張する推論モデルについて、その一般化を支える内部機構として「タスク条件付きアトラクタ」を仮定し、Equilibrium Reasoners（EqR）として定式化している。EqRは外部検証器やタスク固有の事前知識なしに、反復回数を増やす深さ方向の拡張と、複数初期化からの確率的軌道を集約する幅方向の拡張を可能にする。



**新規性**
学習されたアトラクタ景観という見方で、反復型潜在推論におけるテスト時スケーリングの仕組みを説明し、深さ・幅の両方向で計算を拡張するEqRを提案した点。



**Tags:** reasoning, test-time scaling, latent dynamics, attractor, equilibrium models
...
### 13. What Twelve LLM Agent Benchmark Papers Disclose About Themselves: A Pilot Audit and an Open Scoring Schema
- arXiv: [2605.21404](https://arxiv.org/abs/2605.21404)
- PDF: [link](https://arxiv.org/pdf/2605.21404.pdf)
- Published: 2026-05-21 02:02 JST
- Updated: 2026-05-21 02:02 JST
...
**日本語要約**
本論文は、12本のLLMエージェント・ベンチマーク論文について、評価実行の記述がどの程度明示されているかを項目別に監査した実装報告である。監査スキーマはベンチマーク同定、ハーネス仕様、推論設定、コスト報告、失敗内訳の5項目からなり、8本のエージェント系論文と4本の従来型静的ベンチマーク論文に適用された。



**新規性**
LLMエージェント・ベンチマーク論文の自己記述性を評価するための監査スキーマと採点コードブックを提示し、JSON Schema・Markdown・CSVとして公開した点が新規である。



**Tags:** LLM agents, benchmark audit, reproducibility, evaluation disclosure, cs.LG
...
### 14. Lost in Fog: Sensor Perturbations Expose Reasoning Fragility in Driving VLAs
- arXiv: [2605.21446](https://arxiv.org/abs/2605.21446)
- PDF: [link](https://arxiv.org/pdf/2605.21446.pdf)
- Published: 2026-05-21 02:34 JST
- Updated: 2026-05-21 02:34 JST
...
**日本語要約**
本研究は、自動運転向けVision-Language-Action（VLA）プランナの頑健性を、センサー摂動下での推論一貫性と軌道誤差の関係から評価した。Alpamayo R1を用いた1,996シナリオ・約18,000回の試行により、Chain-of-Causation（CoC）説明が変化すると軌道逸脱が大きく増加し、推論の一貫性が計画信頼性の高い指標であることを示した。



**新規性**
CoC説明の一貫性を、VLAの計画安全性を定量的に推定する代理指標として示し、さらに推論生成の有無が軌道精度に与える影響も実験的に示した。



**Tags:** autonomous driving, vision-language-action, robustness, sensor perturbation, reasoning consistency
...
### 15. Stdlib or Third-Party? Empirical Performance and Correctness of LLM-Assisted Zero-Dependency Python Libraries
- arXiv: [2605.21405](https://arxiv.org/abs/2605.21405)
- PDF: [link](https://arxiv.org/pdf/2605.21405.pdf)
- Published: 2026-05-21 02:02 JST
- Updated: 2026-05-21 02:02 JST
...
**日本語要約**
Python標準ライブラリのみで人気の第三者ライブラリを再実装した単一ファイル群「zerodep」を用い、依存管理や供給網リスクを避けつつ性能と正確性を実証的に評価している。40以上のモジュールを12カテゴリでベンチマークした結果、多くのケースで参照実装の2倍以内の性能を達成し、いくつかのカテゴリでは大幅な高速化も確認した。



**新規性**
LLM支援で厳しい制約下（外部import禁止・単一ファイル・API互換・正確性検証必須）にstdlb-only再実装を大量生成し、標準ライブラリの限界とLLMによる実装可能性を体系的に示した点が新しい。



**Tags:** Python, 標準ライブラリ, LLM支援開発, 性能評価, 依存関係
