...
Generated at: 2026-08-07 10:43 JST
Run ID: b1be2084-c9c9-4181-bca5-1d5a5c6290de
...
**日本語要約**
MetaboLLMは、代謝学の異種知識を統合するために継続事前学習、教師あり微調整、構造化検索を組み合わせた代謝学特化LLMであり、知識・関係・記述タスクでベースモデルや医療適応モデルを上回りました。さらに、生成した生化学記述を代謝物グラフに変換するMetaboLLM-GINにより、患者レベル予測で高い性能を示し、解釈可能な生物学的知見も得られました。



**新規性**
代謝学特化LLMで生成した生化学記述を代謝物グラフへ変換し、予測可能かつ解釈可能な表現として利用する枠組みを提案した点が新規です。
...
- PDF: [link](https://arxiv.org/pdf/2608.06123.pdf)
- Published: 2026-08-06 23:54 JST
- Updated: 2026-08-06 23:54 JST
...
**日本語要約**
本研究は、国際政治紛争における大規模言語モデル（LLM）の政治的バイアスを測るために、国名を入れ替えた反実仮想プロンプトを比較する枠組み「Poli-Bias」を提案する。応答の差を単一指標ではなく5つの解釈可能な次元に分解し、13種類のLLMで国名やユーザー属性が国際法上の評価や説明に影響しうることを示した。



**新規性**
国名を入れ替えた法的に等価な紛争シナリオの比較を通じて、政治的公平性とシコファンシーを細粒度に監査する反実仮想フレームワークを提示した。



**Tags:** large language models, political bias, counterfactual evaluation, international law, model auditing
...
### 3. Reducing belief in conspiracy theories as they unfold using large language models
- arXiv: [2608.06151](https://arxiv.org/abs/2608.06151)
- PDF: [link](https://arxiv.org/pdf/2608.06151.pdf)
- Published: 2026-08-07 00:18 JST
- Updated: 2026-08-07 00:18 JST
...
**日本語要約**
大規模言語モデルとの多ターン対話が、重大事件の直後に生じる陰謀論的信念を低減できるかを、米国成人を対象とした2つの実験で検証した。いずれの実験でも、LLM介入群は無関係な話題の対話群や静的な事実シート群よりも陰謀論的信念が有意に低下し、その後の別の危機事件に関する陰謀論信念にも下流効果が示された。



**新規性**
事件発生直後の「進行中の陰謀論」に対して、LLMとの対話を用いた認知的介入が信念低減に有効であることを実験的に示した点が新しい。



**Tags:** large language models, conspiracy theories, misinformation intervention, human-AI interaction
...
**日本語要約**
RLVRにおけるon-policy self-distillation（OPSD）は、教師モデルを用いて学生が訪れたprefixに密なトークンレベル監督を与える手法だが、従来法では生成中の時間的なずれの履歴を十分に反映できない。提案手法DASHは、各局所蒸留信号と系列平均の差から適応的な伝播ゲートを作り、バックワードな多段集約を制御することで、生成時の不一致の進み方に応じて監督重みを調整する。3つの数学推論ベンチマークと3つのモデル規模で、追加のforward passなしに既存のvanilla OPSDより一貫して性能向上を示した。



**新規性**
局所的な蒸留信号の履歴依存性を、系列平均との差に基づく適応ゲートと多段集約で反映し、OPSDのトークン重みを動的に調整する点が新しい。



**Tags:** reinforcement learning, self-distillation, reasoning models, token-level supervision, math reasoning
...
### 5. LLM Inference Under Bursty Workload Distribution: Modifying the WAIT Algorithm
- arXiv: [2608.06135](https://arxiv.org/abs/2608.06135)
- PDF: [link](https://arxiv.org/pdf/2608.06135.pdf)
- Published: 2026-08-07 00:07 JST
- Updated: 2026-08-07 00:07 JST
...
**日本語要約**
本研究は、LLM推論におけるスケジューリングで、実世界のバースト的で変動するリクエスト到着を考慮するため、WAITアルゴリズムを軽量に拡張した手法を提案する。提案法は、事前のトラフィック知識なしに到着間隔からリクエスト強度をオンライン推定し、シミュレーション評価では低い到着率シフト条件で既存手法より高いスループットを示しつつ、遅延は同程度に保った。



**新規性**
到着率が時間変動するバーストワークロードに対して、観測された到着間隔からオンラインで強度推定を行い、WAITを適応化した点が新規である。



**Tags:** LLM推論, スケジューリング, バーストワークロード, オンライン推定, 待ち行列制御
...
### 6. SAGA: Score-Weighted Adaptive Generation Alignment for Low-Resource Nordic Language Models
- arXiv: [2608.06179](https://arxiv.org/abs/2608.06179)
- PDF: [link](https://arxiv.org/pdf/2608.06179.pdf)
- Published: 2026-08-07 00:41 JST
- Updated: 2026-08-07 00:41 JST
...
**日本語要約**
本論文は、低資源で形態論的に豊かな北欧言語に対して、人手の嗜好ラベルの代わりに依存構文解析器の判定を用いるSAGAを提案する。SAGAは parser の評価を preference pair に変換し、文法品質と語彙多様性を組み合わせた報酬や reward-gap によるフィルタリングを行うことで、Danish・Icelandic・Norwegian Bokmål において文法的品質の改善を示した。



**新規性**
依存構文解析器の supervision を用いて human preference annotation を置き換え、delta-DPO のための学習信号を自動生成する点が新しい。



**Tags:** preference optimization, low-resource languages, dependency parsing, language model alignment
...
### 7. Contextual Information Policy Optimization for Search Agents
- arXiv: [2608.06128](https://arxiv.org/abs/2608.06128)
- PDF: [link](https://arxiv.org/pdf/2608.06128.pdf)
- Published: 2026-08-07 00:01 JST
- Updated: 2026-08-07 00:01 JST
...
**日本語要約**
検索エージェントは外部証拠を取り込みながら多段階推論を行えますが、既存手法は最終回答の正しさに偏り、取得した証拠をその後の推論にどう使ったかを十分に評価していません。本論文は、証拠に基づく行動にターン単位の密な報酬を与えつつ全体の正解報酬も併用するCIPOを提案し、証拠から切り離された推測や確認バイアスを抑えることを目指します。



**新規性**
取得した外部証拠の利用そのものを明示的に最適化対象に含め、プロセス注釈や追加の報酬モデルなしで証拠利用を促す強化学習枠組みを提案した点が新規です。



**Tags:** search agents, reinforcement learning, evidence use, reasoning, large language models
...
**日本語要約**
本論文は、ポストトレーニング適応技術（再学習、微調整、検索拡張、モデル編集、unlearning など）を整理するサーベイであり、機構・目的・データ要件・持続性・構造範囲・モデル種別の6次元からなる分類体系を提案する。さらに、微調整、検索拡張、プロンプトなどの混同されやすい用語を区別し、技術の関係性やAIガバナンスへの応用可能性を示す。



**新規性**
ポストトレーニング適応を6次元で体系化し、技術比較やモデル変更の記録、ガバナンス分析に使える共通語彙を提示した点が新しい。
...
**日本語要約**
EnvACEは、LLMエージェントの長期的なツール使用学習において、外部環境とのやり取りの代わりに「world rehearsal」を用いる強化学習手法です。方策は行動生成と環境応答の再現を交互に行い、その両方をタスク成功報酬で共同最適化することで、行動と環境応答の関係をパラメータ内に内在化します。BFCL-v4、tau^2-Bench、VitaBench、FinMCP-Benchで強い性能と移植性を示し、テスト時の私的な事前リハーサルでも追加の外部相互作用なしに性能向上が得られます。



**新規性**
外部シミュレータや実環境に依存せず、方策自身が環境応答を再現するworld rehearsalを通じてエージェントの内部世界モデルを学習させる点が新規です。
...
- PDF: [link](https://arxiv.org/pdf/2608.06183.pdf)
- Published: 2026-08-07 00:43 JST
- Updated: 2026-08-07 00:43 JST
...
**日本語要約**
MicroEvoは、マイクロアーキテクチャ設計空間探索における広大な探索空間と高コストなPPA評価の問題に対し、LLMとMCTSを組み合わせた知識誘導型フレームワークを提案する。LLM駆動の進化演算、Pareto寄与と多様性を考慮する木探索方策、知識蓄積機構、状態に応じた指示を統合し、限られたシミュレーション予算での探索効率向上を目指す。



**新規性**
オフ・ザ・シェルフのLLMをMCTSと結合し、Pareto意識の木方策と知識の再利用を組み込んで、マイクロアーキテクチャ探索を効率化した点が新しい。



**Tags:** microarchitecture design, LLM, MCTS, Pareto optimization, design space exploration
...
### 11. Mind the Gaps: Mixture-of-Minds for Human Simulation
- arXiv: [2608.06115](https://arxiv.org/abs/2608.06115)
- PDF: [link](https://arxiv.org/pdf/2608.06115.pdf)
- Published: 2026-08-06 23:48 JST
- Updated: 2026-08-06 23:48 JST
...
**日本語要約**
本論文は、集団の平均的傾向は捉えられても個人差を平坦化しがちなLLMベースの人間シミュレーションに対し、狭い対象領域で個人レベルの予測を行うAudience Simulation Model「Anacreon」を提案する。Anacreonは著者埋め込みで個人を分離し、実在の質的コーパスを種人物ごとにクラスタリングして、各クラスタに専用アダプタを学習する。また、人口統計・心理特性・アンケート応答に加えてchain-of-emotionを利用し、選択肢のシャッフルや学習分布の平衡化でプロンプト脆弱性と正のバイアスを抑える。



**新規性**
個人を分離する著者埋め込みとクラスタごとの専用アダプタからなるmixture-of-minds構成により、個人レベルの人間シミュレーション精度を高めた点が新規である。



**Tags:** human simulation, large language models, mixture of experts, individual-level prediction, survey modeling
...
**日本語要約**
LLMベンチマーク評価は安全性や信頼性の根拠として使われる一方、実運用でのモダリティ差やWeb検索の有無、複数回実行時のばらつきは十分に考慮されていない。本研究はChatGPTのチャットUIとAPIを、Web検索の有無を含めてBBQとSafetyBenchの401プロンプトで比較し、正確性だけでなく一貫性、類似度、引用の根拠付け、棄権行動も分析した。その結果、UIとAPIで精度差があり、Web検索で精度が最大8ポイント低下し、同一プロンプトでも最大21%で応答が不一致だった。



**新規性**
同一モデル परिवार内でも、モダリティ・検索条件・反復実行によって応答特性が大きく変わることを、精度以外の指標も含めて体系的に示した点が新しい。



**Tags:** LLM benchmark, AI safety evaluation, modality, web search, response consistency
...
### 13. Learning Globally Reusable Skills for Coding Agents
- arXiv: [2608.06153](https://arxiv.org/abs/2608.06153)
- PDF: [link](https://arxiv.org/pdf/2608.06153.pdf)
- Published: 2026-08-07 00:19 JST
- Updated: 2026-08-07 00:19 JST
...
**日本語要約**
本研究は、LLMベースのコーディングエージェントが追加学習なしで継続的に技能を改善するための技能進化手法GSEを提案する。GSEは技能間の関係を表すSkill Relation Graphを維持し、クラスタベースの技能統合と再実行による検証を通じて、局所的な更新の過学習や行動退化を抑えつつ一般化可能な技能を獲得する。



**新規性**
技能を個別に更新するのではなく、技能間関係の共同最適化とクラスタ統合・再実行検証を組み合わせて、再利用可能な技能へと進化させる点が新しい。



**Tags:** LLMエージェント, 技能進化, コード生成, ソフトウェア工学, 一般化
...
**日本語要約**
本研究は、時系列予測にRAG（Retrieval Augmented Generation）を適用するTS-RAGを提案している。入力系列に対して類似する時系列を検索し、その情報を参照しながら予測精度の向上を図る。



**新規性**
単純な参照系列の連結ではなく、入力系列と検索した類似系列の情報を効果的に融合するための専用reference tokensを導入した点が新しい。



**Tags:** time series forecasting, retrieval augmented generation, transformer, reference tokens
