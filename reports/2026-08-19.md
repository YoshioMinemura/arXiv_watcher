...
Generated at: 2026-08-19 08:32 JST
Run ID: 3f1abf59-3187-47ff-9c4e-70a3b0813357

## Summary
- Queries executed: 1
- New matched papers: 8
...
### 1. Le Critique: Privileged Value Functions for LLM Reinforcement Learning
- arXiv: [2608.16739](https://arxiv.org/abs/2608.16739)
- PDF: [link](https://arxiv.org/pdf/2608.16739.pdf)
- Published: 2026-08-18 00:49 JST
- Updated: 2026-08-18 00:49 JST
...
**日本語要約**
本論文は、LLMの強化学習における分散低減とクレジット割り当ての課題に対し、学習済み価値関数を用いる手法を検討している。特に、トークンレベルの信号を追加できるPrivileged Value Functions（PVF）と、価値関数の精度に応じて群平均との差分ベースラインと価値ベースラインを切り替えるTETHERを提案し、複数の推論タスクで既存の価値関数ベースラインを一貫して上回ることを示した。



**新規性**
PVFにより方策目的を偏らせずにタスク関連のトークンレベル信号を注入できる点と、TETHERが価値関数の精度に応じてベースラインを適応的に補間する点が新規である。



**Tags:** LLM, reinforcement learning, value function, variance reduction, reasoning tasks
...
- PDF: [link](https://arxiv.org/pdf/2608.16806.pdf)
- Published: 2026-08-18 02:02 JST
- Updated: 2026-08-18 02:02 JST
...
**日本語要約**
LLM駆動の具現化エージェントでは、言語モデルが環境状態、物体属性、空間関係、実行フィードバックを統合してタスクを地に足のついた形で遂行し、生成した行動計画をスキルライブラリやモーションプランナ、コントローラに渡して実行します。従来のLLMエージェントがWebページや文書、外部ツールを主な情報源としていたのに対し、本論文はロボットシステムへの統合により、状態理解と高レベル計画が重要になることを背景として述べています。



**新規性**
LLM駆動の具現化エージェントにおいて、状態そのものが攻撃面になりうるという観点から、state-semantic injection という新たな脅威を提示している点です。



**Tags:** LLMエージェント, 具現化AI, ロボティクス, セキュリティ, 状態注入
...
- PDF: [link](https://arxiv.org/pdf/2608.16742.pdf)
- Published: 2026-08-18 00:52 JST
- Updated: 2026-08-18 00:52 JST
...
**日本語要約**
大規模言語モデルによるコード生成では、特にリポジトリ規模の課題で正しさを保証することが難しいため、本論文はテスト駆動開発をコード生成に適用するTDD-Agentを提案する。まず実行可能なテストを生成して期待動作を明確化し、その後、実行フィードバックを用いてコードとテストの両方を反復的に改善する。LiveCodeBenchではtest-firstなプロンプトが改善を示し、RepoEvalでは既存のretrieval-basedおよびagent-based手法より一貫して高い性能を示した。



**新規性**
テストを固定的な検証器ではなく、コードとともに反復的に改善される推論アーティファクトとして扱い、test-first生成と双方向の反復改善を組み合わせた点。



**Tags:** code generation, test-driven development, large language models, repository-level tasks
...
- PDF: [link](https://arxiv.org/pdf/2608.16775.pdf)
- Published: 2026-08-18 01:21 JST
- Updated: 2026-08-18 01:21 JST
...
**日本語要約**
本研究は、サイバーインシデントログ分析におけるLLM出力の信頼性向上を目的に、検索されたログと生成応答の幾何学的関係を捉える Topological Attribution Distance (TAD) を提案する。TADは、セグメント単位のアブレーション帰属を用いて、実際のサイバー攻撃ログに対してどのログが出力に強く影響したかを適応的に特定し、証拠検証と出所追跡を支援する。



**新規性**
トポロジーに着想を得て、検索証拠とLLM出力の「全体的な幾何学的形状」の変化から、セグメントレベルで重要なログを特定する点が新規である。



**Tags:** LLM, サイバーセキュリティ, RAG, 説明可能AI, 証拠帰属
...
### 5. GRIP: Grounded Reasoning via Information-Restricted Premises
- arXiv: [2608.16776](https://arxiv.org/abs/2608.16776)
- PDF: [link](https://arxiv.org/pdf/2608.16776.pdf)
- Published: 2026-08-18 01:23 JST
- Updated: 2026-08-18 01:23 JST
- Authors: Lirui Teng
- Categories: cs.AI
- Score: 16.25
- Match reasons:
...
**日本語要約**
RAGにおいて高容量エンコーダがクエリを潜在状態で支配してしまい、検索した証拠が実質的に無視される「query dominance」という失敗モードを指摘している。提案手法GRIPは、クエリには十分な容量を残しつつ証拠側に強い確率的ボトルネックを課すことで、証拠をクエリから得られない残差情報のみに制限する。5つの推論ベンチマークで強い反復ベースラインを上回り、幻覚を73%削減した。



**新規性**
クエリと検索証拠の情報容量を非対称に設計し、証拠側を情報制限することでquery dominanceを抑えるGRIPを提案した点が新規である。



**Tags:** retrieval-augmented generation, reasoning, hallucination reduction, information bottleneck, grounded generation
...
### 6. Semantic Bandits: In-Context Exploration-Exploitation is Biased by Semantic Priors
- arXiv: [2608.16707](https://arxiv.org/abs/2608.16707)
- PDF: [link](https://arxiv.org/pdf/2608.16707.pdf)
- Published: 2026-08-18 00:25 JST
- Updated: 2026-08-18 00:25 JST
...
**日本語要約**
本研究は、行動ラベルの意味情報を明示的に考慮した「semantic bandit」を導入し、LLMエージェントの探索・活用バランスが自然言語由来の意味的事前分布に影響されることを調べた。実験の結果、意味的に情報を持つラベルは整合的な場合に性能を改善する一方、ずれる場合には探索を抑えて性能を大きく悪化させることが示された。



**新規性**
多腕バンディットに行動ラベルの意味を組み込んだsemantic banditを提案し、LLMの探索行動が事前学習由来の意味的バイアスで系統的に偏ることを示した。



**Tags:** LLM, 探索と活用, マルチアームドバンディット, 意味的バイアス, 意思決定
...
### 7. Would this change your answer? Evaluating Explanations of LLM Behavior In The Wild with Counterfactual Experiments
- arXiv: [2608.16747](https://arxiv.org/abs/2608.16747)
- PDF: [link](https://arxiv.org/pdf/2608.16747.pdf)
- Published: 2026-08-18 00:57 JST
- Updated: 2026-08-18 00:57 JST
...
**日本語要約**
本研究は、LLMの振る舞いに対する説明の良し悪しを、関連する反事実入力での予測可能性（counterfactual simulatability）という観点から評価する。自然発生的なモデル挙動を見つけて反事実的なプロンプト編集で検証するエージェント型パイプラインCHIVEを提案し、これにより高品質な説明と反事実証拠を大量に生成する。



**新規性**
自然に起きるLLMの挙動を自動発見し、反事実実験で説明を検証するCHIVEを導入した点が新しい。



**Tags:** LLM解釈性, 反事実実験, 説明評価, プロンプト編集, エージェント
...
### 8. MIRROR: Multimodal Intelligent Radiology Reasoning and Observation Reporter
- arXiv: [2608.16709](https://arxiv.org/abs/2608.16709)
- PDF: [link](https://arxiv.org/pdf/2608.16709.pdf)
- Published: 2026-08-18 00:25 JST
- Updated: 2026-08-18 00:25 JST
...
**日本語要約**
MIRRORは、放射線科モデルの出力を「分類器」「Grad-CAMによる局在化」「画像を見ない報告文生成」に分離することで、モデルが示していない所見を文章側が勝手に付け足す問題を抑える研究プロトタイプである。胸部X線、脳MRI、頭部CTに対応する共通レジストリを用意し、所見の確率ベクトルに対して報告文の内容を監査可能にしている。ChestMNISTではmacro AUROC 0.729を達成した一方、クラス不均衡下ではBrier scoreのような集計指標が実態を過大評価しうることも示した。



**新規性**
画像を見ない言語生成器に分類結果と局在情報だけを渡す構成により、報告文が分類器の出力を超えて所見を捏造しにくいようにした点が新規である。



**Tags:** medical imaging, radiology report generation, multimodal learning, interpretability
