...
Generated at: 2026-06-10 09:22 JST
Run ID: da9967a8-01b9-436f-9278-4a9d1e92f3ca
...
### 1. The Neutral Mask: How RLHF Provides Shallow Alignment while Leaving Partisan Structure Intact in a Large Language Model
- arXiv: [2606.09735](https://arxiv.org/abs/2606.09735)
- PDF: [link](https://arxiv.org/pdf/2606.09735.pdf)
- Published: 2026-06-09 02:00 JST
- Updated: 2026-06-09 02:00 JST
- Authors: Wendy K. Tam
- Categories: cs.CL
- Score: 35.50
- Match reasons:
...
**日本語要約**
本研究は、Llama 3.1 8B のベースモデルと RLHF 後モデルを比較し、RLHF が党派的な内部構造そのものを消すのではなく、出力を一貫して中立的に見せるように変えることを示した。Sparse autoencoder と特徴操作実験により、ベースモデルにある党派性を表す信号は Instruct モデルでは非活性化される一方、内部の党派的幾何構造は残っていることが示された。



**新規性**
RLHF が「価値の除去」ではなく、党派性の内部表現と出力の因果経路を切断することで表面的な中立性を実現していることを、機械論的に示した点が新しい。



**Tags:** RLHF, alignment, partisanship, mechanistic interpretability, large language models
...
### 2. PsychoSafe: Eliciting Psychologically-Informed Refusals in Large Language Models
- arXiv: [2606.09697](https://arxiv.org/abs/2606.09697)
- PDF: [link](https://arxiv.org/pdf/2606.09697.pdf)
- Published: 2026-06-09 01:19 JST
- Updated: 2026-06-09 01:19 JST
...
**日本語要約**
大規模言語モデルの拒否応答を、単なる不応答ではなく、危機・強要・エスカレーションを含む高リスク対話で有用な支援的コミュニケーションとして扱う枠組みPsychoSafeを提案している。8019件のデータでQwen 3.5 27Bに対してプロンプト設計とパラメータ効率の高い微調整を行い、拒否品質の向上や外部資源への誘導、心理的な配慮の改善を報告している。



**新規性**
拒否を心理学的に根拠づけた構造化支援応答として再定義し、証拠に基づく介入戦略を用いてLLMに学習させた点が新しい。



**Tags:** large language models, refusal behavior, psychological safety, fine-tuning, crisis intervention
...
- PDF: [link](https://arxiv.org/pdf/2606.09669.pdf)
- Published: 2026-06-09 00:51 JST
- Updated: 2026-06-09 00:51 JST
...
**日本語要約**
SpatialWorldは、マルチモーダルエージェントの相互作用的な空間理解を、実世界に近い複雑なタスクで評価するための統一ベンチマークです。8種類のシミュレーション基盤を共通プロトコルで統合し、760件の人手注釈タスクを用いて、視覚のみの部分観測下での探索とテキストベース行動による解決を求めます。15モデルの評価では、最良モデルでもタスク成功率は17.4%にとどまり、能動的探索と長期計画の難しさが示されました。



**新規性**
既存の静的VQAやシミュレータ依存評価ではなく、複数バックエンドを統一した相互作用的な空間推論ベンチマークを提案した点が新規です。



**Tags:** multimodal agents, spatial reasoning, benchmark, interactive evaluation, simulation
...
- arXiv: [2606.09709](https://arxiv.org/abs/2606.09709)
- PDF: [link](https://arxiv.org/pdf/2606.09709.pdf)
- Published: 2026-06-09 01:31 JST
- Updated: 2026-06-09 01:31 JST
...
**日本語要約**
長文生成においてLLMは2,000語を超えると性能が急激に低下する「length collapse」を示し、その原因を静的な階層計画の限界にあると分析している。これに対し、生成過程へ動的なPlan-Write-Reflectサイクルを組み込むIS-CoTを提案し、長文の一貫性と長さ遵守を改善した。さらに、複数教師パイプラインで構築した推論トレースデータでIS-Writer-8Bを学習し、長文ベンチマークで強い性能を示した。



**新規性**
外部エージェントに頼らず、生成中に動的なPlan-Write-Reflectを交互に行うIS-CoTを導入し、長文生成の崩壊を抑えた点が新規である。



**Tags:** 長文生成, Chain-of-Thought, LLM, 一貫性, 計画と反省
...
- PDF: [link](https://arxiv.org/pdf/2606.09821.pdf)
- Published: 2026-06-09 02:58 JST
- Updated: 2026-06-09 02:58 JST
...
**日本語要約**
大規模言語モデルの強化学習では、学習と推論のずれや方策の古さにより、安定化のための信頼領域制御が重要です。本論文は、比率クリッピングやDPPOのハードマスクが分布シフトを十分に扱えない点に着目し、方策変化に対する滑らかな二次正則化を用いるDRPOを提案しています。



**新規性**
DPPOのハードマスクを、優位度で重み付けされた滑らかな二次正則化に置き換え、境界外でも勾配を遮断せず補正信号を与える点が新規です。



**Tags:** LLM, reinforcement learning, trust region, policy optimization
...
- arXiv: [2606.09700](https://arxiv.org/abs/2606.09700)
- PDF: [link](https://arxiv.org/pdf/2606.09700.pdf)
- Published: 2026-06-09 01:21 JST
- Updated: 2026-06-09 01:21 JST
...
**日本語要約**
LLMベースのコンテンツモデレーションは主にトークン化されたテキストに依存するため、人間が視覚的手がかりで有害と認識できる表現を見落とす脆弱性があることを示した。著者らは、空白・強調・配置などのタイポグラフィ操作で有害表現を埋め込むHuman-Perceptible Adversarial Attacks (HPAA)を提案し、少ないクエリでブラックボックス環境でも回避可能であることを複数のデータセットと10のモデレーションシステムで検証した。



**新規性**
人間には有害と読める一方で機械には検出されにくいタイポグラフィベースの攻撃クラスHPAAを定義し、ブラックボックス・少クエリで自動生成できることを示した点が新しい。



**Tags:** adversarial attack, content moderation, LLM security, human perception
...
### 7. SearchSwarm: Towards Delegation Intelligence in Agentic LLMs for Long-Horizon Deep Research
- arXiv: [2606.09730](https://arxiv.org/abs/2606.09730)
- PDF: [link](https://arxiv.org/pdf/2606.09730.pdf)
- Published: 2026-06-09 01:52 JST
- Updated: 2026-06-09 01:52 JST
...
**日本語要約**
本研究は、長期的な深層調査タスクにおいて、主エージェントがタスクを分解し、必要に応じてサブエージェントへ委任して結果を統合する「delegation intelligence」を獲得させる枠組みを提案する。著者らは、委任の判断や結果の返却方法を誘導するハーネスを設計し、その軌跡を教師あり微調整データとして用いてモデルに学習させた。



**新規性**
自然文からは得にくい委任行動の学習データを、ハーネスで合成してSFTに利用し、委任知能をモデル重みに内在化させた点が新しい。



**Tags:** agentic LLMs, task delegation, long-horizon reasoning, deep research, supervised fine-tuning
